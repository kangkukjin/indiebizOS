...
```
source    (105개 액션) - 데이터 검색 (웹, 금융, 사진, 블로그, 기억, 건강)
interface  (79개 액션) - UI 자동화 (브라우저 / 안드로이드 / macOS 데스크탑)
system     (64개 액션) - 시스템 관리, 워크플로우, 파일, 알림, 코드 실행
forge      (46개 액션) - 콘텐츠 생성 (슬라이드, 동영상, 차트, 이미지, 음악, 웹사이트)
stream     (18개 액션) - 미디어 재생 (유튜브 / 인터넷 라디오)
...
- **용례 RAG** - 과거 성공한 IBL 사용 사례를 검색하여 참조로 주입, 에이전트 시행착오 감소 (13라운드 → 3라운드)
...
### IBL 용례 RAG (경험으로부터 학습)

에이전트는 성공적인 IBL 실행을 학습하는 RAG(Retrieval-Augmented Generation) 시스템을 통해 점점 더 정확해집니다:

- **용례 사전**: ~970개 (합성 데이터 + 자동 승격된 실행 로그)
- **하이브리드 검색**: 70% 시맨틱 (ko-sroberta, 768차원) + 30% BM25 (FTS5)
- **프롬프트 주입**: 사용자 메시지 수신 시 유사 용례 3개를 XML 참조로 주입
- **자동 학습**: 성공한 도구 실행이 자동으로 용례로 승격
- **자동 생성**: 새 패키지 설치 시 기본 용례를 자동 생성

```
사용자: "대전 유성구 아파트 가격 검색해줘"
         ↓
    [RAG 검색] → 유사 과거 용례 발견
         ↓
    <ibl_references>
      <ref intent="아파트 매매 실거래가" code='[source:apt_trade]("지역코드")' score="0.88"/>
         ...
         ↓
    AI가 첫 시도에 올바른 IBL 생성 (시행착오 없음)
```

**실측 효과**: 부동산 조회 기준 에이전트 라운드 수 13회 → 3회로 감소.

### 자기완결적 액션 패키지 (35개 설치됨)

각 패키지는 **자기완결적**입니다: 자체 `ibl_actions.yaml`로 제공하는 IBL 액션을 선언합니다. 패키지 설치 시 해당 액션이 `ibl_nodes.yaml`에 **자동 등록**되고 RAG 시스템에 기본 용례가 **자동 생성**됩니다. 제거 시 액션이 **자동 해제**됩니다:

- **수동 YAML 편집 불필요** — 패키지 설치만 하면 액션이 즉시 사용 가능
- **깨끗한 제거** — 패키지 제거 시 해당 액션만 정확히 삭제
- **즉시 RAG 커버리지** — 새 액션이 설치 즉시 기본 용례를 가짐 (콜드 스타트 없음)
...
│   ├── ibl_usage_db.py  # IBL 용례 사전 DB + 하이브리드 검색 엔진
│   ├── ibl_usage_rag.py # RAG 참조 모듈 (검색 → XML → 프롬프트 주입)
│   ├── ibl_usage_generator.py # 합성 용례 데이터 생성기 + 패키지 자동 생성
│   ├── package_manager.py # 패키지 설치/제거 (액션 + 용례 등록 트리거)
...
- 24개의 활성 프로젝트 운영 중
- 6개 IBL 노드, 321개 원자적 액션
- 35개 액션 패키지 + 9개 확장 패키지 설치됨
...
- IBL 용례 RAG (~970개 용례) — 하이브리드 검색(시맨틱 + BM25)으로 참조 주입
- 성공한 실행의 자동 학습 + 패키지 설치 시 용례 자동 생성
