...
The seam keeps them apart. `runs_on` tags mark each action honestly — `anywhere`, or bound to a machine that actually has the capability — and an action a body can't perform is **forwarded to one that can, transparently**, each side carrying its credential with no manual login. Nothing in the vocabulary names a brand: your hub can be a Mac or a Windows PC, and an action that reaches for a camera says *camera*, not *phone*.
