...
Subscription doesn't stay anonymous (level 0), either. Register the id and password you created when signing up at a neighbor's warehouse, and the poller logs in through the manifest's `login` contract — **files at your granted level flow into your feed and search.** Expired or revoked cookies re-login automatically, and if login fails the poll continues anonymously (the feed never stops). There is no separate key exchange: the credentials you made at signup *are* the key.
