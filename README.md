...
### → The shared warehouse — the node's front door, and a two-way network

The surfaces above are *destinations you make*. The **shared warehouse (공유창고)** is the node itself having an address — and, unlike everything above, it has a **reading half** too, so nodes don't just publish at each other, they subscribe.

**Publishing is a folder.** `공유창고/0/ … 공유창고/4/` on your disk are served live at your node's bare root. You put things in with Finder, drag-and-drop, or an IBL line — and that's the whole publishing act. There is **no index and no manifest file**: every request walks the directory fresh, so the filesystem *is* the truth. And nothing is converted — a PDF is served as a PDF, a spreadsheet as a spreadsheet. **Schema-on-read: the warehouse serves bytes; interpretation is the reader's job** (the *asker pays*, which is why serving costs the owner ~nothing).

**The number on the folder is the gate.** A visitor's level (0 → 4) is the same scale as their grade in your neighbor CRM — promote someone once, and it applies to messaging, portal membership, and the warehouse alike. A viewer sees levels `0..their level` **merged into one flat list**, highest wins, so they never learn which file sat at which level; a file above their level returns **404, not 403** (existence itself is information). The one thing that does leak is a single `has_restricted` flag — a deliberate *smell*: "there is more here," with no names or counts. Knowing the address is not enough; unlike the `/s/` shares, the warehouse's security is the level, so the front door is confidently left open.

**`/manifest` is the machine face.** Beside the human page, the same walk is served as JSON — a title, the file list with sizes and mtimes and direct URLs, `has_restricted`, and a self-describing `about` plus a **`login` block** stating exactly how to authenticate. So a *foreign AI agent* can read your node without scraping HTML or reverse-engineering anything, which is the whole point of giving a node an address.

**The reading half: a neighbor feed.** Register a neighbor's warehouse and a background poller fetches their `/manifest` every 30 minutes, diffs paths and mtimes, and turns the difference into a timeline — `seed` on the first poll (their back catalogue, like seeing past posts right after a follow), then `new` and `changed`; files that vanish leave quietly, with no event. The retained snapshot doubles as a **filename index of your whole neighborhood**, so one search sweeps every neighbor's warehouse at once. This layer is **pure machine — no model, zero tokens**: collecting the scent is mechanical, interpreting it is the AI's job.

**A warehouse address is a way to reach a person.** It isn't a separate address book — it's stored as a *contact type* on a neighbor, right next to their email and Nostr key. One neighbor can have several warehouses, and someone you only know by address is still a proper neighbor (register with no name and the node names them from their own manifest).

**Reposting is a file, not a protocol.** Repost a neighbor's item and a standard `.url` shortcut lands in your own warehouse folder. When your own manifest is served, that pointer resolves to the original — a subscriber's click goes straight to the origin warehouse, never through you, and a repost-of-a-repost still points at the source. **Friend-of-a-friend discovery reduced to a file anything can read**, and the durable hop is the *warehouse address*, which outlives any individual file.

Notably, **this cost the language zero new verbs.** Publishing composes out of vocabulary that already existed (`self:copy`, `table:document`, `self:delete`); the outermost discovery ring — a Nostr profile whose `website` is your warehouse, plus an `#IndieNet` note carrying only a greeting and the address — is `[self:business_document]{op: "publish"}`. A stranger finds you on an open relay, then reads *you* out of your own warehouse.
