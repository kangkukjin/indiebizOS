...
_PIPELINE_TEMPLATES = [
    ...
    (
        ...
        '[sense:search]{source: "gnews", query: "오늘 뉴스"} >> [others:channel_send]{channel: "telegram"}',
        "sense,others", "pipeline"
    ),
    # Fallback 패턴
    (
    ...
    (
        "구글 홈페이지 열어줘",
        '[limbs:browser]{op: "navigate", url: "https://google.com"}',
        "limbs", "single"
    ),
    (
        "화면 스크린샷 찍어줘",
...
def load_nodes_yaml() -> dict:
    """ibl_nodes.yaml 로드"""
    yaml_path = Path(__file__).parent.parent.parent / "data" / "ibl_nodes.yaml"
