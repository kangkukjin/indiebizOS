"""row_conditions.py — table:filter·self:ledger가 공유하는 조건 언어 + 정렬 키.
...
def _match(item, where, *, get_value=None):
    """item(dict) 이 where 조건을 만족하나.

    get_value는 저장 표면의 필드 접근만 주입한다(기본 dict.get, ledger는 점 경로).
    조건 파싱·논리 결합·값 비교는 이 모듈 한 벌을 공유한다.
    ...
    if isinstance(where, list):
        return all(_match(item, w, get_value=get_value) for w in where)
    if isinstance(where, dict):
        ...
        if field is not None:  # 구조형 {field, op, value}
            ...
            return _apply_op(op, get_value(item, str(field)), where.get("value"))
        # 단축형 {col: value, ...} — 모두 동등(AND)
        return all(_num_eq(get_value(item, str(k)), v) for k, v in where.items())
    return True
