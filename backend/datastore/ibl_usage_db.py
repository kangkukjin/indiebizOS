...
from ibl_signature_slot import (          # noqa: F401  재수출: 원장의 문이 쓰는 이름
    ...
    set_code_validator, _syntax_reason,   # 구문 검증자 슬롯도 여기로(2026-09-07, 1500줄 규칙)
)
...
class IBLUsageDB:
    ...
    def add_example(self, intent: str, ibl_code: str,
        ...
        _syn = _syntax_reason(ibl_code, function_body=bool(alias) or category == "phrase")
        if _syn:
    ...
    def hippo_disabled(cls) -> bool:
        ...
        강제 토글: INDIEBIZ_HIPPO=on(켬)/off(끔). 심층기억 지도(associative_recall 의 memory_map 공급원=로컬 목차)는 별개."""
