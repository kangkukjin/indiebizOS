...
# `content` 도 산문의 이름이다(2026-09-09): [self:write]{content} 가 받는 이름을 [self:read] 산문에 물으면 같은 것 —
# 노출 실험 v3 에서 모델이 `${본문.content}` 를 7회 짚었고 전부 정직 오류로 죽었다(같은 이름=같은 것, 09-04 규칙의 연장).
_PROSE_PATHS = {"message", "text", "content"}
    ...
    if isinstance(obj, str):
        ...
        if s.startswith("{") or s.startswith("["):
            try:
                ...
                # `[줄 11-22 / 전체 80줄]` 머리로 시작하는 읽기 산문이 JSON 으로 오인돼 `.text`/`.message` 산문 규칙
                # (09-04)이 한 번도 돌지 않던 자리(09-09 v3 실측: `${doc.text?}` → None). 파싱이 안 되면 산문이다.
                if path.lstrip(".") in _PROSE_PATHS:
                    return obj
                raise ValueError(
                    f"$변수 필드 추출 실패: 결과가 JSON 이 아니라 '{path}' 경로를 풀 수 없습니다.")
        elif path.lstrip(".") in _PROSE_PATHS:
