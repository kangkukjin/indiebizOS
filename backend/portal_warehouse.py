"""portal_warehouse.py — 노드의 공개 얼굴 = 레벨 창고(공개 서빙 + 방명록).

  · GET  /portal/home     — 창고 홈(사람용). 사이트 맨 루트(/)가 이 페이지다.
  · GET  /portal/manifest — 이웃 폴러가 읽는 기계용 목록
  · GET  /portal/file     — 파일 서빙 단일 관문 (레벨 게이트가 서빙 자체에 걸린다)
  · GET/POST /portal/gb   — 방명록 (한 창고에 스트림 하나)

api_portal.py 1903줄 분할(2026-08-05 감사 부채 ⑨)에서 갈라져 나온 가장 큰 덩어리.
방명록이 같은 파일에 있는 이유: 창고 홈이 방명록을 렌더하고(`_gb_for_level`) 방명록은
창고의 접근 가능 파일 목록을 읽는다 — 서로를 부르므로 나누면 순환이 된다. 함수 안
지연 임포트로 순환을 숨기는 것(감사 ⑦이 진단한 바로 그 부류)보다 같이 두는 게 정직하다.
"""
