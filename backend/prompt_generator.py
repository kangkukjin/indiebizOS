...
class PromptGenerator:
    ...
    # 사고 방식 (모든 에이전트 공통) - Chain-of-Thought 기반 단계적 사고 유도
    THINKING_GUIDE = """## 사고 과정 (Chain-of-Thought)
...
응답하기 전에 스스로 점검하세요:
- **사실 확인**: 내가 말한 정보가 정확한가?
- **완전성 확인**: 요청에 완전히 답했는가?
- **실행 가능성**: 제안한 해결책이 실제로 작동하는가?

### 여러 방법 고려 (Alternative Approaches)
하나의 문제에 여러 해결책이 가능할 때:
1. 2-3가지 접근 방법을 먼저 생각
2. 각 방법의 장단점 비교
3. 가장 적절한 방법 추천 (선택은 요청자에게)
...
- 수행 내용: (구체적인 작업)
- 결과: (결과물 또는 상태)
- 파일: /outputs/파일명.확장자 (생성된 파일이 있는 경우)
```
...
**좋은 위임** (구체적, 맥락 포함):
"사용자가 PDF 보고서를 원합니다. 매출 데이터를 분석해서 차트와 함께 정리해주세요."
