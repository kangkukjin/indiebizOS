    ...
    def _execute_tools_and_continue(
        ...
        for tc_id, tc_info in tool_calls.items():
            ...
            if cancel_check and cancel_check():
                print(f"[Ollama][depth={depth}] 사용자 중단 — 도구 '{tool_name}' 스킵")
                messages.append({
                    "role": "tool",
                    "tool_call_id": tc_id,
                    "content": "[사용자가 중단했습니다]"
            ...
            # 클라이언트에 도구 결과 전달 — UI에는 details가 있으면 details 사용
            ...
            # 3. role="tool" 메시지 추가 (AI에게는 content만)
            messages.append({
                "role": "tool",
                "tool_call_id": tc_id,
            ...
            # [images] 이미지가 있으면 user 메시지로 주입 (vision 모델 지원 시)
            if tool_images:
