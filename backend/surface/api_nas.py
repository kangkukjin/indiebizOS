...
# 경량 Finder 페이지 HTML 두 벌은 api_nas_lite.py 로 분리(2026-08-06, 1500줄 규칙).
from api_nas_lite import _NAS_LITE_HTML, _NAS_LITE2_HTML  # noqa: E402
...
_FINDER_ASSETS = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "finder")
...
async def serve_nas_webapp(request: Request):
    ...
    static_webapp = Path(__file__).parent.parent / "static" / "nas" / "index.html"
    if static_webapp.exists():
        return HTMLResponse(content=static_webapp.read_text(encoding='utf-8'))
    # 2) nas_webapp.html 폴백
    webapp_path = Path(__file__).parent.parent / "nas_webapp.html"
