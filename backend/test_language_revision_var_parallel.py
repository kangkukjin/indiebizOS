...
열려 있었으므로, 결함은 능력이 아니라 **자리의 비대칭**이었다.
  ...
  P7. 폴백(??) 자리는 미개방 — 거절문이 **자리 목록이 아니라 규칙**을 말한다
...
def test_P7_폴백_거절문은_목록이_아니라_규칙을_말한다():
    """열지 않은 자리도 **왜** 안 되는지 말한다 — 맨 '파싱 실패'는 자가교정을 못 이끈다.

    ★첫 판(2026-09-01)은 열린 자리를 **나열**했다("파이프 머리와 병렬 분기에서 쓸 수
    있습니다"). 목록은 반드시 뒤처진다 — 통화를 받는 자리가 하나 더 생기는 날 그 문장은
    조용히 거짓이 된다. 그래서 규칙("변수는 통화를 내는 자리에")과 **더 싼 처방**
    (`$a = [액션] ?? [액션]` — 폴백을 값 만들 때 걸면 게으름이 살아 있다)을 요구한다.
    """
    with pytest.raises(IBLSyntaxError) as e:
        parse(_TWO + '$a ?? $b')
    msg = str(e.value)
    assert "폴백" in msg, msg
    assert "통화를 내는 자리" in msg, f"규칙이 아니라 자리 목록을 말하고 있다: {msg}"
    assert "$a = [액션] ?? [액션]" in msg, f"더 싼 처방을 안 가리킨다: {msg}"
