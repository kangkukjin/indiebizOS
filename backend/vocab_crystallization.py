...
세는 것은 사용 빈도가 아니라 ibl_design_philosophy.md §4 의 **마찰 신호**다:
...
def run_crystallization_scan(force: bool = False) -> Dict[str, Any]:
    ...
    n_candidates = (
        len(result["raw_bypass"]) + len(result["fallbacks"]) + len(result["recurring_combos"])
    )
    if n_candidates:
        summary = (
            f"우회 계열 {len(result['raw_bypass'])} / 폴백 {len(result['fallbacks'])} / "
            f"반복 조합 {len(result['recurring_combos'])} — {_FLAGS_PATH.name} 참조"
        )
