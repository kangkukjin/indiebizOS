...
- 파이프(`>>`)의 `results[]`는 step 요약, `final_result`는 미리보기(큰 items/표=앞 8행+`_preview`, 긴 산문=앞 12,000자)다. 전체는 턴에 남는다. `$이름 >> [table:take]{n}`·`[table:select]`로 필요한 값만 받거나 `code:"", read_result:{id: result_ref.id, offset, limit}`로 중간 step을 포함한 원문을 읽는다. **상세 조회 때문에 작업을 재실행하지 마라.** 단일 액션·블록(`[try]`·`[if:]`)의 `results`는 핸들러 고유 필드일 수 있다(예: `[sense:search]`). `_results_summarized`·`steps_total`·`final_result`로 파이프 봉투를 구별하라. 블록 봉투 표식은 `_caught`·`_untransformed`다.
