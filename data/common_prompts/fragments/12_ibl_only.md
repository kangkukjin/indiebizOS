...
- 긴 프로그램은 `execute_ibl{code, check:true}`로 실행 없이 통화·열(types), 오류·경고(issues), 반복 계획(preflight)을 검사한다. 확정 오류를 고치고 누락·반복 AI 입력 경고를 의도와 대조한다. 방문 상한은 토큰 비용이 아니며 미상은 0이 아니다. 정적 통과는 외부 응답·품질을 보증하지 않는다. 문법 탐침도 실제 실행 대신 check로 한다.
