...
> 앱 표면(🎧 오디오 브리핑 계기)은 2026-09-05 은퇴했다 — 데스크탑 진열·`data/instruments/audio_briefing.yaml` 모두 제거. 제작 레시피(이 문서)와 산출 파일은 그대로 살아 있고, 재생은 파일을 직접 열어서 한다.

## 핵심 원칙
1. **항상 고정 파일명에 저장** — mp3=`audio_briefing_current.mp3`, 스크립트=`audio_briefing_current.md`. "현재 판" 하나만(매번 덮어씀). 해시 이름 쓰지 말 것 — 찾는 쪽이 고정명을 당긴다.
2. **저장 대상은 `project_id: "앱모드"`로 지정** — mp3·스크립트가 앱모드 프로젝트 outputs 에 떨어져야 판이 한자리에 쌓인다. `[engines:tts]`·`[self:write]` 에 `project_id: "앱모드"` 를 실으면 라우팅이 이를 존중해 `projects/앱모드/outputs/` 로 굽는다.
   - 이 레시피는 대개 위임(`[others:delegate]{scope:"system"}`)을 통해 시스템 AI가 실행하는데, **명시 project_id 는 호출자(시스템 AI)의 컨텍스트 경로보다 우선**한다(`ibl_routing.resolve_project_path`). 그래서 하드코딩 절대경로도 `self:move` 도 필요 없다 — 데스크탑 앱·MCP·위임 어느 경로든 `project_id: "앱모드"` 하나면 목적지에 바로 떨어진다.
...
두 파일이 늘 같은 이름의 "현재 판"이라 다음 호가 덮어쓰기만 하면 된다.
