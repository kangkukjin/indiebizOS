...
## source="nl" — 국립중앙도서관 납본 소장 검색 (2026-08-04 신설)

`[sense:book]{keyword: "...", source: "nl"}` — nl.go.kr 통합검색을 키 없이 크롤(tool_nl.py).

- **두 소스는 렌즈가 다르다**: 정보나루(기본)=공공도서관이 소장·대출하는 책의 이용분석
  코퍼스 — 도서관들이 구입하지 않은 신간·소부수·독립 출판은 **존재 자체가 없다**.
  국립중앙도서관=납본 전수 — ISBN 달고 출간된 국내 책의 국가 서지.
  **"이 책이 존재하나/서지가 뭔가"는 nl 이 정본, "얼마나 읽히나/어느 도서관에 있나"는 정보나루.**
- 반환: title·meta(저자·출판사·연도·자료형태)·summary(청구기호·자료실)·isbn13.
  isbn13 을 정보나루 detail 로 이어 대출통계 교차 가능(공공도서관에 있는 책이면).
- category 파라미터: 기본 "도서". "학위논문"/"기사" 등도 가능, 빈 문자열=전체.
...
- **정보나루에서 "없다"는 "미출간"이 아니다**: 공공도서관 미구입일 뿐일 수 있음 — source:"nl" 로 존재 확인 후 판단.
- **지역 필터의 커버리지 공백(실측 2026-08-04)**: `region` 을 주면 libSrchByBook(소장 도서관)·loanItemSrch(popular) 모두 **일부 지역만 집계**된다 — 서울·부산·광주·세종은 정상, 경기·충북·대전·충남은 0. 빈 결과=버그 아님. 전국(무필터)이나 서울로 폴백.
...
정보나루의 제목·저자·키워드 검색과 국립도서관 검색은 `page`를 전달한다(기본 1). 국립도서관 화면을 해석하지 못하면 오류이며, 명시적으로 0건임을 확인한 경우에만 자료가 없다고 반환한다.
