...
비즈니스 **아이템은 곧 공유창고에 진열되는 물건**입니다. 아이템을 넣거나 고치면 다음이 자동으로 따라옵니다(사용자가 따로 "발행"하지 않습니다):

```
공유창고/<비즈니스 레벨>/<비즈니스 이름>/<제목>.md      ← 낱개 문서(이웃 폴러·AI 가 집어가는 알갱이)
공유창고/<비즈니스 레벨>/<비즈니스 이름>/<제목> 1.jpg    ← 사진 사본(공개면이 EXIF 벗겨 서빙)
공유창고/<비즈니스 레벨>/<비즈니스 이름> 카탈로그.html   ← 사람이 여는 표지 한 장(사진 축소판 임베드)
```
