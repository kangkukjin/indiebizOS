...
- 2026-09-09 실측: 정기 보고서 발행은 manage_events 이벤트가 아니라 trigger 원장에 등록돼 있었고, 트리거를 disable 하면 이벤트 원장의 미러본([IBL] <트리거명>)도 같이 enabled=false 로 내려갔다 — 이벤트 쪽을 따로 toggle 할 필요 없었다(양쪽 재조회로 확인).
