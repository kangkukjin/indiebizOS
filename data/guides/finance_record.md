...
1. 폰에서 결제 알림(하나카드·청주페이)을 **지우지 않고** 모아둔다 — 활성 알림에만 텍스트가 남는다.
2. 폰을 USB 로 연결하고 수거: `[self:finance]{op: "sync"}` (계기 지출 탭 '지금 수거하기' 버튼).
3. 수거가 끝나면 알림을 지워도 된다 (ext_id 해시 dedup — 여러 번 수거해도 안전).
