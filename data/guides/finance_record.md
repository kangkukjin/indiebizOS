...
2. 수거: `[self:finance]{op: "sync"}` (계기 지출 탭 '지금 수거하기').
   ...
   - ★**평소엔 누를 필요가 없다(2026-09-18)**: 포획소가 결제 알림을 적으면 3초 뒤 폰 백엔드의
     `POST /finance/sync/run` 을 깨운다 → 폰이 자기 원장에 수거하고 곧바로 PC 원장과 머지(아래 절).
     PC 에 안 닿으면 보류해 두고 10분 주기·다음 알림·폰 백엔드 기동 때 따라잡는다.
     상태는 폰 `GET /finance/sync/status`(pending·last). USB 수거는 예비 경로.
...
- 수거된 알림은 **거래로 직접 병합**: 승인=지출, 취소·환불=음수 지출(합계 자동 차감),
  인센티브·캐시백 등의 금액과 지급·적립·입금 완료가 함께 확인되면 수입이다.
  지급 예정·실패 안내는 거래로 넣지 않고, 결제에 붙은 인센티브 사용·지급 안내는 수입으로 바꾸지 않는다.
  수령액은 지급 완료 문구에 붙은 금액이며 잔액·누적액과 구분한다. 수입 원문도 note에 보존한다.
...
- 이 왕복을 돌리는 주체 = 폰 `phone_finance_sync.py`(phone-companion 파이썬, 맥 위임 채널
  `INDIEBIZ_MAC_URL` 직결·런처 세션 인증 — 공개 릴레이 전송 아님). 09-06~09-18 사이엔 API 만 있고
  부르는 자가 없어 USB 수거만 PC 원장을 갱신했다.
- 같은 결제를 두 몸이 각자 수거해도 **ext_id 가 같아 한 행으로 접힌다**(머지 통계의 `collided`).
- ★**주체 이름이 두 몸에서 같아야 한다**: owner uuid = `uuid5("finance-owner:"+이름)` 이라
  이름이 다르면(맥 '<사용자 이름>' / 폰 기본값 '나') 같은 사람이 두 주체로 갈린다.
