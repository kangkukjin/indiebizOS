...
> 시 코퍼스 정리) · `data/system_docs/memory.md`(기억 8종 지도).
...
**심층기억은 이 절차의 영향을 받지 않는다**(2026-09-17). 예전에는 심층기억도 해마 모델을 빌려 써서, 재학습 때마다 기억의 벡터가 옛 공간에 남았다(재색인 대상에 없었다 — 실측 코사인 중앙 0.71). 지금 기억은 재학습하지 않는 기본 인코더를 쓰고 DB 마다 인코더 표식으로 스스로 재임베딩한다. 해마 모델을 바꿔도 심층기억·세계의 기억 쪽에는 할 일이 없다.
