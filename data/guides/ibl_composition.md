...
check:true는 문법·알려진 타입과 함께 반복 계획을 검사한다. `each_input_limit`은
확정된 입력 건수보다 each 한도가 작다는 경고이고, `repeated_ai_batch`는 반복 안에서
바뀌지 않는 여러 행을 AI 어휘에 매번 보내는 구조다. 위치(`at`)와 근거(`facts`)를 보고
전건 처리·행별 처리가 의도인지 확인한다. 일부 처리나 공통 자료의 반복 판단이 목적이면
경고가 정당한 프로그램에도 나올 수 있으므로 입력을 자동 축소하지 않는다.

`preflight.declared_ai_visits_upper_bound`는 알려진 반복·함수를 펼친 **AI 어휘 방문 상한**이다.
배타적 분기도 합산하므로 실제보다 클 수 있고, 내부 모델 호출·조건 판단·재시도·토큰·시간은
계산하지 않는다. `unknowns`는 동적 입력·동적 본문·미확인 함수·분석 한도 등 판단하지 못한
자리다. 건수가 미상이더라도 고정 each 한도가 있으면 방문 상한은 알 수 있다.
`null`은 상한 미상이며 0회라는 뜻이 아니다. 스크립트 내부와 도구 출력 크기는 추측하지 않는다.

정적 통과는 웹 응답·내용 품질·실행 성공의 보증이 아니다.
고정된 작은 입력으로 결과 열과 분기를 확인한 뒤 실제 규모를 검증한다.
