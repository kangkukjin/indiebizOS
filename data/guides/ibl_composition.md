...
저장된 관용구가 없어도 현재 프로그램 안에서 작은 일을 지역 함수로 정의하고 서로 호출한다.
...
**분해 이유**: 한 품목의 계산, 여러 품목의 내역 생성, 한 구매안의 예산 판정은 입력·결과가 서로 다르다.
이 경계로 나누면 계산 규칙을 한 곳에 두고 여러 안에 적용할 수 있다. 한 번만 쓰는 부분도
복잡한 책임을 구분하는 데 도움이 되면 함수로 만들고, 단순 호출 하나마다 함수를 만들지는 않는다.
...
**구현과 최상위 조합**: 세 정의와 마지막 호출을 하나의 `execute_ibl` 프로그램으로 보낸다.
정의 자체는 작업을 실행하지 않는다. 마지막 호출에서 `예산검토 → 내역작성 → 금액계산`이 연결된다.
아래 예제는 저장된 관용구에 의존하지 않는다.
...
같은 프로그램 안에서는 이 정의들을 여러 번 호출할 수 있고, 저장은 필요하지 않다.
