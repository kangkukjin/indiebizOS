...
## 원격 런처·폰 표면 — 같은 id 의 선언형 4탭 계기

`[sense:place]` 의 `app:` 블록(location-services `ibl_actions.yaml`)이 **🗺️ 지도** 계기를 선언한다(`instrument: directions`, 명시적 `modes:` 검색·저장·길찾기·대중교통). 데스크탑은 같은 id 를 커스텀 계기(MapInstrument)가 덮고, 원격 런처(`/launcher/app`)·폰 네이티브 앱은 이 선언을 제네릭 렌더러로 그린다 — 표면별 코드 0. 검색 탭 = 검색어+카테고리 select → 지도(이 지역에서 검색)+목록(☆ 저장 행 버튼, 행 클릭=상세 드릴: 블로그 언급·카카오맵 링크) · 저장 탭 = 원장 items(폴더 칩=`filter.from_field: tag`, 삭제 버튼) · 길찾기 탭 = 자동차 navigate_route, 대중교통 탭 = mode:transit 요약 목록. 장소 검색·원장 읽기/쓰기는 전부 `@hub`(PC)로 — 폰도 PC 의 원장 한 곳을 본다. 폰 반영은 APK 재빌드(`phone-companion/scripts/setup_phone.sh --build`, 어휘가 APK 에 번들).
...
설정·반환·도시간 제한은 [transit_route.md](transit_route.md) 참조. 지도 앱의 길찾기에서 자동차/대중교통을 선택한다. 대중교통은 빠른 순·환승 적은 순·도보 짧은 순으로 정렬하고, 경로를 누르면 승하차·노선·도보 구간이 펼쳐진다. 지도에는 선택 경로의 정류장 위치를 표시하며 노선 선형·도보 경로는 표시하지 않는다.
