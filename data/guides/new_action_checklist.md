...
- view 프리미티브 15종: metric / kv / kv_list / card_list / image_grid / sparkline / list_action / thread / form / editable_list / map / calendar / group / blocks / media_player — media_player=오디오 플레이어(`<audio>`, items의 src=파일 절대경로/URL → `/launcher/file` 서빙). 표시 템플릿 `{path|filter}`. blocks=문서 IR(read{blocks:true}·table:structure 출력) 렌더. **응답 shape은 추측하지 말고 live `/ibl/execute`로 확인 후 작성.**
