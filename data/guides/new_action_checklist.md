...
6. **파라미터·op 도 정본으로 (2026-08-05 가드화 — `scripts/check_param_canon.py` 가 커밋 차단)**:
   같은 개념에 새 이름을 만들지 말 것. 검색어=`query`(q/keyword/search_term ✗) · 결과 상한=`limit`(count/max_results/display/top ✗) · 좌표=`lat`/`lng`(latitude/longitude/lon ✗). op 는 읽기 하나=`detail`(get ✗), 짝 규칙 add↔remove(소속) · save/create↔delete(엔티티). 관습어 수용이 필요하면 `aliases:` 블록으로(정본 키 하나에만 묶을 것).

> 임시방편(영구 별칭) 금지. 이름을 바꾸면 정본·교재 표면을 다 옮긴 뒤 별칭을 은퇴시켜 단일 어휘를 유지한다.

7. **은퇴는 등록한다 — "교재 표면"의 정의 (2026-08-27 신설, 관문 `scripts/check_retired_contracts.py`)**:
   낱말의 이름·뜻·계약을 바꾸거나 은퇴시킬 때 `data/retired_contracts.yaml` 에 **한 줄**(사유·대체·금지 문구)을 적는다. 그러면 관문이 매 커밋 전 표면을 대신 훑는다 — **표면 목록을 사람이 기억하지 않는다.**

   한 낱말의 교재 표면은 다음 여섯이며, 앞의 셋만 빌드가 대조한다:

   | 표면 | 지키는 것 |
   |---|---|
   | `ibl_actions.yaml`(desc·target_description) | `build_ibl_nodes.py --check` |
   | 파생물(`ibl_nodes.yaml`·`tool.json`·`android.nodes.yaml`) | 같음 (손 편집 금지) |
   | 문서 마커 구간 | 같음 |
   | **핸들러의 오류·도움말 문자열** | ← **아무도 안 봤다** |
   | **독스트링·주석** | ← **아무도 안 봤다** |
   | 가이드 산문·시드 스크립트 | ← 부분적 |

   ★2026-08-27 실측(`aa904ffc`): 47회차가 flatten 의 `each >> flatten` 계약 은퇴를 앞의 셋에서만 집행해, **모델과 사용자가 실제로 읽는 런타임 오류 문구**가 은퇴한 관용구를 계속 지시했다. 카탈로그는 참인데 몸이 거짓을 말한 것이다.  (retired-ok: 그 은퇴를 설명하기 위해 옛 관용구를 이름 불러 인용하는 자리)
...
| **단일값 (`scalar`)** | 통화 아님 — **값 하나**(시각·답변 문장·본문 텍스트). 명확한 dict 반환(에러 없이). ★아래 '1행 스냅샷'을 먼저 읽고 고를 것. | 레코드를 scalar 로 접으면 팬아웃·필터가 영영 막힌다 |
| **행동 (`effect`)** | 통화 미기대 — 명확한 `success`/`error`(또는 파일 경로·산출물) 반환. **부작용이라 정기 실행 불가** → fixture 면제, `--check` 구조검사만. | — |

> ★**1행 스냅샷 — 레코드는 스칼라가 아니다 (2026-08-24 B36-2 부류 수리로 명문화).**
> **한 개체를 조회해 필드 여러 개짜리 레코드를 낸다면 `items` 1행이다.** 갈림길 질문 하나:
> *"이걸 대상 여러 개에 대해 돌려서 거르고 싶어질 일이 있나?"* — 있으면 레코드다.
> 판정 예: 영상 메타(`{title, duration, uploader, view_count, upload_date}`) · 좌표→주소
> (`{address, region_1depth…}`) · 종목 기본정보 = **전부 1행 스냅샷**(`items: [행]` 병기,
> 원 키는 보존해 기존 소비자를 안 깬다). 반면 시각 하나·답변 문장 하나·파일 본문·
> 이질적 컨텍스트 blob(중첩 리스트가 섞인 world snapshot)은 **진짜 scalar**다.
...
> 담는 것도 같은 병이다(`self:discover`) — 통화의 이름은 `items` 하나다.
...
- **이웃도 선언하지 않는다 — 이어진 뒤 관측된다**(2026-08-30). 카탈로그의 `⟨동반: >>a · &b⟩` 는 교재·실행에서 그 낱말 뒤에 실제로 이어진 낱말의 실측(`scripts/ibl_partner_sweep.py` → `data/ibl_partners.json`, 주간 순찰이 재관측·상위 2·최소 3회)이라 새 액션은 첫 조합이 관측될 때까지 비어 있다. 그러니 **시드는 인자뿐 아니라 이웃도 가르치는 교재**다 — 단일 호출만 시드하면 그 낱말은 카탈로그에서 계속 섬으로 보인다(실측 08-30: 낱말별 교재 조합 노출률→실행 조합률 r=0.72). 시드 뒤 `python3 scripts/ibl_partner_sweep.py` 한 번.
- ★**별칭을 핸들러 코드 안에서 풀지 말 것 — `aliases:` 블록으로 선언한다**(2026-08-23). 핸들러가 `_arg(ti, "ticker", "symbol")` 처럼 사적으로 풀면 **카탈로그는 그 사실을 볼 수 없어 두 이름을 다 광고**하고, 언어가 한 자리에 두 이름을 가르친다(실측: `sense:stock` ticker 210 / symbol 126, 한 호출에 함께 온 적 0). 선언하면 호출은 그대로 둘 다 통과하고(`ibl_routing._normalize_param_aliases`) 카탈로그만 정규 키 하나를 말한다. `ibl_param_sweep` 이 **분열 후보**(상호배타 + 호출을 분할하는 빈출 키 쌍)를 매주 신고한다 — 신고만 하고 자동으로 접지 않는다(어느 쪽이 정규인지 정하는 건 코드·문서가 이미 아는 사실을 옮기는 일이지 추측할 일이 아니다). ★**한 op 의 정규 키를 다른 op 의 별칭으로 접지 말 것**(`image_read` 의 `intent` 는 op:critic 의 정규 키라 `question` 별칭에서 제외).
