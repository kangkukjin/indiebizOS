...
4. **보편 우선·중복 금지**: 기존 액션과 [구조·결합·반복 확장](../../docs/IBL_EXPRESSION_REVISION_2026_09_08.md)으로 먼저 표현한다. 새 액션은 출력 모양보다 고유 API·인증·파싱 등 *접근(access)*이 실제로 다른지로 판단한다.
...
현재 값·결합·평문 반환 계약은 [어댑터 작성](ibl_v2_adapter.md)을 따른다.
...
```yaml
      my_action:
        ...
        #     `--check` 의 '선언 내부 모순'·'구현-읽기 감사'(좁은 선언 포함)가 잡는다.
