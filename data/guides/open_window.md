...
| `messenger` | 외부 메신저/이메일(Nostr/Gmail) 통합 인박스·이웃/연락처 (옛 indienet·이웃관리) | "메신저 확인", "메일 봐줘", "인디넷 열어" |
...
```
(레거시 `app:"indienet"`도 같은 메신저 창으로 라우팅됨)
...
| "메일 확인", "텔레그램 봐줘", "외부 메시지" | `messenger` |
...
- **app 오타**: `project` / `system_ai` / `messenger` / `business` / `multichat` / `folder` 정확히. `projects`(X), `mail`(X), `chat`(X). (`indienet`=`messenger` 레거시 별칭으로 동작)
