...
- 보증금 `"19,000"`(만원, 콤마 문자열)은 `[table:filter]{where: ["보증금 >= 20000", "보증금 <= 40000"]}`가 숫자로 정확히 건다(정규식 우회는 경계를 못 그림). items는 10개로 잘려 보여도 filter·groupby는 전량을 훑는다. 단지명은 호가와 실거래가 다를 수 있다('주공6단지' vs '분평주공6') — `==` 0건이면 이름부터 의심.
