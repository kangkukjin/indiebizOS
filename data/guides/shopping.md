...
```
[sense:search_shopping]{query: "무선 이어폰"}                 # 다나와 가격비교(기본)
[sense:search_shopping]{query: "노트북", display: 10}
[sense:search_shopping]{query: "에어팟", site: "all"}          # 다나와+중고(PC 전용)
```
...
| `used` | 중고나라·번개장터 | PC 전용 | Playwright 스크래핑. **현재 0건**(아래) |
| `all` | 다나와 + 중고 | PC 전용(폰선 다나와만) | 느리다 — 사실상 다나와와 같음 |
| `naver` | — | 죽음 | 안내 문구만 반환 (아래 §3) |

중고를 찾을 땐 이 액션의 `site: "used"` 대신 **`[sense:used]`** 를 써라. 소스가 넷
(번개장터·당근·중고나라·네이버카페)이고 동네 스코프까지 되며, 브라우저 자동화가 아니라
폰에서도 돈다.

★`site: "used"` 는 **2026-08-04 실측 0건**이다 — 중고나라·번개장터가 개편되면서
Playwright 셀렉터(`ul.grid > li`, `div[class*='ProductItem']`)가 낡았고 예외를 조용히
삼켜(`except: pass`) 빈 목록으로 나온다. 고칠 가치가 없다: 같은 두 소스를
`[sense:used]` 가 내부 API 로 이미 깨끗하게 준다. 이 축은 은퇴 후보다.
...
- 폰에서 `site: "used"` 나 `"all"` 을 기대하는 것 — 중고 스크래핑은 Playwright 라
  폰에선 graceful 미지원 메시지가 온다. 폰 중고 검색은 `[sense:used]`.
- 중고를 이 어휘로 찾는 것 — 소스가 하나뿐이고 파싱도 약하다. `[sense:used]` 를 쓸 것.
