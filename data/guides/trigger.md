...
`[self:schedule]` (한 번 지연 실행) · `[self:manage_events]` (캘린더 이벤트)는 별도 액션. 이 가이드 범위 밖. (캘린더 보기는 앱 모드 `calendar` 계기·런처 스케줄 뷰로.)
...
[self:trigger]{op:"create", trigger_id:"morning_news",
  ...
  pipeline:'[sense:search]{source: "gnews", query:"AI"} >> [others:channel_send]{channel:"gmail", to:"me", subject:"오늘의 AI 뉴스"}'
}
```
