...
- 이미지 생성 실패 → Gemini 이미지 생성 재시도 (`forge:image_gemini`)
...
**실패 시 대안**:
1. `[forge:image_gemini]("프롬프트")`로 재시도
2. Gemini도 실패 → 사용자에게 보고하고 지시를 기다림. **이미지 없이 다음 단계로 넘어가지 마세요.**

### Step 3: 씬 HTML 코드 작성
각 씬을 별도 HTML 파일로 작성합니다. `system:write`로 파일에 저장합니다.
```
[system:write]("outputs/video_project/scene_01.html") {"content": "<!DOCTYPE html>..."}
[system:write]("outputs/video_project/scene_02.html") {"content": "<!DOCTYPE html>..."}
```
...
```
[system:edit]("outputs/video_project/scene_01.html") {"old_string": "...", "new_string": "..."}
```
...
```
[forge:tts]("첫 번째 씬의 나레이션 텍스트") {"output_filename": "narration_01.mp3"}
[forge:tts]("두 번째 씬의 나레이션 텍스트") {"output_filename": "narration_02.mp3"}
```
...
**실패 시**: `[forge:image_gemini]("프롬프트")` 재시도하고, 계속 실패하면 사용자에게 보고. **이미지 없이 TSX 작성으로 넘어가지 마세요.**

### Step 3: TSX 컴포지션 코드 작성
React/TSX 코드를 파일로 작성합니다.
```
[system:write]("outputs/video_project/composition.tsx") {"content": "import {AbsoluteFill, useCurrentFrame}..."}
```
...
```
[forge:render_remotion]("outputs/video_project/composition.tsx") {"duration_in_frames": 300, "fps": 30, "width": 1280, "height": 720, "asset_paths": ["이미지경로1.png", "이미지경로2.png"], "output_filename": "remotion_video.mp4"}
```

### Step 5: 오류 수정 (필요 시)
렌더링 실패 시 에러 메시지를 확인하고 TSX 코드를 수정합니다.
```
[system:edit]("outputs/video_project/composition.tsx") {"old_string": "...", "new_string": "..."}
```
...
```
# HTML 영상 - 코드를 직접 전달
[forge:video]("주제") {"scenes": [...], "narration_texts": [...]}

# Remotion 영상 - 코드를 직접 전달
[forge:remotion]("<TSX 코드>") {...}
```
...
| 액션 | 용도 | 핵심 파라미터 |
|------|------|---------------|
| `forge:image_gemini` | Gemini AI 이미지 생성 | prompt |
