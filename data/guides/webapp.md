...
신규 제작은 아래 결정 트리로 기존 계기·구현 선례부터 고른다. 기존 앱 수정은 그 앱의
README·작업 규칙·실행 설정·현재 변경 상태부터 확인한다. 이어 필요한 단계에만 다음
...
외부 웹앱의 npm 테스트나 브라우저 확인을 대신하지 않는다. 제작·검사·배포 전체를 맡기는
범용 관용구는 현재 이 가이드에서 제공하지 않는다.
