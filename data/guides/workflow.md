...
```
[self:workflow]{op: "save", name: "구글신문", description: "구글 뉴스 수집 후 신문 생성", pipeline: "[engines:newspaper]{keywords: \"AI, 경제, 문화\"} >> [limbs:os_open]"}
```
...
```
[self:workflow]{op: "run", workflow_id: "구글신문"}
```
...
```
[self:workflow]{op: "get", workflow_id: "구글신문"}
```
...
```
[self:workflow]{op: "delete", workflow_id: "구글신문"}
```
...
```
RIGHT: [engines:newspaper]{keywords: "AI, 경제"} >> [limbs:os_open]
WRONG: [engines:newspaper]{keywords: "AI"} >> [limbs:os_open]{path: $prev.file}
```
...
2. `[self:workflow]{op: "save"}`로 저장
3. 저장된 ID 반환

### 사용자가 워크플로우 실행을 요청할 때
1. `[self:workflow]{op: "list"}`로 목록 확인
2. 매칭되는 워크플로우 찾기
3. `[self:workflow]{op: "run", workflow_id: "이름"}`으로 실행
