...
```
[system:save_workflow]() {"name": "구글신문", "description": "구글 뉴스 수집 후 신문 HTML 생성", "pipeline": "[source:search_news](\"google\") >> [forge:create](\"newspaper.html\") {type: \"newspaper\"}"}
```
...
2. `[system:save_workflow]`로 저장
3. 저장된 ID 반환

### 사용자가 워크플로우 실행을 요청할 때
1. `[system:list_workflows]()`로 목록 확인
2. 매칭되는 워크플로우 찾기
3. `[system:run]("이름")`으로 실행
