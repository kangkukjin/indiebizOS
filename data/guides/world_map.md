...
IBL 어휘에 들어오지 않은 수단까지 넓게 담아 **사용자가 이름을 말하지 않아도 과제에 맞는 방법을
떠올리고 활용하도록** 돕는다. 지식 본문을 저장하는 백과사전은 아니다.
예를 들어 좋은 웹 화면을 만드는 요청에는 전문가가 쓸 방법과 기존 자원을 선택하는 일도 포함된다.
익숙한 구현만 시작하기 전에, 바로 활용할 수 있는 적절한 수단을 놓치고 있지는 않은지 검토한다.
이미 적절한 접근은 돕고, 새로움 자체를 위해 대안을 강요하지 않는다.
학습·연구 목적의 직접 구현과 명시 제약을 존중한다.
