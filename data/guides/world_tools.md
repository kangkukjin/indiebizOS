# 방법의 지도 — 세상의 도구와 방법 (World Tools Map)
...
IBL의 어휘는 도구를 가리키고 문장은 그 도구를 조합해 일을 한다. **방법의 지도는 더 많은 도구·방법의 이름을
공급하여 실제로 선택할 수 있는 수단을 넓힌다.** 이름이 모델이 아는 사용법·적용 조건을 떠올리게 하고,
필요한 통로를 확인해 기존 IBL의 실행 수단으로 연결한다. 지도에서 고르는 기준은 이번 일을 할 수 있는 방법인가다.
관련 주제라는 이유만으로 채우지 않는다. 지도 편집 기준은 `data/knowledge_catalog/README.md`에 둔다.

★**이름도 지도에서 떠올릴 수 있다**: `data/knowledge_catalog/world.yaml`과 fragments는 이 지도와 조사본에서 추린 이름·뜻·분류·관계의 카탈로그다. 모든 작업 에이전트에서는 질문에 맞는 작은 조각을 의식 직전에 골라 의식·실행자에 함께 전달한다. 의식은 그 단서의 관련성을 판단하며 이미 적절한 접근을 유지한다. 전체 어휘·관계 열람은 [세계 지도 사용법](world_map.md)을 따른다. 이 가이드는 **통로**(pip 이름·설치 명령·헤드리스 호출법)·**실측 비고**(로컬 `world_tools_local.md`, §3)·**화이트리스트**를 맡는다. 짧은 카탈로그 단서를 받은 것은 가이드 본문 열람이나 설치 승인을 대신하지 않는다.
...
- **자작 관문이 세웠을 때**(2026-09-07): 한 턴에 새 구현 코드가 쌓였는데 세상의 도구를 한 번도 확인하지 않으면 `[self:write]` 가 거절된다. 이 지도를 열거나 후보를 `check: true` 로 물으면 그 턴 내내 걷힌다 — 확인하고도 마땅한 도구가 없으면 그대로 쓰면 된다(관문은 답이 아니라 확인을 요구한다). 몸의 코드(RED·등록 스크립트·패키지)는 관할 밖이다.
...
- **이 몸의 실측**이 있으면 먼저 본다: `[self:read]{path: "data/guides/world_tools_local.md"}` (없으면 없는 것 — 만들 필요 없음).
...
두 파일이다. **정본 = 세상의 사실**, **로컬 = 이 몸의 판정**. 갈래를 틀리면 갱신이 멈춘다(`scripts/update.py` 는 ff-only — 상류가 손대는 정본을 몸이 고치면 충돌).

- **정본** `data/guides/world_tools.md` — 지도에 없는 도구로 **성공**했으면 표에 한 줄: 도구·잘하는 일·통로(`pip:이름`/🔧/🌐). 세상 어디서나 참인 것만.
- **로컬** `data/guides/world_tools_local.md`(저장소 밖, 없으면 `[self:write]` 로 만든다) — 이 몸에서 판정된 것: pip 빌드 실패·버전 제약·대체 통로·"이 통로로 성공함". 정본의 비고 열에 적지 않는다.
- 통로 자체가 **틀렸음**(이름 변경·바이너리 필요)이 세상의 사실이면 정본 비고를 고친다. 지우지 않고 사실을 적는다.
- 몸에 있음/없음은 어느 쪽에도 적지 않는다(§2 첫 줄 — `check: true` 가 정본). 예산 36KB — 넘치면 드문 부류부터 압축.
