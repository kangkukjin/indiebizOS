...
★**이름도 지도에서 떠올릴 수 있다**: `data/knowledge_catalog/world.yaml`은 이 지도와 조사본에서 추린 이름·짧은 설명의 카탈로그다. 켜진 에이전트에서는 질문에 맞는 작은 조각을 의식 직전에 골라 의식·실행자에 함께 전달한다. 의식은 그 단서의 관련성을 판단한다. 이 가이드는 **통로**(pip 이름·설치 명령·헤드리스 호출법)·**실측 비고**(로컬 `world_tools_local.md`, §3)·**화이트리스트**를 맡는다. 짧은 카탈로그 단서를 받은 것은 가이드 본문 열람이나 설치 승인을 대신하지 않는다.
