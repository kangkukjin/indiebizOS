...
★**이름은 의식이, 통로는 지도가**(2026-09-06 판정): 세상이 이 일을 주로 어떻게 푸는지는 의식 에이전트가 task_framing 에 가중치로 적는다(지도 불요). 이 지도에 남는 것은 가중치가 모르는 셋 — **통로**(pip 이름·설치 명령·헤드리스 호출법)·**실측 비고**(이 몸에서 판정된 사실, §3 으로 누적)·**화이트리스트**(pip 이름은 지도의 것 그대로).
