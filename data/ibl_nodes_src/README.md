...
액션에 `runs_on: anywhere|pc_only|phone_only`를 달아 실행 환경을 선언한다 (미지정=`anywhere`).
- `anywhere` (기본): 이식 가능 로직/HTTP. handler/driver 라우터면 **검증된 폰 패키지**(`build_ibl_nodes.PHONE_VERIFIED_PACKAGES`)일 때만 폰서 실행.
- `pc_only`: 집 PC 하드웨어·무거운 의존·미검증 패키지. **폰서 직접 실행 못 함 → 맥(연합 두뇌)에 단건 라우팅**(예: `limbs:os_open`/`open_window`=데스크탑 GUI, `self:manage_events`=무거운 api_system_ai 의존).
...
- `backend/ibl_access.py` (`load_nodes_raw`) — 1차 로더, 캐시 보유
