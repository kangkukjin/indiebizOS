...
신고한다(기본 skip의 부분 데이터 보존). stop·전 분기 실패는 통화 유무와 무관하게 실패한다.
...
def handle_dead_branches(op_name, objs, params, get_items, get_table):
    ...
    failed = [{"branch": i, "error": str(o.get("error") or o.get("reason") or "실패 봉투")[:300]}
              for i, o in enumerate(objs, 1)
              if isinstance(o, dict) and (o.get("success") is False
                  or (o.get("error") and o.get("success") is not True))]
    if failed and (on_error == "stop" or len(failed) == len(objs)):
        return objs, failed, {
            "success": False,
            "error": (f"{op_name}: 분기 실패 — "
                      + ("전부 실패했습니다." if len(failed) == len(objs)
                         else "하나라도 실패하면 전체 실패(on_error:stop).")
                      + " 통화 보유 여부와 관계없이 중단합니다."),
