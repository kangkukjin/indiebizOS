"""렌더 방식 **이미지+글자** — 개념 일러스트(Nano Banana Pro) + 또렷한 타이포 레이어.

덱 design_system 이 `image_<톤>` 일 때의 경로(2026-08-06 복원). 세 렌더 방식 중 가운데:
  이미지 only(slide_native)  = 이미지 모델이 글자까지 통째로 그림
  **이미지+글자(이 모듈)**   = 글자 없는 그림 + HTML 타이포 레이어 합성
  HTML(shadcn_slides)        = 글자도 그림도 HTML

흐름: 저작 AI(구성 선택 + 제목/장면/필드) → Gemini 이미지 생성 → tone×composition 합성.
그림에 **글자를 그리지 못하게 막고**(COMMON_SUFFIX) 대신 글자 자리를 비우라고 scene 에
지시한 뒤, 그 여백 위에 HTML 타이포를 얹는 것이 이 경로의 핵심이다.

구성 아키타입(6): diptych(좌우대비) / hero(선언) / side_panel(설명) / center_anchor(중심상징)
                  / annotated(주석도면) / process(3단 흐름).

★구성은 **AI가 내용을 보고 고른다** — 사람이 고르는 축이 아니다(tool_input["composition"]은
프로그래매틱 강제용으로만 남는다). 사람이 고르는 건 톤과 렌더 방식 둘뿐.
"""
...
def create_image_slide(tool_input: dict, output_base: str, style: str, slide_id: str = None) -> str:
    ...
    ★구 "auto"(슬라이드마다 AI가 톤을 갈아탐)는 2026-08-06 폐지 — 덱 하나는 톤 하나가
    원칙이고(slide_styles 첫머리), 톤은 이제 사람이 고르는 축이다.
    """
    styles_mod = _load_styles()
    if not styles_mod.is_image_style(style):
        return json.dumps({"success": False,
            "message": f"이미지+글자 렌더를 지원하지 않는 톤: {style}. 가능: {styles_mod.style_keys_help()}"},
            ensure_ascii=False)
