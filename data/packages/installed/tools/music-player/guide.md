...
폴더 드릴은 **탐색/재생 2탭**이다 — '폴더'(하위 폴더 + 이 폴더에 직접 든 곡 = 파인더 화면
그대로) / '이 폴더 전부 재생'(하위 폴더 곡까지 전체). 탐색 탭에 *하위* 곡을 깔면 그 폴더에
직접 든 곡으로 오해된다(실사용 신고).
...
| folders | folder(생략=최상위) | 한 단계 탐색 — items=직속 하위 폴더(맨 앞 ⬆️ 상위 행) + direct=이 폴더에 직접 든 곡 + tracks=그 폴더 아래 전체 곡(상한 500) |
