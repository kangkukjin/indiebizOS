...
폴더 탭의 드릴은 **재귀**다: `item_click.recursive: true` 가 "지금 보고 있는 드릴 뷰를
그대로 다시 쓴다"는 뜻이라, 한 벌 선언으로 깊이 제한 없이 파고든다(단계마다 손으로
중첩해 쓰면 declared depth 에서 반드시 막힌다). 렌더러 2곳·검증기 모두 같은 규칙.
