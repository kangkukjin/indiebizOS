...
## 외장 폴더가 갑자기 비어 보이거나 열리지 않을 때

폴더 등록이 남고 볼륨도 연결돼 있다면, 실제 백엔드의 읽기 권한을 확인한다. macOS는
서버를 시작한 앱에 따라 이동식 볼륨 접근 권한이 달라질 수 있다. 장애 복구 중 개발 도구에서
백엔드를 따로 시작했다면, 진행 중인 작업을 확인한 뒤 정상 종료하고 **IndieBiz 시작.app**으로
다시 실행한다. 자식 워커만 교체하면 기존 실행 주체가 유지되므로 해결되지 않는다.

`/showcase/list`는 디렉토리를 읽지 못하면 HTTP 503과 권한·저장장치 오류를 반환한다.
읽기 실패를 빈 목록(HTTP 200)으로 숨기지 않는다. 등록 폴더를 지우거나 다시 발행할 필요는 없다.
