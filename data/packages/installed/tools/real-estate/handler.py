...
# ── 단일 액션 op 함수 (2026-08-05 장식 스텁 → 진짜 디스패처 전환) ──────────
...
def _op_query(tool_input, context):
    """[sense:realty]{op:query} — 실거래가/현재 매물 조회 (기본 op).

    ★2축: source(molit/zigbang/naver) 분기는 _op_query 안 — source 축은 빌드 가드 밖.
    """
    ...
    if _source in ("zigbang", "직방", "naver", "네이버", "네이버부동산"):
        ...
        return result
    return _realty_price(tool_input)  # op=query, source=molit (기본) — 실거래가 공용 경로


def _resolve_query_params(tool_input):
    """기간·지역 해소 공용 전처리 (옛 execute 의 realty_price/레거시 이름 공용 블록 그대로).

    Returns: (err, region_code, start_month, end_month, count_per_month, _resolved_region)
    err 가 dict 면 호출자가 그대로 반환할 오류 응답.
    """
    ...
    if not region_code:
        ...
        if region_name:
            ...
            if rc.get("code"):
                ...
                return ({"success": False, **rc}, None, None, None, None, None)
    if not region_code:
    ...
    _now = datetime.now()
    # 상대기간: months=N → 최근 N개월 (앱 기간 select 어휘. 명시 start/end 가 우선).
    _months = tool_input.get("months")
    if _months and not start_month and not end_month:
        try:
            _n = max(1, int(_months))
        except (TypeError, ValueError):
            _n = 3
        end_month = _now.strftime("%Y%m")
        _y, _m = _now.year, _now.month - (_n - 1)
        while _m <= 0:
            _m += 12
            _y -= 1
    ...
    if not start_month:
        ...
        while _m <= 0:
            ...
            _y -= 1
        start_month = f"{_y}{_m:02d}"
    return None, region_code, start_month, end_month, count_per_month, _resolved_region


def _realty_price(tool_input):
    """실거래가(molit) 공용 경로 — 옛 execute 의 realty_price 본문 그대로."""
    err, region_code, start_month, end_month, count_per_month, _resolved_region = \
        _resolve_query_params(tool_input)
    if err is not None:
        return err
    ...
    # name: 단지/건물 명칭으로 좁히기 (앱 드릴다운 — 같은 단지 기간 추이)
    name = tool_input.get("name") or tool_input.get("q")
    if name and isinstance(result, dict) and isinstance(result.get("data"), list):
        result["data"] = [it for it in result["data"]
                          if name in (it.get("명칭") or it.get("아파트명") or it.get("건물명") or "")]
        if isinstance(result.get("summary"), dict):
            result["summary"]["총거래건수"] = len(result["data"])
...
# 2026-08-05 진짜 디스패처 전환 — 값=실행 함수 참조. --check 가 이 dict 키로 src.ops.values 와 정확 비교.
# ★2축: source(molit/zigbang/naver) 분기는 _op_query 안 — source 축은 빌드 가드 밖.
_OP_DISPATCHERS = {"realty_op": {"query": _op_query, "codes": _op_codes}}
...
def execute(tool_input: dict, context):
    ...
    if tool_name in ("realty_price", "apt_trade_price", "apt_rent_price", "house_trade_price", "house_rent_price"):
        if tool_name == "realty_price":
            return _realty_price(tool_input)
        # 레거시 이름(fall-through 보존): 기간·지역 해소만 수행 — 해소 오류면 기존처럼 그 오류를
        # 반환하고, 아니면 아래 Unknown tool 처리로 떨어진다(기존 동작 그대로).
        _err = _resolve_query_params(tool_input)[0]
        if _err is not None:
            return _err
