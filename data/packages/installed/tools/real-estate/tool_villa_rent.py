"""
연립다세대 전월세 실거래가 조회 모듈 (기간 범위 지원)
국토교통부 RTMSDataSvcRHRent API (연립/다세대)
"""
...
def get_villa_rent(region_code: str, start_month: str, end_month: str = None, count_per_month: int = 30):
    """연립다세대 전월세 실거래가 기간 범위 조회"""
    ...
    try:
        ...
        if all_rents:
            ...
            for r in all_rents:
                if r["계약유형"] == "전세":
                    try:
                        jeonse_deposits.append(int(r["보증금"].replace(",", "").strip()))
                    except:
                        pass
        ...
        if e.code in (401, 403):
            return {"success": False, "error": "연립다세대 전월세 API 권한이 없습니다(403). data.go.kr에서 '국토교통부_연립다세대 전월세 실거래가 자료'를 동일 키로 활용신청하면 바로 됩니다."}
