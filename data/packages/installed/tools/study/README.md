...
| **Wikidata** | 지식 그래프 개체 해소 | QID로 동명이인·동음이의 못박음 | 개체 후보, 구조화된 사실 |
...
### entity_op — [sense:entity] (NEW, 2026-07-11)
**Wikidata 지식 그래프 개체 해소 (무API키)**
- `op:resolve`(기본): 이름/질의 → 후보 개체별 **QID·라벨·설명**. 흔한 이름의 **동명이인·동음이의**를 QID로 못박음 (예: "이순신" → 무관 Q50184(1545–1598) vs 무장 Q12611867(1553–1611) 분리). 일반 인물 OSINT는 `search_ddg`, 국내 연구자는 `researcher_op`.
- `op:detail`: `id`(QID 예 Q42) 또는 `query` → 구조화된 **속성:값 사실**(출생·직업·소속·대표작 등, records 통화). `query`만 주면 최상위 후보 자동 해소.
- 라벨 언어 `lang`(기본 ko, 없으면 en 폴백). detail은 P-속성·Q-값 라벨을 배치 해소해 사람이 읽는 문자열로 반환.
