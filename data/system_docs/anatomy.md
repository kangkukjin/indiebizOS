...
**성장 원리**: 어휘를 조합해 긴 프로그램을 만들고, 검증된 프로그램을 관용구로 보관하며,
관용구를 다시 조합해 더 큰 일을 한 번에 맡긴다. 간결한 부품으로 복잡한 절차를 표현하고,
훈련으로 그 부품과 연결을 다듬는 언어 시스템이다. → [조합의 축적](vision.md#조합의-축적--더-큰-행위를-한-번에-표현하는-언어)
...
- **왜 도구 더미가 아니라 *언어*인가**: 표현력은 어휘 크기가 아니라 *조합*에서 나온다. 적게 고른 프리미티브 + 풍부한 조합 = 사실상 무한. 업계의 "자연어 SKILL.md" 대신 정합적 언어로 도구 폭증을 다스린다.
  - **조합 문법**: `>>`는 선언된 인자에 성공 값을 전달하고, `&`는 가지 순서의 목록을 반환한다. `??`는 실패만 대체하며 빈 목록은 정상 값이다. 조건·반복·콜백은 같은 식 규칙을 사용한다.
  - **한 호출 = 한 프로그램**: 변수는 값이며 함수 인자는 명시한다. `[table:each]{parallel:4}{return $it}`는 전건을 순서 보존 처리하고, 펼침은 `mode:"flat_map"`, 실패 수집은 `on_error:"collect"`로 명시한다. 도구·원천의 실패 증거는 결과와 함께 남는다.
  - **워크플로우·관용구 = 함수**: `[def:이름]($인자){return ...}`를 저장하고 `[fn:이름]{인자:값}`으로 호출한다. 현재 정의가 없는 기존 함수도 같은 이름 호출에서 원래 반환 계약을 유지한다. 기존 스크립트도 직접 호출할 수 있다.
...
새 작성은 [주 IBL 교재](../guides/ibl_composition.md)의 실행 가능한 조합 예제와 [명세](ibl.md)를 따른다.
객체·목록·호출 인자 안에서도 함수를 조합하며 원문 순서·실패 중단·영수증 재개를 보존한다.
순수 식 자리와 직접 제어 블록의 경계는 유지한다. [컨테이너 호출 설계](../../docs/IBL_CONTAINER_CALLS_2026_09_26.md).
주인 AI의 직결·MCP 도구는 현재 문법을 기본으로 한다. 기존 저장 원문의 판본은 실행 의미 보존용 메타데이터이며 새 교재를 따로 선택하는 기준이 아니다. 과거 문법 설명은 `docs/compatibility/`에 보존한다.
...
결합 도구의 목록·봉투 입력과 평문 반환은 명시 계약으로 연결하고, 건너뛴 분기의 미완료 증거를
...
권한·버전·스키마·상태 규칙을 적용하고 기록·이력·승인 작업·영수증·후속 처리를 한 DB에서 확정한다.
공통 업무 화면은 기존 앱의 `web_app` 연결을 사용한다. 회원 공유 자료는 명시된 업무 공간에만 저장한다.
공간당 1,000개 기록 상한 등 현재 계약은 [사용 가이드](../guides/managed_records.md),
목표와 구현 범위는 [관리되는 기록과 업무 명령 설계](../../docs/MANAGED_RECORDS_DESIGN_2026_09_20.md)를 따른다.
...
---
*최근 변경(2026-09-25): 조합 검사기와 상상훈련 56회차 도구 계약·57회차 실행 근거 수리 포인터를 반영. 이력 정본=git log·changelog.log(`[self:body]` 회상).*
