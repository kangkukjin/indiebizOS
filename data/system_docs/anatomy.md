...
사용자 메시지
  ...
  ↓ [4b] 최종 평가   목표·실행 기록·결과를 도구 없는 평가자에게 전달 → 승인 또는 1회 부분 보완
...
**모델 기어**(절약/균형/최대 레버)는 분류·실행·의식·보조 AI의 모델 등급을 정한다. 의식은 계획·중간 감독을 맡고, 보조 AI(저장 키 `평가`)는 최종평가·노트북 답변·단계 기준 검사에 쓰인다. 이미지 읽기·채점은 이미지 입력이 가능한 실행 모델을 우선 사용하고, 미지원·미확인 때 별도 비전 모델로 보완한다. 조종실 설정은 선택된 이미지 모델·대체 이유와 별도 비전·음성 받아쓰기·오디오 분석·임베딩 설정을 조회 전용으로 표시한다. **최초 숙고** 토글은 시작 시 THINK 경로를 조절한다. 중간 감독·최종 검수는 작업 조건에 따라 적용된다.
