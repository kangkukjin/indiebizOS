...
**검수 보완의 경계**: 첫 검수는 본문과 핵심 근거를 대조하고, 수정·새 조사는 실행자에게 한꺼번에 인계한다. 인계는 기존 출처의 호출 입력·증거 ID를 포함한다. `supervisor_review`는 재검수 입력에서 동일 사건 전문의 반복을 줄이고, 이미 읽은 동일 바이트와 새 변경 구간을 구분한다. 짧은 도구 결과도 실제 제시한 페이지의 읽기 근거를 남긴다. 인용 구간의 기록 누락은 한 번의 출처 재확인으로 처리하며 자동 승인하지 않는다. 계획에는 실행과 같은 `ibl_access.idioms_map`을 제공한다. [수리 근거](../../docs/EPISODE_3405_3406_REPAIRS_2026_09_11.md).
...
**내용과 기억의 근거**: 최종 텍스트 산출물은 본문 지문·읽은 범위와 의미/출처/계수 검수 근거를 연결한다. 계수는 최종 행의 필터·사건 식별자로 다시 계산하며, 행 변환 뒤 옛 criteria와 행수는 상류 근거로 분리한다. 자동 기억·기억 정리·경험 학습은 원문 선택을 우선한다. 부분 실행 용례는 전체 목표의 자동 반사 후보로 쓰지 않는다. [결과물 검증 규약](../guides/result_quality.md), [수리와 검증](../../docs/EPISODE_3395_ROOT_REPAIRS_2026_09_11.md).

**검수 후 전달**: 감독 턴에서 등록 스크립트의 환경에 비공개 작업대가 전달된다. `보고서HTML`은 공유창고 출력을 이곳의 초안으로 만들고 공개 대상 경로를 선언한다. 에이전트의 알림도 같은 검수 대기열에 보관한다(시스템 서비스 알림은 별도). 최종 의식이 초안·알림 manifest 지문까지 승인하면 하네스가 그 바이트를 원자적으로 공개하고 알림을 전달한다. 미승인·취소·지문 변경이면 대기물을 보존하며, 공개 실패는 목표 달성으로 기록하지 않는다. 임의 셸/쓰기의 모든 외부 부작용을 가로채는 보편 트랜잭션은 아니다. 구현 계약은 `supervision_delivery.py`, 수리 근거는 `docs/EPISODE_3352_REPAIRS_2026_09_10.md`.
...
**모델 기어**(절약/균형/최대 레버)는 분류·실행·의식·보조 AI의 모델 등급을 정한다. 의식은 계획·감독·최종 승인을 맡고, 보조 AI(저장 키 `평가`)는 노트북 답변·단계 기준 검사·기존 평가 경로에 쓰인다. 이미지 읽기·채점은 이미지 입력이 가능한 실행 모델을 우선 사용하고, 미지원·미확인 때 별도 비전 모델로 보완한다. 조종실 설정은 선택된 이미지 모델·대체 이유와 별도 비전·음성 받아쓰기·오디오 분석·임베딩 설정을 조회 전용으로 표시한다. **최초 숙고** 토글은 시작 시 THINK 경로를 조절한다. 중간 감독·최종 검수는 작업 조건에 따라 적용된다.
