...
사용자 메시지
  ...
  ↓ [4b] 최종 평가   의식이 규정한 턴만 승인 또는 1회 부분 보완 (EXECUTE·Reflex는 4a/4b 생략)
...
**평가와 보완의 경계**: 평가자는 직접 검색·편집하지 않고 필수 요구 누락·증거 모순·허위 수행 보고·명백한 산술 오류를 판정한다. 발견한 결함을 한꺼번에 실행자에게 넘기고 보완은 최대 한 번이다. 재평가에는 갱신된 호출 증거와 수정 응답을 전달한다. 사소한 표현이나 추가 개선 가능성을 새 목표로 만들지 않는다. 페이지 열람·인용 영수증을 모델에게 재수집시키지 않으며, 평가 중 응답·산출물·공개 대기열의 변경은 하네스가 지문으로 거부한다. [역할 분리 기록](../../docs/FINAL_EVALUATOR_SEPARATION_2026_09_11.md).
...
**모델 기어**(절약/균형/최대 레버)는 분류·실행·의식·보조 AI의 모델 등급을 정한다. 의식은 계획·중간 감독을 맡고, 보조 AI(저장 키 `평가`)는 최종평가·노트북 답변·단계 기준 검사에 쓰인다. 이미지 읽기·채점은 이미지 입력이 가능한 실행 모델을 우선 사용하고, 미지원·미확인 때 별도 비전 모델로 보완한다. 조종실 설정은 선택된 이미지 모델·대체 이유와 별도 비전·음성 받아쓰기·오디오 분석·임베딩 설정을 조회 전용으로 표시한다. **최초 숙고** 토글은 시작 시 THINK 경로를 조절한다. 의식이 규정한 턴에만 중간 감독·최종 검수를 적용한다. 의식 없는 EXECUTE·반사는 도구 실패·쓰기·알림·장시간 실행에도 평가나 자기반성 호출을 추가하지 않는다. 명시적 #think·REPAIR 등 실제 의식이 작동하는 경로는 검수 대상이다.
