...
|------------|------------------|------|
| 신경계 | IBL (6노드, 141 액션) | 감각/행동의 상시 연결 |
...
- **6개 노드, 141 액션** (engines:newspaper 은퇴, engines 변환자/emitter를 table 노드로 분리 + 라운드 2 통합 + op 어휘화 + 사용성 재감사 + 어휘 정리 + 메신저/비즈니스 IBL화 + neighbor 통합 + 폰 온디맨드 감각 삼각 + 통화 대수/문서 IR emitter + 국회도서관 국가학술정보 인물/학위논문 후)
...
- 도구 패키지: 38개, IBL: 6노드 141 액션 (라운드 2 + op 어휘화 + 사용성 재감사 + 안드로이드 통합 + 메신저/비즈니스 IBL화 + neighbor 통합 + 폰 온디맨드 감각 삼각 + 통화 대수/문서 IR emitter + 국회도서관 국가학술정보 인물/학위논문)
