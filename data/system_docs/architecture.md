...
[4b] 도구 없는 평가자 → 의식의 명시적 달성 기준을 증거와 대조, 실행자에게 최대 1회 부분 보완 인계
    ...
    ↓
[5] 통합 증류 (실행·심층·공간 기억의 가치 선별 0/1회 → 종류별 저장)
```
...
- 신규 증류는 최종 실행 모델·핀과 원문을 동결한 영속 작업이다. 가치가 불확실하면 저장하지 않고,
  빈 결과를 정상으로 둔다. 저장 오류는 판단을 재사용해 재개하며 후보 영수증으로 중복 효과를 막는다.
  포식 브라우저도 실제 관측을 가진 같은 큐를 사용한다. [구현과 한계](../../docs/UNIFIED_DISTILL_2026_09_21.md).
...
- **액션 라우팅**<!-- ROUTERS:START -->(액션 단위 실측, 합 167): handler 139 · system 18 · channel_engine 7 · driver 1 · workflow_engine 1 · trigger_engine 1<!-- ROUTERS:END -->
...
- **한도 ①사람 명령**: `thread_context.task_origin == 'user'` 인 태스크에서만(WS 채팅·`/system-ai/chat`·에이전트 명령 HTTP 4곳만 세팅). 스케줄러·자가점검·위임 사슬·외부 채널은 **미세팅 = fail-closed** → 자율 태스크는 종전대로 `[self:patch]` 제안만. ★자격은 *채팅창을 탔다*가 아니라 **누가 말했는가**다(2026-09-18): 예약 주입문은 WS 채팅 핸들러를 그대로 타므로(`calendar_actions._inject_message_via_ws`, `utterance_author:"schedule"`) 핸들러가 무조건 `'user'` 를 찍으면 예약 턴이 그랜트 자격을 얻는다 — WS 세 자리는 `set_task_origin_for_author(utterance_author)` 로 출처를 **파생**한다(owner 만 `'user'`, 그 밖은 미세팅·옛 값 소거). 회귀 `test_schedule_origin_fail_closed_2026_09_18`. **새 진입점을 만들면 `set_task_origin("user")` 를 붙일 것** — 그리고 그 표면이 `cognitive_stream` 에 닿는지는 관문(`backend/test_user_surface_pipeline.py`)이 묻는다. 묻는 것은 **누가 명령했나** 하나뿐이다. 시스템 AI 냐 프로젝트 에이전트냐(=누가 실행하나)는 한도가 아니다.
...
<!-- IBL_STATS:START -->
- 도구 패키지: **51개** (+ 백엔드 extensions **5개**), IBL: **6노드 167 액션** (sense 43·self 52·limbs 14·others 17·engines 19·table 22)
- backend **.py 437개**(test 제외, git 추적 기준) — 층 디렉토리 `base 53 · datastore 66 · ibl 58 · cognition 76 · services 39 · surface 81`(+ common 21·providers 13·channels 4·drivers 3). 가이드 **83개**(guide_db 등록 **81**)
- op 분기 액션 **77개** — 핸들러 구현은 전부 `_OP_DISPATCHERS` 표준(**34개 패키지**, 나머지는 패키지 밖 backend-native), `--check` 가 src↔tool.json↔handler 를 AST 정확 비교. 부작용 여부는 통화(`returns`)에서 분리된 `side_effect:` 선언(true 46·false 24·미선언 97)
<!-- IBL_STATS:END -->
