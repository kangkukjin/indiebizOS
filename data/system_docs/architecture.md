...
- op 분기 액션 **75개** — 핸들러 구현은 전부 `_OP_DISPATCHERS` 표준(**33개 패키지**, 나머지는 패키지 밖 backend-native), `--check` 가 src↔tool.json↔handler 를 AST 정확 비교. 부작용 여부는 통화(`returns`)에서 분리된 `side_effect:` 선언(true 45·false 24·미선언 97)
<!-- IBL_STATS:END -->
