...
|------------|------------------|------|
| 신경계 | IBL (5노드, 199액션) | 감각/행동의 상시 연결 |
...
- **5개 노드, 199 액션** (라운드 2 통합 + op 어휘화 후)
...
- 활성 프로젝트: 20개
- 도구 패키지: 36개, IBL: 5노드 199액션 (라운드 2 + op 어휘화)
...
---
*마지막 업데이트: 2026-05-31 — THINK 경로 framing 재사용 게이트(연속 turn 의식 Opus 스킵). 이전(2026-05-28): 라운드 2 정리(199 액션) + op 어휘 단일화 + 삼각 검증 인프라(--check + pre-commit + self-check 합류) + dispatcher 표준화(9 패키지) + consciousness 메타 인지 가드. 이전 갱신(2026-05-17): 인지 파이프라인 재설계, 연상기억 통합, 중급 모델 전환.*
