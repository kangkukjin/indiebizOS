...
|------------|------------------|------|
| 신경계 | IBL (5노드, 144액션) | 감각/행동의 상시 연결 |
...
- **해마**: 베이스 `ko-sroberta-multitask`에서 fine-tuning. code Top-5 94.5%, description 93.5%, **실제 런타임 검색 ~99%** (2026-06-04 Modal GPU 재학습, 144 액션 어휘). 모델은 런타임 천장이라 재학습 거의 무차별 — 어휘 아닌 intent 의미를 매칭해 vocab에 강건.
...
- **5개 노드, 144 액션** (라운드 2 통합 + op 어휘화 + 사용성 재감사 후)
...
- 해마: 144 액션 어휘 기준 재학습 완료 (2026-06-04 Modal GPU), code Top-5 94.5%/런타임 ~99% — 자동 경험 증류 (점수 < 0.7 시)
- 심층메모리: 같은 모델 공유로 시맨틱 검색 (2026-05-16 도입)
- 상세 (단계별 흐름·증류 조건·DB 스키마·학습 절차): **execution_memory.md**

### 도구 패키지 시스템 (노드 구현체)
36개 패키지가 IBL 노드의 실제 구현체로 동작. 폴더 기반 탐지 + 동적 로딩. op-bearing 9 패키지는 `_OP_DISPATCHERS` 표준 채택(2026-05-28) — `build_ibl_nodes.py --check`가 AST 정확 비교로 src↔tool.json↔handler 일치 검증. 패키지 구조·설치·생성 절차는 **packages.md** 참조.
...
- 활성 프로젝트: 20개
- 도구 패키지: 36개, IBL: 5노드 144액션 (라운드 2 + op 어휘화 + 사용성 재감사)
...
---
*마지막 업데이트: 2026-06-04 — IBL 사용성 재감사 종결(144 액션): travel/trigger/iframe/storage/world_bank 내부해소·자연어화 + ACTION_PARAM_ALIASES 중앙 별칭을 비-handler 라우터까지 적용(잠복결함 수정) + 해마 144액션 어휘 재학습(런타임 ~99% 천장, 재학습 무차별로 판명). 이전(2026-05-31): THINK 경로 framing 재사용 게이트. 이전(2026-05-28): 라운드 2 정리 + op 어휘 단일화 + 삼각 검증 인프라 + dispatcher 표준화.*
