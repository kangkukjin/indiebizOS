...
- **액션 라우팅**<!-- ROUTERS:START -->(액션 단위 실측, 합 168): handler 140 · system 18 · channel_engine 7 · driver 1 · workflow_engine 1 · trigger_engine 1<!-- ROUTERS:END -->
...
- backend **.py 463개**(test 제외, git 추적 기준) — 층 디렉토리 `base 56 · datastore 67 · ibl 77 · cognition 77 · services 39 · surface 81`(+ common 23·providers 13·channels 4·drivers 3). 가이드 **87개**(guide_db 등록 **84**)
- op 분기 액션 **77개** — 핸들러 구현은 전부 `_OP_DISPATCHERS` 표준(**34개 패키지**, 나머지는 패키지 밖 backend-native), `--check` 가 src↔tool.json↔handler 를 AST 정확 비교. 부작용 여부는 통화(`returns`)에서 분리된 `side_effect:` 선언(true 46·false 25·미선언 97)
<!-- IBL_STATS:END -->
