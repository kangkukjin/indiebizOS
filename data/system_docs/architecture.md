...
- **액션 라우팅**<!-- ROUTERS:START -->(액션 단위 실측, 합 153): handler 125 · system 18 · channel_engine 7 · driver 1 · workflow_engine 1 · trigger_engine 1<!-- ROUTERS:END -->
...
<!-- IBL_STATS:START -->
- 도구 패키지: **41개** (+ 백엔드 extensions **5개**), IBL: **6노드 153 액션** (sense 41·self 50·limbs 14·others 17·engines 9·table 22)
- backend **.py 300개**(test 제외, git 추적 기준) — 층 디렉토리 `base 26 · datastore 40 · ibl 41 · cognition 48 · services 28 · surface 61`(+ common 16·providers 13·channels 4·drivers 3). 가이드 **71개**(guide_db 등록 **70**)
- op 분기 액션 **72개** — 핸들러 구현은 전부 `_OP_DISPATCHERS` 표준(**29개 패키지**, 나머지는 패키지 밖 backend-native), `--check` 가 src↔tool.json↔handler 를 AST 정확 비교. 부작용 여부는 통화(`returns`)에서 분리된 `side_effect:` 선언(true 42·false 16·미선언 95)
<!-- IBL_STATS:END -->
