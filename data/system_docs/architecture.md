...
[0] 연상 단계 (해마 + 심층메모리, 단일 검색)
    ...
    ↓
[1] Reflex 분기 (호출 측 결정)
    ├─ top_score ≥ 0.88 → EXECUTE + reflex_hint (무의식 스킵)
    └─ 미만 → [1B] 무의식 (경량 AI) → EXECUTE/THINK
    ↓                                              ↓ THINK
EXECUTE/Reflex                          [2] 의식 에이전트 (본격 AI)
    │                                      task_framing + 달성 기준
    ↓                                              ↓
[3] 실행 에이전트
...
- 해마: 2,019건 학습, Top-5 94.8% — 자동 경험 증류 (점수 < 0.7 시)
- 심층메모리: 21개 DB / 162건, 같은 모델 공유로 시맨틱 검색 (2026-05-16 도입)
- 상세 (단계별 흐름·증류 조건·DB 스키마·학습 절차): **execution_memory.md**

### 도구 패키지 시스템 (노드 구현체)
34개 패키지가 IBL 노드의 실제 구현체로 동작. 폴더 기반 탐지 + 동적 로딩. 패키지 구조·설치·생성 절차는 **packages.md** 참조.
...
---
*마지막 업데이트: 2026-05-17 — 인지 파이프라인 재설계(Reflex 분기 외부화), 연상기억 통합(해마+심층메모리 시맨틱), 중급 모델 전환 일관성 확보, 해마 재학습(Top-5 94.8%)*
