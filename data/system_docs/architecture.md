...
- **액션 라우팅**<!-- ROUTERS:START -->(액션 단위 실측, 합 154): handler 126 · system 18 · channel_engine 7 · driver 1 · workflow_engine 1 · trigger_engine 1<!-- ROUTERS:END -->
...
7. **수리 교리는 소비처에 둔다 — 경계는 범위가 아니라 원인 사슬(2026-09-02).**
   실행자가 받는 것은 의식의 task_framing(1-2문장)뿐이라, 의식 프롬프트에만 적힌 교리는 병목을 지나며 접히고 실행자 base 프롬프트의 "요청 밖 개선 금지"만 온전히 도달했다 — 결과가 "뿌리는 범위 밖이라 안 고침 + 고칠까요?". 그랜트가 살아 있는 턴에는 `fragments/13_repair.md` 가 turn_context 로 실행자에 직접 실린다: 수리 단위=원인(사슬 위면 명령 안, 값 보정·예외 삼키기·특례 분기=미룸) · '명령 밖'=사슬 밖 · 되묻기=정책 변경·파괴적 변경 둘뿐 · 보고 형식 [증상→원인 사슬→고친 자리→손대지 않은 것→적용 상태]. 평가자는 그 형식으로 '뿌리를 안 고친 것'을 미달로 잡되 '뿌리를 말한 것'은 감점하지 않는다.
...
<!-- IBL_STATS:START -->
<<<<<<< HEAD
- 도구 패키지: **41개** (+ 백엔드 extensions **5개**), IBL: **6노드 154 액션** (sense 43·self 50·limbs 14·others 17·engines 8·table 22)
- backend **.py 306개**(test 제외, git 추적 기준) — 층 디렉토리 `base 29 · datastore 41 · ibl 42 · cognition 48 · services 28 · surface 62`(+ common 16·providers 13·channels 4·drivers 3). 가이드 **71개**(guide_db 등록 **70**)
- op 분기 액션 **74개** — 핸들러 구현은 전부 `_OP_DISPATCHERS` 표준(**30개 패키지**, 나머지는 패키지 밖 backend-native), `--check` 가 src↔tool.json↔handler 를 AST 정확 비교. 부작용 여부는 통화(`returns`)에서 분리된 `side_effect:` 선언(true 42·false 18·미선언 94)
=======
- 도구 패키지: **41개** (+ 백엔드 extensions **5개**), IBL: **6노드 154 액션** (sense 43·self 50·limbs 14·others 17·engines 8·table 22)
- backend **.py 306개**(test 제외, git 추적 기준) — 층 디렉토리 `base 29 · datastore 41 · ibl 42 · cognition 48 · services 28 · surface 62`(+ common 16·providers 13·channels 4·drivers 3). 가이드 **71개**(guide_db 등록 **70**)
- op 분기 액션 **74개** — 핸들러 구현은 전부 `_OP_DISPATCHERS` 표준(**30개 패키지**, 나머지는 패키지 밖 backend-native), `--check` 가 src↔tool.json↔handler 를 AST 정확 비교. 부작용 여부는 통화(`returns`)에서 분리된 `side_effect:` 선언(true 42·false 18·미선언 94)
>>>>>>> e58f3c6a (계기 은퇴(engines:icon·오디오 브리핑 계기) + 런처 계기 목록 배경 재확인)
<!-- IBL_STATS:END -->
