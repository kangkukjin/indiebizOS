...
시스템 AI의 채팅 실행 서비스(`chat_streams.py`)는 위임 발생 시 태스크를 유지해야 합니다.

연결·실행 수명은 `chat_runs`가 나눈다. 취소 Event와 조향 대상은 실행별로 보관하고 워커가 같은 문맥을 승계한다. 연결 종료는 조향 입구만 닫고 이미 승인한 작업은 계속한다. 같은 client_id로 재접속해도 옛 실행의 직접 응답·늦은 종료가 새 연결에 닿지 않는다. 타임아웃 뒤 결과 보관/자동 보고는 기존 전달 계약을 따른다. 작업 TLS는 워커에만 쓰며 공유 이벤트 루프에는 설정·초기화하지 않는다.
