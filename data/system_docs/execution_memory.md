...
### 현재 성능 (2026-05-31 Modal GPU 재학습 — 199 액션 / op 어휘 기준)

| 지표 | Baseline (범용 ko-sroberta) | Fine-tuned 해마 | 개선 |
|---|---|---|---|
| Top-1 (code) | 22.0% | **75.2%** | +53.2%p |
| Top-3 (code) | 35.2% | **90.8%** | +55.6%p |
| Top-5 (code) | 42.0% | **95.3%** | +53.3%p |
| Top-5 (description) | 58.1% | **81.7%** | +23.6%p |

- 학습 데이터: DB 2,267건 + 규칙 변형 → 318 고유 패턴 (199 액션 / op 어휘 100% 커버)
- 학습 환경: **Modal 서버리스 GPU(T4, cuda)**, batch=16, max_seq 128, 10 epoch(best @9). 8GB 맥에어 OOM 제약을 클라우드로 오프로드.
- 베이스 모델: `jhgan/ko-sroberta-multitask`

> **batch 스윕(2026-05-31, seq128):** b4=91.7%, **b16=95.3%(채택)**, b32=94.7%, b64=94.0%. 맥에어 시절 batch=4(OOM 회피)가 품질을 깎고 있었음 — GPU로 batch=16까지 풀어 +3.6%p 회복. (옛 311 액션 어휘 기준 2026-05-16 학습은 95.6%였으나, op 어휘 재정의로 비교 기준이 다름.)
...
```
data/training/
├── ibl_training_balanced_20260516.json   ← 현재 학습 데이터 (2,019건, 311 액션 100% 커버)
...
*마지막 업데이트: 2026-05-31 — 해마 Modal GPU 재학습(199 액션 op 어휘, batch=16, Top-5 95.3%). 이전: 2026-05-17 단일 검색 일원화, 심층메모리 시맨틱 검색 도입, 점수 정규화, 의식/평가 입력 정리*
