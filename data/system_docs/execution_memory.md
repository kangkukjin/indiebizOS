...
[1] Reflex 분기 (호출 측에서 결정)
    ├─ top_score ≥ 0.88 → 무의식 스킵, 곧장 EXECUTE + reflex_hint
...
### 현재 성능 (2026-05-16 재학습)

| 지표 | Baseline (범용 ko-sroberta) | Fine-tuned 해마 | 개선 |
|---|---|---|---|
| Top-1 (code) | 19.6% | **65.9%** | +46.2%p |
| Top-3 (code) | 38.2% | **90.2%** | +52.0%p |
| Top-5 (code) | 45.3% | **94.8%** | +49.5%p |
| Top-5 (description) | 68.6% | **96.0%** | +27.4%p |

- 학습 데이터: 2,019건 (이전 535건 → 4배 확장)
- 311 액션 100% 커버리지 (이전 79.4%)
- 학습 시간: 5h 35m (10 epoch, MPS, batch_size=4)
- Best epoch: 9 (검증 점수 0.948)
...
| `REFLEX_SCORE_THRESHOLD` | 0.88 | 이 이상이면 의식·무의식 모두 스킵하고 EXECUTE |

### 점수 정규화 (2026-05-17)

이전에는 FTS5 단독 폴백 시 raw BM25 점수(0~10+)가 반환되어 REFLEX 임계값 0.88이 무력화되는 문제가 있었다. `_combine_scores` 외에 단독 분기에도 max-normalize를 적용해 모든 경로에서 점수 0~1을 보장한다.
...
*마지막 업데이트: 2026-05-17 — 단일 검색 일원화, 해마 재학습(Top-5 94.8%), 심층메모리 시맨틱 검색 도입, 점수 정규화, 의식/평가 입력 정리*
