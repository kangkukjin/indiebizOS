...
객체 필드·목록 원소·호출 인자는 함수·도구 호출과 `>>`·`&`·`??` 조합을 받는다.
필드·원소는 원문 순서로 평가하며 인자를 완성한 뒤 바깥 호출을 실행한다. 실패 시 뒤의 값은 실행하지 않는다.
자동 병렬·롤백은 없고 같은 함수의 두 위치는 두 호출이다. 각 호출은 기존 권한·공유 예산·취소·출처·
영수증 경로를 쓰며, 같은 실행 재개에서 완료 영수증을 재사용하고 완료 불명 호출은 차단한다.
산술·비교·내장 함수 인자·조건·람다·보간·기본값은 순수 식 자리다. 그 안의 호출은 먼저 변수에 받는다.
객체·목록 안의 직접 제어 블록은 허용하지 않는다. 앞 문장이나 명시 함수 본문에 둔다.
설계·검증: [컨테이너 호출](../../docs/IBL_CONTAINER_CALLS_2026_09_26.md).
