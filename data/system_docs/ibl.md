...
IBL의 진짜 엔진은 142개 액션이 아니라 `>>`(순차) `&`(병렬) `??`(폴백) + 접근 + 가이드다.
**141 × 조합 × 외부 어휘 = 사실상 무한.** 더 많은 단어 ≠ 더 강한 언어.
