...
1. **문법**: `[node:action]{params}` 패턴, 연산자(`>>` 순차, `&` 병렬, `??` 폴백, `;` 독립 문장), **병렬 괄호 분기**(`A & (B >> C) >> [table:merge]` — 분기 하나에만 전처리 파이프, 2026-08-19 개정·괄호 안=일반 step 파이프만), `$변수`(맨몸 `$이름` = 괄호 `${이름}`, 2026-08-22 — 경계가 `\w` 라 한글 조사·단위가 이름에 먹히는 것을 괄호로 끊는다; **2026-09-04 개정 둘**(관용구 리허설이 적발): ①괄호형이 **따옴표 밖**(수치 자리 `n: ${개수}`)에서도 맨몸형과 같은 참조로 파싱된다 — 옛 파서는 `{` 에서 끊겨 구문 오류 ②산문 결과(brief·document 같은 문자열 통화)에 `.message`·`.text` 를 물으면 그 산문 — dict 봉투의 message 와 같은 이름이 같은 것을 가리켜 `$본문.message` 가 결과 모양에 따라 죽고 살던 비대칭 소멸(다른 경로는 종전대로 정직 오류). 같은 날 어휘 개정: `[sense:search_youtube]{queries}` 배치 팬아웃(`sense:search` gnews 의 queries 와 같은 통화 — query 태그·video_id 중복 제거·sections); **2026-09-05 어휘 개정**: `[table:chunk]{size, by, overlap, field, text}` — 긴 문자열 하나 또는 본문 열이 있는 items(자막 구간)를 덩이 items 로(행·문단·줄·글자 경계, 정직 상한). 자막 84K 가 brief 입력 상한 6만 자에서 죽던 자리 — 이제 자르기→덩이마다 brief→종합 brief 관용구로 조립된다(표준 코어 `table` 의 변환자, data-ops 패키지); **2026-09-06 언어 개정 — 형태 보존**(사용자 판정 "1·2·3 다 근본 집행", ep2882 AI 동향 57액션 프로그램 step 27): 변환자는 통화의 *형태*를 보존한다 — items 가 들어오면 items 가 나가고, 표형(columns/rows)은 **명시 표형 입력**을 받은 자리에서만 유지된다(`union`·`select`·`rename`·`groupby` 가 items 를 표로 바꿔 내던 것을 폐지 — 선언 `emits: items` 와 실물이 한 벌). 빈 items 도 통화다(표 경로 승격 시 0행 표) — `union결과 & 0행items >> [table:union]` 이 '1=table, 2=items' 로 죽던 자리. 같은 날 바인딩 수리: `${x.items.0.path}` 의 items 파생 가드가 경로 앞의 점 때문에 한 번도 돌지 않았다(09-05 수리의 유령). 가드 `backend/test_language_revision_form_preservation_2026_09_06.py`; **2026-08-27 개정 둘**: ①치환 의미론 — 통짜 `.path` 참조는 원형(list/dict/스칼라)으로, bare=v4 추출·문장 속=문자열화 불변(**2026-09-05 수리**: `$x.items` 는 table·blocks 로 방출된 통화에서도 파이프 이음매와 같은 판정기 `derive_items` 로 파생된다 — union·groupby·select 결과의 `.items` 참조가 "items 필드가 없습니다" 로 죽어 다단 union 조합이 막히던 자리. 효과·스칼라 봉투는 종전대로 정직 오류) ②`$변수 >> [액션]` 파이프 머리 허용 — 통화 방출, 미할당=파싱 에러(**2026-09-01 개정**: 같은 규약이 병렬 분기 자리에도 — `$a & $b >> [table:join]`, 표기·판정은 파이프 머리와 한 벌. 폴백 `??` 자리는 미개방); **2026-08-28 개정 넷**(사용자 판정 "언어의 한계는 다 고쳐" — 보고서 완성 프로그램 실측이 적발한 표현 공백): ㉠확장 경로 **괄호형 전용** — `${x.items.*.f}` 열 벡터 사상(원형 list — not_in 안티조인의 value 자리, 부분 결측=None 자리·전 결측=정직 오류), 끝의 `?` 옵셔널(`${x.y?}` 결측 경로·`${x?} >>` 미기록 변수를 오류 대신 빈 값/빈 통화로 — 물음표를 적은 자리만, 맨몸 경로는 종전대로 정직 오류) ㉡`[table:each]{on_error: "keep"}` — 실패 행을 `{원 행…, _error}` 로 통화에도 흘려 성공·실패 분기(filter _error eq null/exists)·교체·재시도를 문장 안에서 조합(keep 은 전량 실패도 통화로) ㉢파이프 세그먼트 if 블록의 불일치 = **직전 통화 통과**(+`_if_skipped` 표식) — 빈 통화 폴백 관용구 `A >> [if: empty($items)]{대안}` 성립(옛 message 봉투 대체는 은퇴 등록) ㉣document blocks 의 `when` 조건부 절 — 값이 비면 블록 생략+`blocks_omitted` 신고)·`$file:N`, if/case/goal 블록, 파이프 설탕(`| where:` 등 → `[table:*]` desugar), **조건 언어**(2026-08-22 개정, `ibl_predicates.py`: 좌변 = `node:action{…}[.경로]` 소스 참조 | `$변수[.경로]`(앞 문장 결과, 실행 없음) | `count()`/`empty()`/`exists()` | `[table:brief]{…}` AI 술어(message 가 값) · 연산자 `== != > >= < <= matches`(정규식) · `and`/`or`/`not`·괄호 · 판정 불능≠거짓; **2026-09-05 값 의미론 개정**(사용자 판정): 달력 날짜 `YYYY-MM-DD` 와 시간대 있는 시각의 순서·동등은 그 시각의 *자기 시간대* 달력 날짜로 판정 — `date >= "2026-08-15"` 하한이 `…T06:36:45+00:00` 에 판정 불능으로 죽던 자리. 시각 있는 naive vs aware 는 종전대로 판정 불능. where DSL 의 `gte`/`lte` 별칭은 조건 언어와 한 벌), **봉투 다이어트**(에이전트 경계의 `results[]`=step 요약·`final_result`=원형, `verbose:true` 로 원형 — 하네스 이음매 `execute_ibl` 하나의 응답 모양이라 언어 밖. ★**진단은 다이어트 대상이 아니다** — 2026-08-28: 부분 실패의 `errors[]` 가 다문장 요약에 통째로 접히고 `message` 만 'errors 참조'를 가리키던 침묵 모순을 수리해, 요약이 **상한 3건·건당 300자 다이제스트 + `error_count`** 를 함께 나른다. 넘치면 `errors_digest_truncated` 로 절단을 신고한다(침묵 클램프 금지). 가드 `backend/test_envelope_errors_digest.py` E1~E4) · `[self:write]{spill:true}` 스필 싱크(뒤 step 엔 `{items:[], ref}` 참조만). **제어 블록**(2026-08-22 M3·M4): `[try]{…} [catch]{…} [finally]{…}`(catch 안 `$error`), `[on_error: stop|skip|null]` 문장 접두(`>>` 실패 규약 — 기본 stop, skip/null 은 건너뛴 step 을 봉투에 신고; **2026-09-05 수리**: 할당 문장 앞에도 선다 — `[on_error: null] $x = [self:read]{…}` 가 종전엔 파싱 실패였고, 이제 대체 통화가 곧 `$x` 의 값이다. 같은 날 실패 진단 개정: 죽은 할당 문장의 원인이 `root_failures` 와 error 문장에 실리고 뒤 문장의 `$x` 참조는 "아직 값을 기록하지 않았다" 대신 그 원인을 인용하며, 봉투 error 가 뿌리와 연쇄를 가른다 — ep2827 '독립 문장 8개 실패' 가 뿌리 1개를 숨긴 자리), `??` 가지의 괄호 파이프 `A ?? (B >> C)`, `[repeat: N | until 조건 | while 조건, max(필수), every(≤60s), collect, as]{…}`(문장 안 결정론 반복 — 벽시계 300s 상한 신고, 더 길면 goal/schedule). 기능어 `[table:reduce]{init, step, as}`(식 한 줄 fold). **자동 스필**(이음매 통화 200K자 초과 → `data/spill/` 참조, 소비자 투명 해소, 24h 캐시 GC)·**재개**(실패 봉투 `resume:{from_step, prev_ref}` → `execute_ibl(code, resume)` — 같은 code 를 그 step 부터; **2026-09-06 부분 성공 봉투 재사용**: 문장이 하나라도 죽은 봉투는 살아 있는 `$변수` 원형을 스필해 `resume_vars{vars_ref, vars, failed_vars}` 로 싣고, 실행자는 죽은 문장만 고쳐 그 문장(들)을 code 로 보내며 `resume:{vars_ref}` 를 실으면 산 변수가 재실행 없이 주입된다(파서 `preset_vars`·실행기 `_preset_results`, 슬롯 대역 `RESUME_SLOT_BASE`). ep2882 에서 39/39 step 이 살아 있는데 전체를 다시 돌려 검색·`[table:ai]` 를 두 번 지불하던 자리. 가드 `backend/test_resume_vars_2026_09_06.py`; **2026-09-06 언어 개정 — 턴 범위 변수**(사용자 판정, `$rN` 번호 핸들·resume 명시 확장 대신): `$이름 = …` 로 할당한 변수의 수명은 프로그램 한 번이 아니라 **턴**(같은 task_id)이다 — 다음 execute_ibl 호출이 resume 없이 `$이름` 을 그대로 본다(노트북 REPL 모형; 파서 `preset_vars`·실행기 `_preset_results` 같은 기판, 저장소 `data/spill/turn_vars_*.json` 24h GC). 규약: 프로그램 안 재할당이 덮는다 · 명시 `resume:{vars_ref}` 가 같은 이름이면 명시가 이긴다 · task_id 없는 직접 표면은 턴 범위 없음(침묵 폴백 금지) · 예약 이름 미주입 · 봉투 `turn_vars{injected, live, too_large}` 가 정직하게 말한다. 왜: 벽시계≈출력 토큰÷v 인데 모델이 앞 호출 결과(JSON·행·본문)를 다음 호출에 다시 치던 자리(ep2890 22.8K자·ep2884 112K자) — 있는 것은 치지 말고 가리킨다. 번호 핸들을 택하지 않은 이유: 코퍼스에 남으면 재사용 불가·뜻 없음. 가드 `backend/test_turn_vars_2026_09_06.py`; 같은 날 **되받아쓰기 관문**(엔진 규약, 사용자 판정 "경우마다 막을 수 없다 — 일반적으로 막아라"): execute_ibl 초크포인트가 모델이 친 긴 문자열 인자(≥200자, `$file:N` 해소 뒤)를 이 턴의 변수 값·직전 결과 그림자(이름 없는 최근 봉투 8개)·대상 파일 현재 내용과 대조 — 그대로 겹치는 조각(≥40자) 합 ≥2,000자 또는 숫자·URL 토큰 ≥20개 중 ≥50% 가 이미 있으면 봉투 `retyped{level:warn, verbatim_chars, data_tokens, sources, hint}`(실행은 함), 대상 파일과 ≥8,000자 겹침(통째 재작성)은 실행 전 `error_type:"retyping"` 거절. 어휘 이름 무관·문자열 일치만, 임계는 `data/lifecycle_policy.yaml retyping:` 데이터(`ibl_retyping.py`). 함께: 의식 초안 인계 — 검증 통과 imagined_ibl 은 턴에 보관되고 실행자는 `execute_ibl(code: "$초안")` 로 그대로 실행(재타이핑 0) · 반성 수치 줄·턴 종료 `[출력해부]`(typed·retyped·pointed·fn) · 평가자 응답 형식 "미충족 항목만 ≤5줄" · 최종 응답 규약 "파일 산출물이면 경로+요지 3줄". 가드 `backend/test_retyping_gate_2026_09_06.py`; **2026-09-06 봉투 기본값 반전**(사용자 승인 — 프로그래매틱 툴 호출·MemEx 와 같은 방향 \"모델이 보는 것=모델이 요구한 것\"): 에이전트 경계(호출 통로 ≠ app)의 `final_result` 는 큰 구조 데이터(items/표 8행 초과·직렬화 ≥3,000자)이면 앞 8행 + `_preview{shown,total,chars,columns,note}`, 긴 산문(≥12,000자)은 앞 12,000자 미리보기. 작은 결과·효과·스칼라·JSON 문자열은 불변, 단일 step 원형도 같은 규칙. 전체 값은 턴 저장소(변수·그림자)에 살고 `verbose:true` 가 전체 요구. 임계 `lifecycle_policy.yaml envelope_preview:`(`ibl_envelope.preview_envelope`). 가드 `backend/test_envelope_preview_2026_09_06.py`; 같은 날 **`[table:each]{parallel}`**(언어 개정, 사용자 판정 "그 셋으로 착수"): 기본 1, 2~8 이면 준비(치환·파싱)·집계는 입력 순서대로, 실행만 스레드 풀 — 결과 순서·봉투 계약 불변, 하위 스텝 예산은 순서 누적으로 병렬 밖 행을 실행하지 않음, on_error stop 은 이미 돈 뒤 행의 결과를 버림(신고), thread-local·궤적은 snapshot/restore 승계. ep2897 자막→struct 원샷 6건 순차 10.8분 자리. 어휘 개정: `[self:struct]{known, instruction}` + 근거 앵커 확장(모델은 `_quote` 첫 구절만, 대조 뒤 코드가 문장 끝까지 확장 `quote_expanded`) — 품질을 지키며 출력을 줄이는 2단 추출(제목 → 절제 → 선정분만 상세)의 재료. 가드 `backend/test_each_parallel_struct_anchor_2026_09_06.py`)·**트레이스백**(2026-08-27: 모든 실패 봉투에 `traceback: {frames(바깥→안쪽 경로 — pipeline·each·block·parallel·fallback·workflow), error(원형), error_type(tool_error|exception|syntax|binding|quality†), input(실패 지점 입력 통화 요약), py_tail(예외 꼬리)}` — each 행·병렬 가지 같은 부분 실패에도 예외 없이 붙는 경계 규약(동일 오류 반복은 상세를 첫 발생에만 원형, 이후 `detail_at` 참조), 다이어트 밖. †quality 는 품질 계약이 쓴다. 정본 = `docs/IBL_TRACEBACK_HANDOFF.md`)·**criteria 품질 계약**(2026-08-27 언어 개정: `criteria` 는 `project_id`·`scope` 와 같은 런타임 메타 param — leaf 액션에 선언하면 실행 직후 판정자(모델 수준=**기어 평가 축** role:evaluate — GoalEval 평가자와 같은 축, 절약·균형=경량·최대=고급)가 심사, 미달 시 사유를 instruction 에 얹어 **재시도 1회**(ai_call+instruction 선언 액션만), 그래도 미달이면 `error_type:"quality"` 실패로 그 step 이 트레이스백에 찍힌다(`rejected_result` 동봉). 판정 불능=통과+`unjudged` 신고, 재시도 통과=`_criteria_retried` 정직 표지. ★액션이 criteria 를 자기 param 으로 선언하면(image_read op:critic) 그 액션의 것 — 엔진이 가로채지 않는다. 그 자리의 판정자는 **적용한 기준표를 반환에 신고**한다(2026-08-28 `rubric`: `criteria:<이름>` | `preset:general` | `preset:slide_illustration`) — criteria·preset 둘 다 미지정이면 일러스트 기준표가 조용히 적용돼 문서·장부 스크린샷이 "한글이 들어갔다"로 떨어지던 자리라, 기본값일 때는 경고 한 줄까지 실어 **무슨 잣대로 본 합격인지 모르는 상태**를 봉인했다(prescreen 0층 단락은 기준표 심사가 아니므로 불변). 블록에는 비적용(goal 은 자기 판정 보유). 정본 = `docs/IBL_QUALITY_CONTRACT_HANDOFF.md`) 은 엔진 규약. **M6(2026-08-22)**: 식 할당 `$n = 0` / `$n = $n + 1` / `$s = $r.count * 2`(한 줄 식, `common/safe_expr` — 우변이 액션이 아니면 식), 블록을 파이프 세그먼트로(`[A] >> [if: count($items) > 0]{…} >> [B]`, `[repeat:…]{…} >> [table:dedup]` — 블록은 직전 통화를 `$items` 로 보고 몸에 넘기며 결과가 다음 통화), `while` 이 몸 변수를 봄(회차마다 현재 값으로 몸 치환·루프 뒤 바깥 `$n` 최신값), `$return = …` 반환 규약(`[self:workflow]` run). ★블록 몸의 `$변수`는 파서가 아니라 **실행기가 실행 직전 값으로 치환**한다(안쪽 파이프 인덱스와 바깥 인덱스 충돌 방지). **함수**(2026-09-05 언어 개정, 사용자 판정 "함수 문법 개정 진행"): `[def: 이름]{ …줄들… }` 로 같은 프로그램 안에 정의(몸은 문자열이 아니라 줄, 닫힌 스코프 — 미할당 `$이름`=시그니처, 바깥 변수 불가시, 몸 `todo`=이름만 걸기; **2026-09-07 개정 — 자유 변수는 자리를 가리지 않는다**(사용자 판정 "언어 한계가 있다면 그걸 극복해야지"): 미할당 `$이름` 이 파라미터 값뿐 아니라 **파이프 머리**(`$목록 >> [액션]`)·**병렬 분기**(`$옛것 & $새것`)에 서도 시그니처 슬롯이다. 종전엔 그 두 자리에서 파싱 에러라, 함수·관용구는 통화를 하나(파이프로 흘러드는 앞 통화)만 그것도 첫 문장에서만 받을 수 있었고 — 통화를 둘 이상 받는 관용구(원장 누적·델타·join)는 말할 수 없어 몸에 도메인 머리를 박는 길밖에 없었다(이름 38건이 전부 도메인에 얼어붙은 구조적 원인). 2026-09-01 병렬 분기 개정이 고친 것과 같은 **자리의 비대칭**. ★최상위 프로그램의 미할당은 종전대로 정직한 파싱 에러다(오타를 잡는 자리) — 함수 몸 파싱의 단일 입구는 `ibl_parser.parse_function_body`, 관문 `backend/test_free_var_signature_2026_09_07.py`), `[fn:이름]{인자}` 로 호출(`[node:action]` 모양 — 파서가 프로그램 끝에서 정의를 붙이므로 정의가 호출 뒤에 와도 됨, 파서가 정의 문장을 앞으로 올림; 정의가 없으면 저장 워크플로 원장의 같은 이름 — 두 길이 `[fn:]` 하나로). 반환=`$return` 값(식 할당 봉투가 아니라 값, 리스트면 items 로 승격) 아니면 마지막 통화, 앞 통화는 몸의 첫 문장으로, 인자 누락·todo·정의도 원장도 없음·깊이 상한(5)=정직 실패. 몸통은 step 밖 프로그램별 정의 표(id 참조)에 산다 — 재귀 정의를 step 에 박으면 순환 구조. 검증기(소유 필터·액션 실존·param·runnable)는 `fn` 을 어휘가 아닌 호출로 본다. 정본 `docs/IBL_FUNCTION_SYNTAX_HANDOFF.md`. **예약어**(블록 키워드 — 어휘 이름으로 쓸 수 없음): `if else case goal repeat try catch finally on_error def` + 의사 노드 `fn` + 변수 `$items $it $i $error $return`. 파서(`ibl_parser.py`)는 이름-무검증 — 모르는 어휘도 문법적으로 파싱한다. **파라미터 키**(2026-09-05 언어 개정, 사용자 판정): 따옴표 없는 키는 구조 문자(공백·`: , { } [ ] = 따옴표`)가 아닌 글자의 연속이면 무엇이든 된다 — `{set: {㎡당만원: "…"}}`·`{%비율: 2}` 처럼 단어 문자로 시작하지 않는 열 이름도 값 자리와 같이 키 자리에서 받는다(옛 판은 식별자만 받아 "해석된 키: [없음]" 으로 죽고 진단이 값 이스케이프를 가리켰다). **2026-09-06 언어 개정 — 통화 경계의 승격**(55회차 설계 왕복, 사용자 판정 "국소만 빼고 다 고쳐"): ①`[table:each]{collect: true}` — do 가 통화를 **안 내는** 행(효과·스칼라: 보고서 계산·도면 생성·write 의 path)의 결과를 `{원 행…, 결과 키…}` 행으로 승격(원 행 이름 우선, 겹친 결과 키는 `_2` + `collect_renamed` 신고, 봉투 `collected_rows`). 승격 규칙은 union 의 효과 행과 **같은 함수**(`common.currency.effect_row`) — 두 조합자가 같은 봉투를 정반대로 다루던 자리를 한 함수로 닫음. 기본은 종전(원 행 통과 + passthrough_rows), 통화를 내는 do 에는 뜻 없음. ②`[table:document]{as: "images", src_field, caption_field}` — items N행 → image 블록 N개(도면·사진·차트, 행 수가 변수여도; src 관습 이름 자동 탐지·빈 칸 행은 `images_skipped` 신고, `cards`/`table` 강제도 가능). ③카탈로그 ⟨키: …⟩ — 통화가 아닌 봉투(효과·스칼라)의 필드를 shape 스윕이 적고(kind scalar, fixture 없는 액션은 실사용 원장 `action_health.keys` 에서 `--from-health` 수확), ⟨열⟩(통화)과 라벨을 갈라 정적 검사기는 ⟨키⟩를 열로 쓰지 않는다. ④엔진 관문(언어 밖): 구현이 쓰기 원시에 닿는 scalar/items 액션은 `side_effect: true|false` 선언 필수(빌드 `--check`, `scripts/iblbuild_side_effect_scan.py` — arch_create 가 dry-run 'read' 로 표시되던 자리). 가드 `backend/test_imagination_round55_repairs.py`. **2026-09-07 언어 개정 — 키 자리는 속성 집합**(사용자 판정 "키 자리 전체 개정"): 관계대수의 키 자리(`[table:groupby]{by}`·`[table:sort]{by}`·`[table:dedup]{by}`·`[table:merge]{by}`·`[table:join]{on}`)는 스칼라 하나가 아니라 **속성 집합**을 받는다 — 목록이면 복합키다(교차집계·다단계 정렬·복합 조인키). 목록의 뜻은 슬롯이 정한다는 규약의 명문화: 값 자리의 목록은 종전대로 "항목마다 실행"(`[table:each]`)이지만, 키 자리의 목록은 **한 번의 복합키**다(`select{columns}` 가 이미 그 선례). 그룹 결과는 키마다 **제 열**로 나온다(합성 문자열 한 칸으로 접지 않는다 — 하류 filter/sort 가 그 열을 그대로 쓴다). sort 의 `desc` 는 전 키에 같이 걸린다(방향을 섞으려면 덜 중요한 키부터 두 번 잇는다 — 안정 정렬). 왜: ep2532(09-01)·ep2951(09-07) 부동산 보고서가 `by: ["아파트명","계약유형"]` 로 죽었다 — 관문은 정직했으나 처방("항목마다 실행하려면 each")이 이 자리에서 틀렸고(each 로 돌리면 키마다 따로 그룹핑돼 **의도와 다른 답이 success 로 나간다**), 실행자는 유형마다 filter 를 따로 도는 우회로 갔다(문장 수 = 출력 토큰 = 보고서 시간). 함께 수리 셋: ①join 의 items 경로가 없는 키를 조용히 0행으로 흘리던 자리를 표 경로처럼 정직 거절로(⑧′) ②별칭 `or` 사슬이 `by: []` 를 삼켜 형제 동사와 다른 진단을 내던 자리 ③`[table:join]` 의 `target_key: on` 이 YAML 1.1 불리언으로 접혀 빌드가 `"true"` 유령 param 을 싣고 정작 `on` 은 주 param 으로 선언된 적이 없던 자리(뷰 정의엔 있던 관문을 어휘 선언에도 — 빌드 `--check` 가 부류로 잡는다). 가드 `backend/test_language_revision_composite_keys_2026_09_07.py`. 개정 로드맵·집행 기록 = `docs/IBL_PROGRAM_GRADE_DESIGN.md`.
...
- **문법 마찰의 갈림길** (2026-08-29, 사용자 승인): 모델이 반복해서 틀리는 문법 자리(foot-gun)가 발견되면 "힌트로 덮을까 / 문법에서 뺄까"를 **명시적 갈림길로 세워 판정**한다 — 기본값-힌트 금지(무의식적으로 안내문만 붙이고 넘어가는 것이 그간의 관성이었다: `$이름` 경계 힌트 등). 제거가 항상 답은 아니다 — IBL 은 사람이 읽는 언어라 문법을 빼는 대가가 기계 전용 언어와 다르다. 요구하는 것은 결론이 아니라 **갈림길을 세웠다는 기록**(판정 사유가 changelog 에 남을 것).
...
- **세계상 저장의 원리**: 세 기억이 이미 독립적으로 공유하는 규율을 따른다 — **시스템이 판정하지 않고 증거를 노출한다**(포식기억: `conf`·`freshness`·`provisional`·surface / 심층메모리: 타임스탬프 동반 회상 / 해마: success_rate 표시). 명사·관계가 새로 필요해지면 빈도가 증명했을 때(결정화 사다리 — 용례→관용구→워크플로→가이드; 관용구 = 독립 문장 2~8개의 `;` 골격 + `${슬롯}`, 문법 개정 없이 기억 단위만 더한 층, 정본 `docs/IBL_IDIOM_TIER_HANDOFF.md`) 데이터로만 추가하고, 반드시 반증 가능하게 — 신뢰도·부패 노출·폐기 사유를 달아서.
...
<!-- IBL_STATS:START -->
총 **164 액션** — sense 43 · self 50 · limbs 14 · others 17 · engines 18 · table 22
<!-- IBL_STATS:END -->
...
**날짜는 ISO 8601 표기만이다**(2026-08-27, 사용자 판정 — 시간 의미론 밭 폐쇄): 값 코어에 `datetime` 종류가 생겼고, **선언 표기(`YYYY-MM-DD`[+시각][+`Z`/±HH:MM])만 날짜**다. 표기 밖의 문자열과 달력 위반(2026-02-30 등)은 수선하지 않고 텍스트로 남는다(숫자 문법 엄격화와 동형). 같은 순간은 같은 실체로 접힌다 — `Z`↔`+00:00`, `+09:00` 10시↔`Z` 01시, 날짜만↔그날 00:00 — 그래서 동등·순서뿐 아니라 **관계 키(join/dedup/merge)와 그룹 키까지 순간으로 접힌다**. 지어내지 않는 자리도 명시된다: naive/aware 혼합, 날짜 vs 비표기 텍스트, 날짜 vs 숫자의 순서는 **판정 불능(정직 거절)**이다(시간대 앞에서 조용히 틀리던 문자열 사전식 비교를 폐기). 정렬 버킷은 숫자→날짜→문자열→결측(날짜 안에서는 naive 하위버킷이 먼저)이고, `type:"text"` 강제 정렬은 날짜 해석도 끈다. ★이 개정의 표면 스윕은 **0**이었다 — 앞선 밭 폐쇄가 판정을 코어 한 벌로 접어 둔 덕에 종류 하나를 더하니 filter·블록·compute·정렬·키·응답 변환이 자동 승계했다. 가드 `test_datetime_semantics` 30건.

**`[table:filter]` 부정 멤버십**(2026-08-28): where 조건 언어에 `not_in`(목록 비포함)·`not_contains`(부분문자열 비포함)를 더해 `api_transforms` 표면과 **동형**이 됐다 — 원장 제외(안티조인: 이미 다룬 id 목록을 걸러내기)를 문장 안 술어로 쓸 수 있다. 값 의미론은 `common/value_semantics` 한 벌(`list_membership`·`negative_text_match`)을 빌리고, **좌변이 결측이면 부정도 주장하지 않는다**(위 결측 비교 규약과 같은 태도). 두 낱말은 워드 연산자 표에 등재돼 `"필드 not_in 값"` 문자열이 전-필드 substring 검색으로 침묵 강등되지 않는다. 가드 `backend/test_where_negation_ops.py` N1~N8(동형성 시험 포함).
...
- **이항**(`&` 두 입력): `join{on}` · `union` · `merge` — **형태 보존**(2026-09-06 언어 개정): items 가지끼리는 items 로 잇고, 표형은 명시 표형 가지가 하나라도 있을 때만(그때 빈 items 가지는 0행 표로 승격). 옛 `union` 은 늘 표를 내 2차 `union` 의 0행 가지에서 죽었다(ep2882) — **효과 봉투 1행 규약**(2026-09-05 언어 개정, 사용자 판정 "남긴 것도 고쳐"): `union` 은 items/table 없는 success 봉투(write·notify 같은 부수효과 결과)를 분기당 1행으로 받아 `effect_rows` 로 신고한다 — `[self:write] & [self:write] >> [table:union]` 이 "통화 종류가 같아야" 로 죽던 자리(ep2827). 부수효과만 병렬로 돌릴 땐 맨몸 `[A] & [B]` 로 문장을 끝내도 된다(결과=분기 봉투 목록; 이항 변환자 강제는 *통화를 소비할 때*의 규칙이다) — **죽은 분기 규약**(2026-08-30 언어 개정, 사용자 판정): 병렬 분기가 실패해 통화 없는 에러 봉투를 내면 union/merge 는 기본으로 그 분기를 **건너뛰고 신고**한다(`branches_skipped` + warning — items:[] 실은 실패 봉투를 0행+경고로 흘리던 B24-1c 의미론과 한 벌로 통일; 옛날엔 items 없는 실패 봉투가 "통화 불일치" **오진**으로 전체를 죽였다, ep2355 실측). `on_error: "stop"` 이면 전부-아니면-실패. 전 분기 실패=정직 에러. 산 분기끼리의 진짜 통화 혼합은 여전히 에러이며 분기별 통화를 이름 대 준다.
...
param 값이 **통짜 `.path` 참조 하나**(`items: "$순회.queue"`, `columns: "$표.columns"`,
`n: "$x.count"`)면 JSON *문자열*이 아니라 **원형**(list/dict/스칼라 — 숫자는 숫자로)이
들어간다. 옛 의미론(항상 문자열)은 같은 병을 소비자마다 되읽기로 때우게 했다
(B19-2 items → P30 파이프 원형 → B52 blocks — 셋 다 "변수 치환=JSON 문자열" 병).
경계 둘은 불변: **bare `$변수`**(경로 없음)는 v4 추출 계약(F17-3, 산문 정본 우선)
그대로, **문장 속 참조**("문서 $n건")는 글자 자리이므로 문자열화(+`_list_in_text`
신고, G31-1) 그대로.

### 변수 파이프 머리 — `$변수 >> [액션]` (언어 개정 2026-08-27, 사용자 판정)

할당된 변수를 파이프의 첫 세그먼트(통화 방출)로 쓸 수 있다 — `$평가 >> [table:document]{…}`.
`.path` 머리(`$순회.queue >> [table:sort]{…}`)는 그 경로의 원형을 방출한다(위 치환
개정과 같은 눈). 미할당 변수는 파싱 시점 정직 에러, 할당 문장이 실행되지 않은 경로
(안 탄 분기)는 실행 시점 정직 에러(V49-1 규약 — 빈 값으로 접지 않는다). `$items` 는
집합 바인딩 예약어라 이 문법의 대상이 아니다. 변환자들의 `items` param 도 함께
공개되어(단항 변환자 전부, array) 리터럴·`$변수` 원형 어느 쪽으로도 씨앗이 된다.

### 변수 병렬 분기 — `$변수 & $변수` (언어 개정 2026-09-01, 사용자 판정)

같은 변수를 **병렬 분기 자리**에도 놓을 수 있다 — `$apt재고 & $house재고 >> [table:join]{on: "구"}`.
이미 손에 있는 두 결과를 합치는 가장 자연스러운 문장이 그동안 "병렬 요소 파싱 실패"로 죽었다
(09-01 부동산 보고서 실측 — 그날 세 주행의 유일한 진짜 문법 오류였다). 변수를 파이프 머리로
놓는 길은 2026-08-27에 이미 열려 있었으니, 결함은 능력이 아니라 **자리의 비대칭**이었다.

- 액션과 섞어도 된다 — `$a & [sense:realty]{…} >> [table:union]`.
- 규약은 머리 자리와 **한 벌**이다(표기·미할당 판정·`.path` 원형 방출·`${x?}` 옵셔널·
  `$items` 예약어 제외). 한 곳(`ibl_parser._var_emit_step`)이 두 자리를 다 읽으므로
  "머리에선 되는데 분기에선 안 되는" 방언이 생길 수 없다.
- 안 탄 분기의 변수(미기록)는 그 **분기만** 정직 실패하고 봉투가 `branches_failed` 로
  신고한다(살아남은 분기는 계속 흐른다 — 병렬의 기존 부분 실패 규약 그대로).
- **폴백(`??`) 자리는 열지 않았다.** 뜻이 없어서가 아니다 — 변수는 실패도 담으므로
  `$a ?? $b`("실패면 $b")는 잘 정의되고 구현도 서너 줄이다. 열지 않은 이유는 **그 자리에서
  `??` 의 정의적 성질이 사라지기** 때문이다: 이 연산자의 값은 "앞이 되면 뒤를 실행하지
  않는 것"인데, 이미 값이 된 변수끼리는 둘 다 치른 뒤라 아낄 것이 없다(폴백이 아니라
  선택이다). 그러면 같은 기호가 원소 종류에 따라 게으른 폴백과 열심한 선택 둘로 갈리고,
  `$a ?? [sense:x]{}` 같은 혼합 사슬은 **원소마다 종류를 알아야 무엇이 실행됐는지 안다** —
  이 언어가 `&`·`??` 혼용을 이미 금지한 것과 같은 부류의 모호함이다. 그리고 필요는 더 나은
  자리에서 이미 채워진다: `$a = [액션] ?? [액션]`(생산 시점 폴백 — 게으름이 살아 있다)·
  `[try]/[catch]`·`[if: empty($items)]`·`${a?}`. 결정적으로 `$a ?? $b` 는 **구조적으로 항상
  비싼 형태**(둘 다 치르고 하나를 버린다)라, 일급 문법으로 만들면 싼 형태와 똑같이 쓰기
  쉬워진다 — 선택압(avg_ms·avg_tokens)을 심어 둔 언어가 낭비를 축복할 이유가 없다.
  그 자리의 거절문은 **자리 목록이 아니라 규칙**을 말한다("변수는 통화를 내는 자리에")
  — 목록은 새 자리가 생기는 날 조용히 거짓이 되므로.
