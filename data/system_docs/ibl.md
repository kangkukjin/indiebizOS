...
<!-- RUNS_ON:START -->
- 현 분포: `anywhere` 116 · `pc_only` 36 · `phone_only` 1. (빌드 파생 — 손 수정 금지)
<!-- RUNS_ON:END -->
...
<!-- IBL_STATS:START -->
총 **153 액션** — sense 41 · self 50 · limbs 14 · others 17 · engines 9 · table 22
<!-- IBL_STATS:END -->
...
- **이항**(`&` 두 입력): `join{on}` · `union` · `merge` — **효과 봉투 1행 규약**(2026-09-05 언어 개정, 사용자 판정 "남긴 것도 고쳐"): `union` 은 items/table 없는 success 봉투(write·notify 같은 부수효과 결과)를 분기당 1행으로 받아 `effect_rows` 로 신고한다 — `[self:write] & [self:write] >> [table:union]` 이 "통화 종류가 같아야" 로 죽던 자리(ep2827). 부수효과만 병렬로 돌릴 땐 맨몸 `[A] & [B]` 로 문장을 끝내도 된다(결과=분기 봉투 목록; 이항 변환자 강제는 *통화를 소비할 때*의 규칙이다) — **죽은 분기 규약**(2026-08-30 언어 개정, 사용자 판정): 병렬 분기가 실패해 통화 없는 에러 봉투를 내면 union/merge 는 기본으로 그 분기를 **건너뛰고 신고**한다(`branches_skipped` + warning — items:[] 실은 실패 봉투를 0행+경고로 흘리던 B24-1c 의미론과 한 벌로 통일; 옛날엔 items 없는 실패 봉투가 "통화 불일치" **오진**으로 전체를 죽였다, ep2355 실측). `on_error: "stop"` 이면 전부-아니면-실패. 전 분기 실패=정직 에러. 산 분기끼리의 진짜 통화 혼합은 여전히 에러이며 분기별 통화를 이름 대 준다.
