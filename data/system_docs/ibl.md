...
scope: IBL 명세, 5-Node 구조, 124 액션, 파서/엔진/라우팅
...
총 124개 액션(sense 41, self 41, limbs 17, others 11, engines 14)은 프롬프트 가독성을 위해 카테고리로 그룹화된다. 카테고리는 순수 표시 목적이며, 런타임 동작에 영향을 주지 않는다. 에이전트는 항상 구체적 액션명을 직접 사용해야 한다.
...
총 **124 액션** (… + 폰 온디맨드 감각 삼각: sense:here(위치)·sense:listen(마이크 받아쓰기/녹음)·sense:see(카메라 촬영), phone_only — 이후 self:show_calendar 폐지로 125→124).
...
IBL은 Phase 0(원시 도구 호출)에서 시작하여, 드라이버 기반 프로토콜 추상화(Phase 5-10), 노드 통합(Phase 17-25), verb 시스템 도입과 폐지(Phase 22-24), Goal/Time/Condition(Phase 26)을 거쳐 5-Node 체계로 발전했고, 이후 op 어휘화·사용성 재감사·어휘 정리·메신저/비즈니스 IBL화·neighbor 통합·폰 온디맨드 감각 삼각으로 현재 124 액션이 됐다. 핵심 설계 철학은 "AI가 작성하는 언어"이며, 문법 복잡도보다 표현력을 우선한다.
