...
scope: IBL 명세, 5-Node 구조, 144 액션, 파서/엔진/라우팅
...
총 144개 액션(sense 39, self 42, limbs 40, others 6, engines 17)은 프롬프트 가독성을 위해 카테고리로 그룹화된다. 카테고리는 순수 표시 목적이며, 런타임 동작에 영향을 주지 않는다. 에이전트는 항상 구체적 액션명을 직접 사용해야 한다.
...
총 **144 액션** (op 어휘화 + 사용성 재감사 후).

| 노드 | 액션 수 | 설명 | 주요 액션 |
|--------|---------|------|----------|
| `self` | 42 | 개인 도메인: 시스템 관리, 파일, 트리거/스케줄, 설정, 사용자 소통, 워크플로우 | file, storage, folder_note, fs_query, trigger, workflow, notify_user, run_command |
| `limbs` | 40 | 장치 제어: UI 조작(브라우저, 안드로이드, 데스크톱) + 미디어 재생 | browser, click, navigate, iframe, music, radio, stream, download |
| `sense` | 39 | 감각 확장: 외부 정보 수집 + 내부 데이터 관리 | search_naver, search_news, stock, company, travel, world_bank, crawl, realty, weather |
| `others` | 6 | 협업 통신: 에이전트 위임 + 메시지 송수신 | delegate, ask, channel_send, channel_read, agents |
| `engines` | 17 | 창작: 콘텐츠 생성 (슬라이드, 영상, 차트, 이미지, 음악, 웹사이트, 설계) | slide_shadcn, lecture, html_video, chart, image_gemini, web, newspaper |
...
| `cctv_refresh` | UTIC API 최신 데이터 갱신 | `[self:cctv_refresh]` |
| `cctv_stats` | CCTV 데이터 소스 통계 | `[self:cctv_stats]` |
...
IBL은 Phase 0(원시 도구 호출)에서 시작하여, 드라이버 기반 프로토콜 추상화(Phase 5-10), 노드 통합(Phase 17-25), verb 시스템 도입과 폐지(Phase 22-24), Goal/Time/Condition(Phase 26)을 거쳐 5-Node 체계로 발전했고, 이후 op 어휘화·사용성 재감사로 현재 144 액션까지 압축됐다. 핵심 설계 철학은 "AI가 작성하는 언어"이며, 문법 복잡도보다 표현력을 우선한다.
