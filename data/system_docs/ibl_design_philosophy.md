...
IBL의 진짜 엔진은 144개 액션이 아니라 `>>`(순차) `&`(병렬) `??`(폴백) + 접근 + 가이드다.
**199 × 조합 × 외부 어휘 = 사실상 무한.** 더 많은 단어 ≠ 더 강한 언어.
...
하지만 `[sense:apt_trade]`가 캡슐화하는 건 **국토부 API 키**, CCTV 액션은 **물리적 카메라**다.
...
- **증거: 332 → 199 → 144.** op 어휘화 + 사용성 재감사로 *더 적은 단어 + 파라미터 분기*가 *더 많은 행동*을 표현했다. 언어가 작아지며 더 강해졌다. ([architecture_ibl_op_vocabulary], [architecture_ibl_single_action_pattern])

## 9. 왜 이게 indiebizOS에게만 가능한가 (해자)

강력한 *최소* 어휘는 *하나의 작업 분포*에 최적화돼야 한다. 플랫폼 벤더는 못 한다:
- **일반성**이 큐레이션(narrowing)을 금지 — 누구의 199개?
