...
IBL의 진짜 엔진은 199개 액션이 아니라 `>>`(순차) `&`(병렬) `??`(폴백) + 접근 + 가이드다.
...
- **증거: 332 → 199.** op 어휘화로 *더 적은 단어 + 파라미터 분기*가 *더 많은 행동*을 표현했다. 언어가 작아지며 더 강해졌다. ([architecture_ibl_op_vocabulary], [architecture_ibl_single_action_pattern])
