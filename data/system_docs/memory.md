...
*마지막 업데이트: 2026-08-17(2차) — **가이드가 7종 기억의 시민이 되다**: 가이드(`data/guides/`)는 "이 일을 제대로 하려면 이렇게 해야 한다"는 **절차 기억**인데, 7종 중 **증류(쓰기)도 정리 패스도 없는 유일한 기억**이었다 — 해마는 실행에서·심층메모리는 대화에서·포식기억은 포식에서 증류되고 액션 desc 는 주간 의미 감사를 받는데, 그 desc 의 10~30배 길이로 같은 액션을 설명하는 가이드는 순찰이 0이었다. 그 공백이 2026-08-17 에 79→67개·81KB 수동 정리로 청구됐다. 네 겹으로 봉합: ①**신선도 표식**(`datastore/guide_registry.py`) — 주입 시 `작성 · 최종수정(N일 전) · 이후 무수정 사용 M회` 를 머리에 얹는다. 날짜는 **git 파생**(수동 날짜 필드는 그 자체가 낡는 기억이라 문제를 한 층 위에 복제한다), 67개 파일은 안 건드린다. ★나이만으로는 못 정한다 — 67일 된 가이드가 그동안 12번 무수정으로 쓰였으면 오히려 검증된 쪽이고 0번이면 방치다. ★함정: git 은 *커밋된* 것만 알아 작업 트리 수정이 "몇 주 전"으로 보고된다(실측) → mtime 이 더 최신이면 그쪽 채택. ②**빌드 가드**(`--check`): 유령 등재·끊긴 코드 경로=하드 실패 / 죽은 어휘·고아=비차단 경고(묘비로 남길지는 기계가 못 정한다 — remotion.md 판정이 실제로 뒤집혔다). ③**주간 의미 순찰**(`cognition/guide_audit.py`, 유지보수 번들 6.5번): 신선도 낮은 순 회차당 6개. ★실측 교훈 — **경량 티어로는 안 된다**(최소 형태 질문에도 `[]` 반환) → 실행 축 역할 `guide_audit` 등록 후 전제 역전을 정확히 검출. ④**되먹임 = 증류 4단계**(`cognition/guide_feedback.py`): **쓴 놈이 고친다.** 가장 좋은 감사자는 방금 그 가이드를 쓴 에이전트다 — 순찰은 차가운 산문을 읽고 추측하지만, `table:chart_line` 이 없어 실패한 에이전트는 *안다*. 세 갈래로 갈라 위험을 나눈다: 실측 관찰=자동 덧붙임 / **사실 오류=기계 검증 후 자동 수정**(모델은 무엇을 고칠지 제안하고 *옳은지는 코드가 확인* — old 에 죽은 어휘·끊긴 경로가 실제로 있고 new 가 실재하는 어휘·경로일 때만. notebook 의 '인용 결정론 후검증'과 같은 규율) / 방침·전제 변경=제안 큐(`guide_proposals.jsonl`). ★이것이 ①의 "무수정 사용"을 비로소 **진짜 신호**로 만든다 — 지금까지는 "아무도 안 봤다"일 수 있었는데, 검토가 강제되면 "봤고 고칠 게 없었다"가 된다. ★churn 감시(`guide_edit` 테이블): 매 턴 "고칠 것 있나?"를 물으면 모델은 뭐라도 고치려는 편향을 보인다 — 가이드가 매주 흔들리면 학습이 아니라 churn 이므로 기준을 조인다. **★공통 규율: 계수는 판결이 아니다** — "무수정 사용"은 "안 고쳐졌다"이지 "맞았다"가 아니고(표식 문구에 명시), 판정 불가는 `unchecked` 로 남긴다(깃발 0 이 '깨끗함'인지 '못 봤음'인지 구별 못 하면 조용할수록 안심되는 게 아니라 눈이 먼 것이다). 이전(같은 날 1차) — 수치 정합화 + **채택 판단의 교훈 1건**: 6노드 **144 액션**·해마 코퍼스 **3,310건**·증류 **800건**. 라이브 모델은 2026-08-17 재학습 세대(epoch 4·검증 0.878 — 어휘 압축 163→144 와 상상훈련 6~12회차 시드 51건 흡수). ★**자동 비교표가 "보류"를 권고했으나 뒤집어 채택**했다: 회귀로 보인 항목이 desc-공간 인공물이었고(코퍼스 인출 1.000) 정작 조합 문장 인출은 새 모델이 우세, 라이브 일반화(새 상품명·임계값·요일 변주)도 정확했다 — **비교표가 아니라 라이브 인출이 판단 근거**. 함께: 시딩 함정 2건 명시(백그라운드 로딩 중 벡터 미부착 / `.venv` 파이썬 필수), "어휘를 지우면 코퍼스도 따라온다" 절에 2026-08-15 은퇴 3건의 이관 실적 추가. 이전(2026-08-06) — 수치 정합화: 6노드 150 액션·코퍼스 3,032건·증류 819건. 본문 불변. 이전(2026-08-04) — **해마 성능표를 세대-비교로 교체**: 옛 표는 baseline(범용 ko-sroberta) 대비라 채택 판단에 못 쓴다 → **직전 모델 vs 새 모델 동일 seed42 분할** 표로 교체(code T5 88.9→**92.2%**, desc T5 92.2→**94.2%**, 6지표 전부 우세·회귀 0). 코퍼스 2,925→**2,988**. ★**분할이 다르면 비교가 안 된다**는 함정 명시(07-21 자체측정 92.5% 와 위 88.9% 는 모순이 아니라 분할이 다른 것 — 935쌍/393패턴 → 1,003쌍/399패턴). 재학습 절 앞에 함정 2건(라이브 모델 폴더 직접 덮어쓰기 → 백업 필수 / 재색인 없이 재시작하면 조용히 망가짐) + 절차 정본 `data/guides/hippocampus_retraining.md` 링크. 이전 — 7종 메모리 지도 불변. 현 상태 정합화: 절차기억(#4a) 어휘 = **6노드 163 액션**(sense 48·self 52·limbs 18·others 18·engines 14·table 13 — 웹앱 등기부 `[self:webapp]` 추가), 해마 코퍼스 **2,925건**·증류 누적 808건. ①**증류 꼬리 6분→4초**(2026-08-01, 위 절): 원샷 호출의 하이브리드 thinking 차단 + 0자 응답 가드 + `_after_response` 백그라운드화 — 증류는 응답 뒤 꼬리이므로 사용자가 기다리는 시간이 되어선 안 된다. ②**포식기억 owner 빈도 게이트**(2026-07-29, #7 절): 주인모델은 상시 노출이라 1회 추론이 영구 주입되던 구멍 → 첫 관측은 임시(query 필터), 서로 다른 포식에서 재확인되면 상시 냄새로 결정화. '빈도가 결정화한다' 모티프의 기억 쪽 사례이고, 잃는 정보는 0이다. ③**어휘 축소의 대칭**: 음악앱 5기능 은퇴 때 `--check` 코퍼스 param 정합 가드가 죽은 파라미터를 잡아 해마 용례 15건 삭제를 강제 — 어휘를 지우면 코퍼스도 따라온다. 이전(2026-07-25) — 7종 메모리 지도 불변. 당시 정합화: 절차기억(#4a) 어휘 = 6노드 162 액션(sense 48·self 51·limbs 18·others 18·engines 14·table 13 — 몸 부탁 `[others:ask]`·USB 손발·신문 발행·내 음악 추가), 해마 코퍼스 **2,916건**. **해마 소유-필터**(2026-07-22): 회상 4경로 + 입구에 게이트를 둬 *자기 어휘의 용례만* 배우고 떠올린다(미지 어휘=남의 것으로 판정) — 사전 물리 분리의 기억 쪽 짝. 시딩 경로도 `add_examples_batch` 하나로 수렴(정적 시드 스크립트 부류 폐기). **손발 프레즌스 상시 주입**: 살아있는 게스트 PC 손발의 이름·의미를 0단계 연상에 노출(런타임 상태라 어휘·해마가 원리적으로 모르는 것). 이전(2026-07-17) — 7종 메모리 지도 불변. 현 상태 정합화: 절차기억(#4a) 어휘 = **6노드 157 액션**(engines 변환자/emitter를 table 노드로 분리, others에 공개 표면 가족 추가). 이전(2026-06-27) — 앱 표면 품질 일괄 개선(라디오 즐겨찾기·CCTV 인앱 재생 stream 버튼·여행 날짜+한국 지방공항·투자 TIGER200·날씨 오송·문화 지역·길찾기 거리/예상시간) + 부동산 직방 호가(sense:realty source:zigbang)·AI 공모/창업(sense:contest/startup) + read_guide claude_code 노출 + 폰 네이티브 재빌드. 142 액션(sense 44·self 44·limbs 17·others 11·engines 26)·38 도구 패키지. 이전(2026-06-22) — 포식 기억(forager)을 7번째 메모리로 정합화(제목·"한눈에" 표·흐름도 6종→7종, 다중 몸·`[self:residual]` 음성-단언 측정 반영). 절차기억(#4a) 액션수 111→141. 국회도서관 국가학술정보 인물/학위논문 액션 추가로 코퍼스 갱신.*
...
### 현재 라이브 모델 (2026-08-17 재학습 — 144 액션 어휘, 코퍼스 **3,310**, epoch 4·검증 0.878)

2026-08-05~17 어휘 압축(163→144)과 상상훈련 6~12회차 시드 51건을 흡수한 세대. 백업=`data/models/ibl_embedding.bak.20260817_090211`.
...
- **어휘 정합**: 코퍼스(usage_db)는 항상 최신 어휘로 마이그·재색인 유지(**3,310건**, 증류 누적 `ibl_distilled.json` **800건**. 현 6노드 **144 액션** 어휘). ★**어휘를 지우면 코퍼스도 따라온다** — 음악앱 5기능 은퇴 때 `--check` 의 코퍼스 param 정합 가드가 죽은 파라미터를 잡아 용례 15건 삭제를 강제했고, 2026-08-15 지역정보·연락처·사업원장 은퇴에서도 코퍼스 이관(41행·27행 등)이 은퇴의 일부였다. 은퇴어의 용례는 **후계어로 재배선**하되, 후계가 없으면 삭제한다(합성 코퍼스 15행 사례).
...
*마지막 업데이트: 2026-08-17 — 라이브 모델 = 2026-08-17 세대(코퍼스 **3,310**, 어휘 **6노드 144 액션**, epoch 4·검증 0.878). 2026-08-05~17 어휘 압축(163→144)과 상상훈련 6~12회차 시드 51건 흡수 — **재학습 대기열 소진**. 직전 세대 2026-08-16(epoch 6·0.877, 블록·`table:rename`/`flatten` 흡수). 이전(2026-07-25) — **소유-필터 + 재학습 대기열 소진**: 2026-07-21 로컬 재학습(코퍼스 2,871, code Top-5 +4.9p) 라이브 — 07-17까지 쌓인 재학습 대기열 소진. 이후 창고·음악·손발 용례가 다시 누적돼 현 코퍼스 **2,916건**(어휘 **6노드 162 액션**). **소유-필터 근본 수정**(2026-07-22): 미지 어휘를 '남의 것'으로 단일 판정하고 회상 4경로·입구에 게이트를 달아 남의 몸 어휘가 코퍼스·회상에 새지 않게 함(남의-어휘 용례 정리 포함). 시딩은 `add_examples_batch` 단일 경로(rebuild_usage_db.py·generate_missing_intents.py 폐기 — 정적 시드가 라이브 코퍼스를 덮어쓰던 부류 소멸). 이전(2026-07-17) — **로컬 재학습(맥미니 M4 Pro) + 현 어휘 정합화**: 2026-07-13 로컬 재학습이 백업을 이겨 라이브(코퍼스 ~2,752, rename≠addition). 어휘는 이제 **6노드 157 액션**(engines 변환자/emitter→table 노드 분리, others 공개 표면 가족). 신규 어휘(portal/showcase/family_news/bulletin·stay·entity·used·install_lib·icon)는 capability 게이트로 보강 중이며 다음 임베더 재학습 때 흡수. 이전(2026-06-30) — **해마 재학습 + 코퍼스 정돈**: "시장 보고" 류 연상 미스(query→`[self:schedule]` 0.635, 의도-클래스 불일치) 진단 = 모델/내용이 아니라 **색인 적체**(좋은 예가 학습 json엔 있고 라이브 색인엔 0건). 정돈=올바른 패턴 앵커 5건 curated 추가(지수=stock quote·시장뉴스=search_gnews, news="market" 버그 교훈 박음) + consolidation → 코퍼스 2,624. 로컬 M4 Pro 재학습(batch=8·10ep, code Top-5 88.9%/desc 91.2%; held-out 직전 92.6%보다 소폭↓=커진 코퍼스 노이즈, 실연상 무손상) → rebuild_index. **검증: 그 쿼리 0.635→1.0**(올바른 시장-보고 패턴 연상). ★백엔드 모델 리로드(touch/재시작) 후 라이브. 이전(2026-06-27) — 앱 표면 품질 일괄 개선(라디오 즐겨찾기·CCTV 인앱 재생 stream 버튼·여행 날짜+한국 지방공항·투자 TIGER200·날씨 오송·문화 지역·길찾기 거리/예상시간) + 부동산 직방 호가(sense:realty source:zigbang)·AI 공모/창업(sense:contest/startup) + read_guide claude_code 노출 + 폰 네이티브 재빌드. 142 액션(sense 44·self 44·limbs 17·others 11·engines 26)·38 도구 패키지. 이전(2026-06-22) — 현 5노드 **142 액션**(sense 44·self 44·limbs 17·others 11·engines 26), 해마 코퍼스 ~2,600(rebuild_index 2605 — 국회도서관 국가학술정보 인물/학위논문 액션 추가 등 용례 누적). 코퍼스는 06-12 122-액션 어휘 기준 재학습 이후 미재학습 — 신규 액션은 capability 게이트로 보강(해마는 query↔intent 의미 매칭이라 vocab 변경에 강건). 절차기억(해마)은 7종 메모리의 #4b — 통합 지도는 이 문서 상단. 이전(2026-06-15) — 통화 대수(engines 변환자 9: filter/sort/take/select/dedup/groupby/join/union/merge + 파이프 문법 `|` + 문서 IR emitter) → 122~124에서 136 액션. 이전(2026-06-14) — 5노드 136 액션(해마 코퍼스는 06-12 122-액션 어휘 기준, 이후 미재학습 — 신규 액션은 capability 게이트로 보강). **폰 자아는 해마 비활성**(시스템 AI 응답 속도 위해, 사용자 결정) — 폰은 키워드 검색으로 충분, 해마는 맥-자아 전용. 해마 증류/심층기억 시맨틱 검색의 폰 이식은 불필요로 결정(2026-06-14). 이전(2026-06-12): 해마 **로컬 Mac M4 Pro 재학습**(batch=8, code Top-5 92.6%/desc 92.8%/런타임 ~99%, 코퍼스 ~2,424). 클라우드는 옛 맥에어 OOM 한정 — 이제 재학습=로컬 5~10분. neighbor 통합 relabel + phone_sync·neighbor save/favorite 용례 보강. 이전: 2026-06-05 Modal GPU 재학습(android 어휘), 2026-06-04 144 액션 재학습(런타임 천장 판명), 2026-05-17 단일 검색 일원화·심층메모리 시맨틱 검색·점수 정규화*
