---
title: 메모리 아키텍처 — 8종 기억 통합 지도
scope: indiebizOS의 모든 기억 하위 시스템(의미·작업·일화·절차(실행기억)·관계(심층기억)·자기상태·공간(포식)·세계)의 저장/사용/학습 흐름
owner_code: >
...
# 메모리 아키텍처 — 8종 기억 통합 지도

> indiebizOS의 메모리는 단일 시스템이 아니라 **여덟 개의 독립 하위 시스템**으로 구성된다.
> 앞의 일곱은 인간 기억 분류와 거의 정확히 대응하고, 여덟째(세계 기억)는 모델이 이미 가진 지식으로 가는 다리다(2026-09-17 독립 종으로 승격 — 그전에는 #1 의 하위 절). 이 문서는 그 전체 지도다.
> (해마 단일 시스템의 상세는 아래 **부록: 연상기억 심층** 참조 — 본 문서는 그것을 8종 중 하나로 위치시킨다.)
...
| 4 | **절차 기억** (방법) = 통칭 **실행기억** | IBL 액션 + 워크플로우 + **해마** | `ibl_nodes.yaml`, `data/workflows/*.yaml`, `ibl_usage.db` | "어떻게 하는가" — 자연어→IBL 코드 |
...
## 통합 증류 — 저장 가치가 있는 것만 (2026-09-21)

신규 턴의 실행·심층·포식 기억은 해당 턴 실행 모델의 단일 원샷에서 함께 선별한다.
**0건 저장은 정상 성공**이다. 구체적인 향후 쓰임과 기존 기억 대비 차이가 불분명하면
저장하거나 재심사 큐에 쌓지 않는다. 출처가 정확하다는 것만으로 저장 가치가 생기지 않는다.
사용자 원문·실제 실행·실제 공간 관측을 구분하고 기존 기억을 호출 전에 비교한다.
종류별 검증·저장은 분리하며 저장 재시도는 첫 판단과 mutation 영수증을 재사용한다.
원문·코드·파일 단서를 새로 만들어 기억하지 않는다. 상세와 한계:
[가치 우선 통합 증류](../../docs/UNIFIED_DISTILL_2026_09_21.md).
입력 예산에는 실행 근거를 먼저 넣고 분류 목차를 먼저 덜어낸다. 문장부호만 있는 단위는
사용자 사실 후보에서 제외한다. 해마의 사용 관측·top-1 되먹임은 액션 이름의 교집합 대신
동작 선택 인자와 호출 순서를 함께 대조한다. [3949~3951 수리](../../docs/EPISODES_3949_3951_REPAIRS_2026_09_21.md).
...
정정한다. 제목·전체 목표의 임의 합성과 기준을 버린 폴백은 하지 않는다.
계약과 재현 검증: [과제 원장 — 생성과 완료 기준](../../docs/PURSUIT_LEDGER_HANDOFF_2026_09_09.md#4-생성과-완료-기준).
...
### 모델 호출 절약에 관한 보존된 발견 (2026-09-11)

사용자 요청으로 ep3405·3406 감사와 후속 논의를 보존한다.
정본 근거·호출 계수·이미 수리한 부분·미검증 제안은
[모델 호출 감사](../../docs/EPISODE_3405_3406_MODEL_CALL_AUDIT_2026_09_11.md)에 있다.
재탐색·중간 결과 재열람·늦은 감독 지시·도구 내부 AI 호출이 비용을 만들었다.
IBL 유무 대조가 없으므로 언어 도입 자체가 순비용 증가의 원인이라고 단정하지 않는다.
변수·원문 참조 보존은 이미 있으며, **보존된 값과 판단을 다음 실행이 재사용하게 연결하는 것**이
후속 설계의 초점이다. 원자료 보존·선택적 별칭·필요 결과의 묶음 반환·유효 조건이 붙은
판단/검증 재사용을 검토한다. 전역 변수 저장 의무나 새 문법으로 확정한 규칙은 아니다.
후속 평가는 IBL 횟수와 내부 모델 라운드를 구분하고, 원래 목표·품질·출력량·대기 시간을 함께 본다.
전문가 관점의 후속 선택은 작은 구간의 실행·반환 계약부터 검증하고, 현재 IBL/묶은 IBL/
같은 조건의 Python을 비교해 실행 단위의 효과와 언어 표현의 효과를 구분하는 것이다(실험 미실시).
...
- **주제 트리(topic, 2026-09-03 사용자 판정 "실행기억도 주제별 폴더로")**: 용례마다 주제 가지(`보고서/AI 동향` 꼴, 가이드와 맞물림 `guide:`). 가지마다 문서 `data/hippocampus_tree/<가지>/memory.md`(표식·`> 요약`·`guide:`·`## 용례` 기계 절)가 정본, DB 는 색인(`backend/datastore/hippo_tree.py`, 사람이 고친 줄은 구문 관문을 거쳐 색인 반영). 지도의 **프롬프트 자동 주입은 2026-09-17 폐지**(사용자 판정 — 용례는 해마가 자동 연상하고 문법은 프롬프트에 있다. 실측: 지도 3,930자·추정 3,200토큰에 가지 열기는 375턴 중 21턴). 매 턴 실리는 것은 가이드 목차 `<guide_map>`(가지: 파일명, `hippo_tree.guide_map_text`)뿐이고, 지도 자체는 내부에서 계속 쓴다(증류기의 가지 배정·생명주기·recall 의 node 생략). 가지는 `[self:memory]{op:"recall", node, store:"실행"}` 로 연다. 증류기가 지도를 보고 `topic` 을 적는다(코드 분류기 없음). **주행 절**(2026-09-04 사용자 판정): 가지 문서의 `## 주행` 에 그 주행에서 성공한 문장들을 실행 순서대로 남긴다(가지당 최근 20건·주행당 30문장, 넘치면 절단 표기) — 대표 문장이 없어 '재사용 패턴 없음'이던 프로그램급 주행이 학습 0건이 되던 자리, 지도에 `주행 n` 으로 보인다. 얼린 워크플로가 아니라 용례다. **관용구 절**(2026-09-04 저녁, 정본 `docs/IBL_IDIOM_TIER_HANDOFF.md`): 낱말(용례 한 문장)과 얼린 워크플로 사이의 층 — 독립 문장 2~8개를 `;` 로 이은 골격에 구체값은 `${슬롯}` 으로 비운 것. DB 엔 `category='phrase'`, 가지 문서 `## 관용구` 절이 정본(용례 절과 같은 규약 — 사람이 고친 블록은 구문 관문을 거쳐 색인, 지우면 삭제), 지도에 `관용구 k`. 증류기의 두 번째 질문 「이 주행에서 되풀이될 모양은?」이 뽑고(AI 몫, 코드 분류기 없음), 기계는 **순서 보존 부분열 접지**만 집행한다 — 각 문장은 슬롯을 이번 값으로 되돌렸을 때 **실행 문장**과 머리·인자 키가 같아야 하고 순서는 실행 순서의 부분열(★순서는 흐름이 아니다: `>>` 접지는 한 문장 안, `;` 접지는 부분열). ★**접지의 단위는 호출이 아니라 문장**(2026-09-07 수리, ep2943): 한 execute_ibl 호출은 대개 여러 독립 문장을 담은 프로그램인데 종전엔 호출을 통째로 한 서명으로 접어 비교해 `len(p_acts) != len(c_acts)` 에서 반드시 어긋났다 — **실행된 문장을 글자 그대로 넣어도 접지 실패**(결정론적 재현 231턴 중 58턴 25% 거부 → 0턴, 실사용 접지 실패 12건도 같은 뿌리이며 호출 경계의 거짓 매칭이 `pos` 를 먼저 소비해 '순서 어긋남'으로 둔갑한 것 포함). 양쪽을 `hippo_tree.split_sentences` 로 잘라 비교하며 흐름 접지는 오히려 엄격해진다(종전엔 한 호출 안 아무 데나면 통과). 액션 없는 문장은 산문이면 거절, 상수 바인딩(`$기준 = "…"`)은 이 주행이 같은 이름을 같은 방식으로 묶었을 때만 통과(슬롯이 앉는 자리, ep2882). 회귀 `backend/test_idiom_grounding_statement_unit_2026_09_07.py`. 개인 명사(홈 경로·목록)는 거절 — 슬롯으로 비우는 것이 곧 관문. 회상은 낱말 Top-5 옆 **관용구 Top-2**(`kind="phrase"` 번호 목록, 임계 동일·저신뢰 폴백 없음, 낱말 채널은 관용구 제외라 반사 top-1 은 낱말만), 귀속은 문장 머리 열의 부분열이 실행 궤적에 절반 이상 등장했을 때(회상된 관용구가 쓰인 턴은 새 관용구를 뽑지 않음). 재학습 게이트는 관용구 Top-5 를 분리 보고하고 회귀(−0.5p)면 보류. 주행 절(녹취록)은 관용구의 증거로 남는다. **세 갈래**(같은 날 밤, 사용자 판정): 뽑기(`scripts/replay_idioms.py` 되돌려 묻기 — 에피소드 원장에 같은 반성기·같은 관문, 되풀이 증명 골격만) · 짓기(`--rehearse` — 설계한 관용구를 라이브 `/ibl/execute` 로 한 번 돌려 성공한 것만 저장, 실패=언어 공백 신호) · 가르치기(`ibl_access.idioms_map` — 허용 노드 안의 상시 관용구를 지도 예산 안에서 가지별로 노출. 실행 환경과 의식 계획이 같은 지도를 사용하며, 본문은 이름으로 회상). 첫 밤 14건(설계 8·뽑기 6). 관용구 색인 = 의도×3+머리 열 패턴. 코드는 `backend/cognition/ibl_idiom.py`(rag 에서 분할, 이름은 rag 가 재수출). **서명에 반환**(2026-09-05, `docs/IBL_STATIC_TYPECHECK_HANDOFF.md`): 관용구·워크플로 저장 시 정적 검사기(`ibl_typecheck.return_type_of`)가 반환 모양을 산정해 `ibl_examples.returns` 에 적고, `<ibl_idioms>`·이름 먼저 회상·가지 문서 `호출:` 줄이 `[fn:이름]{슬롯} → items⟨열⟩/prose/effect/?` 로 보인다 — 부르기 전에 무엇이 나올지 알아야 뒤 문장을 쓴다. 일괄 산정은 `scripts/replay_idioms.py --type-idioms --apply`. ★매 턴 유사도 Top-5·반사 경로는 그대로 — 트리는 대체가 아니라 축 하나(정기 작업의 성공 문장 모음 = 결정화 사다리 용례→**관용구**→워크플로→가이드).
- **관용구의 현재 계약(2026-09-07)**: 위 09-04 기록의 자동 관용구 추출·상위 6건 노출은 은퇴했다. 상시 노출은 사람이 고른 `always_on=1`만이며, `data/idioms/curated.json`의 입력 계약·조합 용례를 등록 본문과 일치할 때 보여 준다. 관용구는 `[fn:]` 호출로 앞뒤 낱말과 연결한다. 수동 선정·등록 절차와 관문은 `ibl.md` §7.5, 설계·검증은 `docs/IBL_IDIOM_COMPOSITION_REPAIR.md`.
- **가지 회상의 통화(2026-09-08)**: `recall{node,store:"실행"}`의 `items`에는 해당 가지의 용례·함수와 조상 가지의 이름 있는 함수 카드가 함께 온다. 조상 무명 용례·주행 본문은 상속하지 않으며 같은 이름은 가까운 가지가 우선한다. 기본은 호출·입출력 서명·성적만 보고 본문은 `expand:"이름"`으로 연다. `count=len(items)`, `example_count`는 해당 가지 용례 수, `phrase_count`는 해당 가지 관용구 수, `inherited_function_count`는 상속 함수 수다. `success_rate`는 성공/(성공+실패)의 소수 둘째 자리 값이며 무시도는 -1이다. 기존 `phrases`도 유지한다. 자동 작명은 중단된 상태이며 이 회상 확대가 자동 등록을 뜻하지 않는다. 근거: `docs/IBL_EPISODE3176_REPAIRS_2026_09_08.md`.
- **검색**: 매 요청 1회, 현재 기본은 시맨틱 100%(`DEFAULT_ALPHA=1.0`) Top-5 → XML 주입. 임베딩 모델이 아직 준비되지 않았거나 사용할 수 없을 때 FTS5/BM25가 폴백한다
- **증류**: 기존 성공·중복 관문 → 통합 판단이 재사용 가치 있는 실제 실행 원문만 선택 → 독립 검증 → DB·`ibl_distilled.json` 반영. 성공했다는 이유만으로 저장하지 않으며 자동 관용구/작명은 중단 상태다.
...
**출처 경계(2026-09-12)**: 메시지 전달자와 글의 저자는 다르다. 명시 인용·코드 펜스·출처가 불분명한 긴 답변형 문서는 개인 사실 후보에서 보류한다. 나머지도 추출기가 사용자 자신의 지속 진술인지 확인한다. 포식의 주인 모델 역시 선택한 사용자 근거 원문을 저장하며 AI 평가를 성향으로 바꾸지 않는다. 표지 없는 붙여넣기의 완전한 저자 판별을 보장하지는 않는다. 정정된 타인 의견은 `external_record`이며 사용자 결정이 아니다.

**입구 관문 둘(2026-09-17 기억 재고 감사)**: ①**발화자 축** — 인지 파이프라인의 모든 진입점은 `utterance_author` 를 명시하고(`owner`/`agent`/`schedule`/`member`), `owner`(주인이 직접 친 말)일 때만 심층 증류가 돈다(fail-closed, `agent_pipeline` → `_after_response_async(write_deep=)` → 증류 큐 payload). 에이전트 위임문·보고 회수·예약 주입문(`calendar_actions` 가 WS 주입 dict 에 `schedule` 을 싣는다)은 기계가 짠 글이라 재료가 아니다 — 실측: 시스템 AI 위임문의 하네스 안내문이 프로젝트 에이전트의 `사용자선호` 로 저장됐다(건축 #7). 해마·포식 증류는 이 축과 무관하게 종전대로 돈다. ②**지시 대상 관문** — 원문 문장 단위 저장의 그림자는 떼어 놓으면 무엇에 관한 말인지 모르는 조각이다("그런데 나는 그 차이가…"). 추출 모델에게 가리키는 앞 단위를 `source_ids` 에 함께 고르게 하고, 기계는 셀 수 있는 것만 거절한다(`memory_evidence.unresolved_reference`: 첫 단위의 의존 머리 · 단독 단위 속 지시 관형사). 거절은 `memory.distill.rejected` 궤적 사건으로 남는다. 실저장 42건 재생: 16건 거절(전부 문맥 의존 조각). 진입점 전수 AST 관문 = `backend/test_deep_memory_entrance_2026_09_17.py`. 작업일지·`[보충]` 오병합은 09-12 개편(원문 단위·분리 저장·"작업 진행 내역 제외")이 이미 막았고 남은 것은 그 이전 재고다.
...
- **저장소**: 에이전트별 `memory.db` (memory 패키지, `memory_db.py`)
  ...
  - **주제 트리(node, 2026-09-03 사용자 판정 "심층기억을 평면으로 두지 말고 폴더 구조로 — 블로그와 같다")**: 행마다 주제 가지 경로(`가족/어머니` 꼴). 가지마다 문서 `<DB 옆>/memory_tree_<자아>/<가지>/memory.md` 가 **정본**(표식 + `> 한 줄 요약` + AI 산문 + `## 기억` 기계 절 + 갱신 기록), DB 는 색인 — 포식 기억 `forage_doc` 와 같은 배치. 사람이 절의 줄을 고치면 다음 회상 때 색인이 따라온다(`memory_tree.sync_node`, mtime 대조). **어디에 넣을지는 AI 가 정한다**: 증류기가 지도를 보고 `node` 를 적고(코드 분류기 없음), 남은 미배치 행은 `memory_tree.file_unfiled` 가 모델에게 배치시킨다. 어휘: `[self:memory]{op:"recall", node}`(가지 열기·생략=지도) · `save`(자동 선별 안내, 저장 없음) · `move{memory_id, node}` · `search{node}` 필터.
- **자동 저장** (`unified_distill` + `distill_memory_adapters`): 응답 뒤 사용자 직접 발화의 지속 가치를
  실행·공간 기억과 함께 한 번 판단한다. 앞 문맥을 포함해 관련 기억을 먼저 조회하며 조회는 사용으로
  세지 않는다. 원문 선택·NEW/SAME/UPDATE/REPLACE를 함께 결정하고 추가 관계 모델은 부르지 않는다.
  출력 최대 3건은 상한일 뿐 목표가 아니다. SAME은 새 사실을 만들지 않고 UPDATE는 새 원문을 별도
  연결하며 REPLACE는 명시적 정정과 버전 비교를 요구한다. 옛 큐의 독립 경로는 호환용으로 유지한다.
...
- **사용** (2026-09-17 개정 — 지도 + 선택된 기억, 정본 `docs/TREE_MEMORY_RECALL_COMMON_DESIGN_2026_09_17.md`): 매 요청 시 `<memory_map>`(지도 — 가지·건수·요약, 600자를 넘으면 최상위 가지만)과 `<recalled_memory>`(이 질문에 맞춰 고른 기억 3건)를 함께 주입한다. 고르는 법은 블로그 방식 — 가지 사전(가지 문서의 `> 요약`·`찾는 말:`·`함께 볼 가지:`, `memory_tree.entry_of`)으로 **가지 2개를 먼저** 고르고, 그 안에서 의미+글자 역순위 융합으로 2건, 가지 밖에서 1건(누락 보충). 건당 200자에서 자르고 `‹#id›` 를 남긴다. 코드 = `backend/datastore/tree_recall.py`(저장소를 모르는 공통 회상) + 패키지 `recall_store.DeepMemoryStore`(어댑터). 회상 색인은 `data/recall_index/`(git 밖, 인코더 `ko-sroberta`, 텍스트 해시가 바뀐 것만 재임베딩) — 중복 판정·모순 스캔이 쓰는 `memories_vec`(해마 공간)와 별개다. 인코더 적재·대량 색인은 백그라운드라 재기동 뒤 첫 턴은 글자 채널만 돈다. 가지 밖 항목이 가지 안보다 앞선 턴은 `data/recall_index/outside_hits.jsonl` 에 남는다(사전이 모르는 흩어짐 — 정리 패스의 재료). **왜 되돌렸나**: 09-03 에 Top-3 벡터 주입을 폐지하고 지도만 올렸더니 가지 선택은 정확했지만 **열지 않는 턴이 절반**이었다(속초 대화 25턴 중 12턴이 기억 없이 진행). 실측: AI 가 실제 연 가지 43건을 정답으로(그 에피소드 이전 기억만) 가지 @1 29 · @2 37(86%) — `scripts/evaluate_tree_recall.py`. 모자라면 AI 가 지도를 보고 `[self:memory]{op:"recall", node}` 로 연다.
...
- **저장소** (2026-09-03 개정, `backend/datastore/forage_doc.py` + `forage_memory.py`): **정본은 문서**다 — 위치마다 `data/forage_surveys/<몸>/<경로 그대로>/memory.md`(폴더 트리를 비춘 트리, 저장소 밖·개인정보) 의 `## 단언` 절. 절대 경로를 가진 단언은 몸 표기와 무관하게 `mac/<경로>/memory.md` 한 곳, URL 단언은 `host/path` 로 정규화해 `web/<host>/<path>/memory.md` 한 곳(같은 날 저녁 개정 — 몸 라벨이 `web:<url>` 이어도 무관). `code:`·`book:` 문서와 `web/memory.md` 뿌리 문서에는 경로·URL 없는 단언(주제 라벨)만 남는다. `data/forage_memory.db` 는 **2층** — `forage_map`(폴더 정체·관습·죽은가지·기질)은 그 절의 **색인**(DB 쓰기→문서 재렌더, 문서 편집→mtime 으로 색인 재동기화). (옛 `owner_model` — 몸독립 주인모델 — 은 2026-09-18 은퇴: 아래 입구 관문 조항)
- **닫힌 루프**: ③ AI 가 장소를 보고 `[self:forage]{op:"recall"}` 로 회상(도구 결과로 `<forage_memory>` 수령 — 옛 자동 주입 `_search_forage_memory` 는 2026-09-03 폐지, 아래 **진행** 참조) → ② AI가 포식(`file_find`/`grep`/`read`) → ④ 종료 훅에서 *일반화 가능한 지도 델타만* 통합 증류(`distill_memory_adapters`, 실제 관측에 접지·날 내용과 특정 결과 제외) → ⑤ 기존 라벨 위반 이질 내용은 surface 표식(필터버블 반대힘).
- **★입구 관문 — locus 는 주소다**(2026-09-18, `forage_memory.locus_is_address`): 포식 기억의 계약은 "장소를 주면 그 장소의 기억 **전부**"(문서 + 조상 상속 + 자식 골격)다 — 심층·세계의 기억처럼 가지 안에서 몇 건을 고르는 것이 아니다. 그래서 주소(절대 경로 · 웹 host/path · `book:`/`notebook:` 몸)가 아닌 locus 는 거절된다(`forage.note.rejected`). 주제 이름·상대 경로·`<몸>/unknown` 은 어느 장소를 열어도 나오지 않는 기억이 된다(재고 감사: 702건 중 239건). 장소가 열쇠가 아닌 지식은 다른 기억의 일 — 주인에 대한 사실=#5, 방법=#8·가이드. **주인모델(`owner_model`)은 은퇴**했다(같은 종류의 사실이 관문 없는 두 번째 저장소에 쌓였고, 09-03 자동 주입 폐지 뒤로는 읽는 경로도 없었다 — 테이블·읽기 경로·어휘 인자 `layer:"owner"`/`facet`/`value` 제거 완료). 포식 브라우저는 개인화 기억 없이 무상태 검색으로 둔다(같은 날 판정). 수리 판정은 그 파일의 절대 경로에, 장소 없는 일반 교훈은 가이드 제안 큐로. 정본 `docs/FORAGE_MEMORY_AUDIT_2026_09_18.md`.
- **★장소 찾기 — 고르는 일은 여기 하나뿐**(2026-09-18, `forage_recall_store.py` + `tree_recall`): 장소를 모를 때 `recall{query}` 가 의미+글자 융합으로 **후보 장소(주소)** 를 짚어 `places` 로 준다 — 항목은 단언이 아니라 장소 하나이고, 장소 문서 제목 아래 `찾는 말:` 줄(주인이 그 장소를 부를 일상 표현)이 사전이다. 후보는 기억이 아니라 주소라서, 맞는 곳을 `locus` 로 지명하면 그 장소의 전부가 온다. 평가 = **장소 @1**(`scripts/evaluate_forage_place.py`: 글자 일치 52% → 85%). 빗나가서 다른 곳을 연 질문은 정리 패스(`fold_place_trail`)가 결국 연 장소 문서에 `찾는 말 후보` 로 되먹인다. 몸 표기는 주소에서 정해지고(`canonical_body` — `mac`/`web`, 책·노트북만 제 이름), 단언이 40행을 넘는 문서는 자식 폴더 문서로 갈린다(`split_heavy_docs`). 자동 주입은 없다(2026-09-03 판정 그대로).
- **안전판 4**(누적의 그림자 방지): 폐기가능(prune_reason)·prior_class 게이팅(구조적만 committal prune)·surface 카운터패스·provenance+confidence.
- (역사) owner 빈도 게이트(2026-07-29): 주인모델의 1회 추론이 상시 주입되던 구멍을 "서로 다른 포식에서 재확인돼야 결정화"로 막았던 장치. 주인모델 은퇴(2026-09-18)로 함께 걷혔다 — territory 승격의 같은 모티프는 남아 있다.
...
- **다중 몸**(`forage_map.body`=포식 *공간*): 디스크(`mac`)·코드(`code:<repo>`)·웹(`web`) 등 같은 자아가 여러 공간을 포식하면 body가 갈린다(하드웨어 자아=게이트 / 포식 공간=body 키 분리). 절대 경로·URL 단언은 몸 표기와 무관하게 한 트리로 모인다. **맥 자아 전용**(주관적 기억은 자아별 사적). **#4 해마와의 차이**: 해마=절차(NL→IBL 코드), 포식=공간(공간→지도 지식). 둘 다 증류+정리 대칭.
...
UTF-8 바이트/2 추정 1800토큰·6000문자는 조각의 비정상 팽창을 막는 상한이며 지도 크기의 제한이 아니다.
실제로는 이름과 관계만 전달한다. 뜻·긴 설명·출처·조회 명령·해시·반복 안내는 주입하지 않는다.
...
`world_pulse_config.json`의 `knowledge_catalog`는 미설정이어도 켜지며 mode 기본값은 structure다.
모든 작업 에이전트의 공통 인지 경로에서 회상한다. 시스템·프로젝트·회원·위임 에이전트와
Reflex·강제 역할·문맥 갱신에도 적용한다. 과거 에이전트 허용 목록은 적용하지 않는다.
전체 진단용 enabled=false만 명시적으로 끌 수 있다. 모델 호출이 없는 세션 초기화에는 필요 없다.
세계 어휘 공유는 주인의 개인 기억 회상이나 실행 권한을 허용하는 것이 아니다.
설정 변경은 다음 턴에 반영된다. 로컬 어휘·FTS 검색이며 추가 모델 호출은 없다.
근거 문서가 바뀐 관계는 stale로 자동 확장에서 제외한다. 필수 조건의 이름을 동봉하고 충족 여부 unknown은 내부에 보존하며
...
사용자 입력
  ...
  (8 세계기억은 턴에서 쓰지 않는다 — 증류 루프 없음. 갱신 = atlas/*.yaml 편집)
```
...
| **④ 얕은 중복 판정** | ✅ 임베딩 클러스터 병합 | `find_duplicate_clusters()` — `memories_vec` 쌍별 코사인(≥0.85, 2026-09-17 기본 인코더에서 재측정해 유지) union-find. **LLM이 클러스터 내 '진짜 동일'만 병합**(예: 사용자 주소 vs 자녀 주소 구분) |
...
**이름 호출의 귀속(2026-09-09 수리)**: 저장 관용구를 `[fn:이름]`으로 실행하면 호출 자리에서 그 몸의 성공·실패를 한 번 기록하고 성공 시 몸의 실행시간을 잰다. 턴 종료의 top-1·관용구 회상 집계는 같은 이름을 다시 세지 않는다. ep3219에서는 성공한 `직전보고서찾아읽기`가 뒤의 무관한 `$원장.items` 오류 때문에 실패로도 기록됐다. 턴 전체 토큰은 특정 함수 비용으로 덮어쓰지 않는다. 본문을 직접 조합한 회상의 기존 턴 귀속은 유지한다. 검증·다른 수리: `docs/EPISODE_AUDIT_2026_09_09.md`.
...
**조언성 감사의 완료 계약(2026-09-11)**: `audit_lifecycle`이 주간 감사 등록·카덴스·결과 분류를 소유한다. `weekly_audits`는 깨끗한 성공(clean), 발견(findings), 생략(skipped), 실패(failed), 관측 부족(insufficient)을 구별하며, 개별 예외도 실패 결과로 남기고 다음 감사를 계속한다. 일부 문서·DB를 읽지 못한 결과는 성공이 아니다. 결과 파일 저장 후에만 완료 시각을 기록하며 저장 실패도 보고한다. 감사별 파일은 독립 복구 경계로 유지한다.

비용이 큰 어휘 임베딩 중복 검사만 입력·검사 코드의 지문과 전체 임베딩 관측을 확인해 주기를 늘린다. 같은 입력의 완전한 clean 3회마다 1주→2주→최대 4주로 늘고, 입력 변경은 대기를 무시하며 실패·발견·관측 부족은 원래 주기로 돌아간다. 빈 코퍼스는 검사 생략이며 clean 횟수에 넣지 않는다. 값싼 감사·빌드 정합성·부작용 관문에는 이 감쇠를 적용하지 않는다.
...
- **입구는 지도 하나**(2026-09-03): 가이드는 실행기억 가지의 산문이다 — 가지 문서 `guide:` 줄(쉼표 목록)이 `<guide_map>` 목차에 실리고(2026-09-17 — 옛 `<execution_map>` 에서 목차만 남김: 회상 용례의 가지로는 의식이 고른 가이드 194건 중 73건, 임베딩 Top-5 로는 100건만 닿아 대체 불가로 실측), 의식은 거기서 `guide_files` 를 고르며 실행자는 `read_guide` 에 파일명을 그대로 넣어 연다(파일명 정확 일치 빠른길). 옛 `<available_guides>`(guide_db 키워드 점수로 고른 최대 10개 목록)와 "작업 전 항상 read_guide 검색" 지시는 폐지 — 검색은 지도에 가지가 없을 때의 폴백. 가지 문서는 몸-사적(gitignore)이라 씨앗은 `guide_db.json` 의 `topic` 필드(추적)로 나르고, 문서의 `guide:` 줄이 있으면 그것이 이긴다(`hippo_tree.seed_guides`). 생명주기는 실제 노출되는 링크의 출처를 `tree:<가지>` 또는 `tree-seed:<가지>`로 센다.

새 설치처럼 실행기억이 비어 있어도 `guide_db.topic`의 가지는 지도에 표시된다. 목차를 만들기 위해 기억이나 가지 문서를 생성하지는 않는다. 별도의 리소스 가이드 목록은 주입하지 않고 이 지도를 공통 입구로 쓴다.

지도와 생명주기는 `hippo_tree.guide_links`의 유효 링크를 공유한다. 가지의 명시적 `guide:`가 씨앗을 대체하면 옛 씨앗은 지지 참조에서 빠진다. 지도에 실제 노출되는 씨앗은 여전히 유효한 입구다. 순환 참조의 생존은 실제 실행·신생 유예·지도/프롬프트/트리거 같은 상위 입구에 닿는지로 판정한다. 서로만 참조하는 고아 묶음은 후보 표식 이전에도 지지가 되지 않는다.

`component_lifecycle.compute_transitions(apply=False, include_details=True)`는 상태·파일을 바꾸지 않고 첫 관측일, 최근 실행 여부, 유예, 전체/유효 참조와 후보 이력을 반환한다. 이 진단이 후보 기간이나 마지막 실행 시각을 전진시키지 않는다. 유예 30일·최근 실행 60일·후보 후 90일 정책은 유지한다.

### 구성요소 생명주기 — 세포 사멸과 수면 후반부 (2026-09-02, 사용자 판정 2건 승인)

기억 8종 중 DB 기억(심층·해마·포식)에는 LRU·cap 가지치기가 있었지만 **가이드·워크플로우·스크립트·낱말**에는
...
*이 문서는 8종 메모리의 통합 지도다. 개별 시스템 변경 시 본 표와 흐름도를 함께 갱신할 것.*
...
- **감독·검수**: 원래 목표·규정·증거 참조와 응답 페이지를 읽는다. 긴 원문은 공유 작업대에서 필요할 때 읽으며 실패·미승인은 성공 경험 증류에서 제외한다. GoalEval/SelfReflect 호환 동작도 의식이 규정한 턴에만 적용한다. 의식 없는 실행·반사의 평가 미실행은 NULL이며 심층 기억은 최종 응답 뒤 기존 기준으로 선별한다.
- **검수 미완료**: 사용자 취소·호출 시간 초과·토큰 예산 중단을 구별한다. UNKNOWN은 목표 실패로 확정하지 않고 학습 보류로 남기며 증류 큐에도 상태·이유·task를 전달한다. `supervision/<turn-id>/review_status.json`의 원래 목표·응답 지문·중단 이유와 같은 작업대의 증거를 다음 요청에서 읽어 재검수한다. 자동 승인·자동 재시작은 하지 않는다. 최종 검수 토큰은 계획/관찰과 별도 몫으로 보호하며, 라운드 단위 청구의 초과분도 실제 비용에는 포함한다.
- **비용 회상**: 해마 주행의 호출·실패 머리는 IBL 범위다. 감독 턴에는 실행 전체와 의식의 호출·오류·폐기·사용량 및 턴 경과를 별도로 덧붙인다. 원장은 `supervision/<turn-id>/cost.json`, 후처리 비용은 `postprocess.json`이다. 심층 기억의 동일 ID 후보들은 묶어 판정하고 새 사실만 보충한다. 기존 기억의 앞부분만 잘라 대조하거나 같은 원문 snapshot에 여러 번 덮어쓰지 않는다.
- **병렬·출처**: IBL `&`와 each 병렬 자식은 부모의 에피소드·토큰 컨텍스트를 상속한다. 심층 기억은 원 발화 전체와 task/episode 출처를 저장하고 UPDATE 보충에도 출처를 이어 붙인다. 단일 작업 조건을 지속 선호로 일반화하지 않는다. ep3364의 과거 비용·기억·영상 정정 근거는 [수리 기록](../../docs/EPISODE_3364_REPAIRS_2026_09_10.md)에 있다.
- **에이전트 간 위임**: 메시지 prepend

보관 소유권: 실행 원문의 `tool_evidence`와 감독 작업대 `supervision/<turn-id>`는 현재
증거 참조에 만료 시점을 약속하지 않는다. `common.spill.gc`의 루트 파일 24시간 청소를
이 디렉토리들에 재귀 적용하지 않는다. 활성 턴, 재개 가능한 검수 미완료 상태,
미승인 공개 대기물, 유효한 기존 증거 참조는 보호 대상이다. 자동 회수에는 작업 신원과
참조 수명을 명시한 이전 절차가 먼저 필요하다.

실행 통로 신원은 `thread_context.execution_key`의 `(agent, task)`다. agent는 소유자와
등록된 별칭, task는 사용자 작업, run은 개별 IBL 실행 사건, supervisor turn은 검수 작업대다.
감독·조향·재규정은 동일 task의 별칭만 연결하며, 감독을 꺼도 조향 수명은 유지된다.
HTTP/MCP는 task를 명시적으로 전달한다. 작업이 없는 요청을 이벤트 루프의 다른 작업으로
추정하지 않는다. HTTP 조향의 task 생략은 활성 작업이 정확히 하나일 때만 허용한다.
원문 보관함은 신원 쌍을 구조적으로 해시한다. 구분자 없는 기존 신원의 보관 경로는 호환하며,
콜론이 섞인 옛 모호한 신원 경로를 다른 작업의 증거로 자동 채택하지 않는다.

감독 비활성 경로의 GoalEval도 평가 모델 오류·빈 응답·판정 부재·증거 수집 실패를
UNKNOWN으로 기록한다. 원 응답은 검수 미완료 표시와 함께 반환하고 성공 경험 학습은
보류한다. 별도 실패 판정 없이 자동 재실행하지 않는다. 근거와 원문은 해당 작업의
`tool_evidence` 보관함에 남긴다. 평가 라운드 미배정은 NOT_RUN, 사용자 취소는 UNKNOWN이다.
...
<memory_map note="이 자아의 심층 기억 지도(목차) — 가지 (건수) — 요약. 내용은 실리지 않는다. 답하기 전에 [self:memory]{op:&quot;recall&quot;, node:&quot;<가지>&quot;} 로 연다. 지속 가치가 있는 사용자 사실은 최종 응답 후 자동 선별된다. save 호출은 필요 없다.">
...
에이전트별 SQLite DB로 운영되는 장기 사실 저장소. 2026-05-16 에 해마와 같은 fine-tuned 모델로 시맨틱 검색을 추가했고, **2026-09-17 에 그 모델에서 떼어 냈다**: 해마 모델은 IBL 용례 회상을 위해 재학습되고 그때마다 공간이 바뀌는데 재학습 절차는 해마만 재색인했다 — 실측으로 09-04 재학습 이전에 저장된 기억의 벡터는 현재 모델로 다시 임베딩한 것과 코사인 중앙 0.71(최소 0.38)이었고, 검색도 중복 판정도 옛 공간의 벡터에 새 공간의 질의를 던지고 있었다. 이제 기억의 벡터는 재학습하지 않는 `jhgan/ko-sroberta-multitask`(공통 회상 `tree_recall` 과 같은 객체)로 만들고, DB 마다 `_meta.vec_encoder` 표식을 남겨 **표식이 어긋난 벡터는 쓰이기 전에 통째로 재임베딩**한다(`memory_db._ensure_vec_current`) — "모델을 바꾸고 재색인을 잊는" 부류의 구조적 봉인. 중복 후보 품질도 기본 모델이 나았다(해마는 'ESP32 는 2.4GHz 만'과 'WiFi 이름은 …'을 0.88 로 묶었다).
...
| **검색** | 두 길, 한 인코더. ① **AI 의 `op:"search"`** = 자동 주입과 같은 엔진(`tree_recall.search` — 가지 경로가 실린 항목 벡터 + 의미·글자 역순위 융합, 의미 채널 하한 0.55, `node` = 가지 필터). ② **증류의 중복 조회** = `memories_vec`(경로 없는 텍스트) 시맨틱 우선 + LIKE 폴백, 컷오프 `SEMANTIC_THRESHOLD` 0.55(옛 0.4 와 같은 통과율로 재측정). 텍스트가 다른 이유: 같은 가지라는 이유로 다른 사실이 가까워지면 중복 판정이 틀어진다 |
| **인덱스** | vec0 가상 테이블 (`memories_vec`, 같은-사실 판정용) + 회상 색인 `data/recall_index/`(관련성용, git 밖) — 둘 다 기본 인코더 768d, 인코더 표식/텍스트 해시로 스스로 최신을 유지 |
| **자동 동기** | save / update / delete 시 vec 인덱스 자동 갱신 |

### 자동 증류 (`_distill_deep_memory`)

최종 검수·응답 후 기존 영속 큐에서 사용자 원문의 지속 사실·선호·확정 결정만 선별한다. 도구 `save`는 즉시 저장하거나 작성 본문을 대기열에 넣지 않고 `saved:false`로 자동 선별 정책을 안내한다. 대화 밖의 직접 호출도 저장하지 않는다. 최종 보고 전의 초안이 나중에 그대로 저장되는 경로는 없다. 취소·예외 또는 검수 미완료(UNKNOWN/NOT_ACHIEVED)인 턴은 심층 기억을 쓰지 않는다. 평가가 필요 없어 생략된 정상 응답은 선별할 수 있다.

사실성만으로는 부족하다. 선별 모델은 `future_use`에 이번 대화 밖의 어떤 판단·행동에 계속 필요한지 설명해야 하고, 코드가 사용자 원문 ID·질문/요청 여부·retention·가치 사유의 존재를 검증한다. 의미상 지속 가치 판단은 기존 경량 모델이 맡으며 추가 평가 모델은 두지 않는다. 저장 본문은 선택된 원문 그대로이고 가치 판단 문구는 사실 본문으로 저장하지 않는다. 출처에는 원 발화·시점·최종 응답 지문을 남긴다. AI가 작성한 분석·권고·조사 결과, 시세 등 휘발성 정보, 사소한 사실, 일회성 작업 조건은 제외하며 0건 저장은 정상이다. 의견은 세계의 사실로 확정하지 않고, 명시적인 지속 선호·확정 결정에 한해 사용자가 그렇게 말했다는 사실로만 남긴다. 조사·작업의 상세는 에피소드와 산출물에서 회상한다.

- NEW: 기존에 없는 새 정보
- UPDATE: 새 사실을 출처가 있는 별도 행으로 보충
- REPLACE: 사용자의 명시적 정정만 적용하며 이전 본문·출처 보존
- SAME: 이미 알고 있음 → 추가 저장·갱신 없음
...
크롤 전문은 `source_ref`와 턴 변수에 보존하고 모델 표시만 `_preview`로 줄인다(2026-09-09). 이 미리보기는 원천 실패가 아니다. 원문 저장과 AI의 전량 검토는 별개이며, 추가 읽기는 저장 값을 사용한다. [원문 보관과 표시 분리](../../docs/CRAWL_SOURCE_RETENTION_2026_09_09.md).

대화가 끝난 후, 해마가 모르는 패턴이었는데 실행이 성공했으면 경량 AI가 그 경험에서 용례를 추출하여 해마에 저장한다.

**원천 절단 검수(2026-09-09)**: 실행 성공과 전량 수집은 다르다. 도구 로그의 `evidence.truncations`에서 `scope: source`가 확인된 호출은 증류 후보에서 제외하고, 재수집에 성공한 후속 호출은 학습할 수 있다. 행 표본(`selection`)·범위 불명(`unknown`)은 수집 범위를 설명하도록 증류 프롬프트에 전달하며 표시용 `_preview`만으로는 제외하지 않는다. 평가 trace에도 절단 건수와 범위를 남긴다. [ep3286 수리와 기존 용례 정리](../../docs/EPISODE3286_TRUNCATION_REPAIR_2026_09_09.md).

### 증류 조건

**효용·비용 관문(2026-09-20)**: 아래 점수 조건을 통과해도 이미 저장된 전체 절차나
URL만 바꾼 기존 원시 호출은 모델 호출 전에 생략한다. 남은 후보는 기존 반성 호출
한 번에서 기존 용례·검수 상태·짧은 결과 발췌를 보고 `keep/skip`, 개선점, 적용 조건,
원문 번호를 함께 선택한다. 새 배움이 없으면 0건이 정상이다. 코드 재작성·자동 복구
재호출은 하지 않는다. 입력 24,000자 초과·검증 불가도 원장은 보존하고 증류는 보류한다.
선택한 원문과 필요한 변수 생산자만 저장하며, 가지의 주행 절에도 검증된 선택 절차만
남긴다. 탐색 전체를 성공 절차로 복제하던 이전 동작을 대체한다. DB·학습 JSON의
`provenance`는 과제/에피소드 식별자(확인 가능한 경우), 원 호출·문장 번호와 해시,
목표 검수, 개선점·적용 조건을 보존한다. 부분 절차도 자동 정리 대상에 포함한다.
상세·검증: [증류 효용 보강](../../docs/DISTILL_VALUE_2026_09_20.md).
...
0.7 미만 + 도구 호출 있음? → distill_experience() (경량 AI)
    ↓
성공 명시·검수·원천 완결성 확인 → 기존 용례와 중복/주소 교체이면 호출 없이 종료
    ↓
입력 예산 안에서 반성 1회 → 새 효용 없으면 종료
    ↓
원문 번호 선택·의존성 닫기·검증 → 설명·근거·출처와 함께 저장
    ↓
...
| 전대상피질 | 별도 평가자 | 의식이 명시한 달성 기준만 증거와 대조. 기준이 없으면 평가 생략 |
...
### 여행 턴 회상·증류 정합성 (2026-09-11)

대화 전경 회상은 사용자 디스크를 자동 순회하지 않는다. 각 준비 단계는 `recall.started/finished/stalled`로 기록하며 멈춤 감시에 별도 모델을 호출하지 않는다. 명시적인 파일 도구는 그대로 사용한다.

심층 기억 검색은 질의와 맞는 발췌·잘림 여부·원문 길이·출처를 반환한다. recall은 소개·자유 산문과 기억 본문을 한 벌씩 반환하며 사용자 발화, AI 기록, 혼합·미확인 출처를 구분한다. 관련성 점수는 현재 사건에 적용할 권한이 아니다. 자동 증류 후보는 사용자가 말한 지속 사실·선호·결정의 원문이며 질문과 AI 답변은 후보에서 제외한다. 보충은 별도 사건 행으로 저장하고 명시적 정정은 이전 본문과 출처 이력을 남긴다.

기존 요청 분류의 `CONTEXT_UPDATE`는 사실 통보의 기억 확인·갱신 범위를 실행자와 감독 도구 양쪽에서 지킨다. 사전의 `context_update_ops`가 허용 연산을 선언한다. 단순 기억 조회와 빈 결과는 새 성공 절차의 학습 재료로 삼지 않으며, 사용자 목표 검수 없는 도구 성공은 전체 목표 성공으로 승격하지 않는다. 구현·검증 기록은 [3507–3510 수리](../../docs/EPISODE_3507_3510_REPAIRS_2026_09_11.md).
