...
`ibl_nodes_src/*.yaml`(단일 진실) → `build_ibl_nodes.py`(삼각 검증) → `ibl_nodes.yaml`(런타임 캐시). 157개 액션이 곧 어휘화된 방법 지식.
...
- **어휘 정합**: neighbor 통합 후 `[others:neighbors]`→`[others:neighbor]{op}` relabel + phone_sync·neighbor save/favorite 용례 보강. 코퍼스(usage_db)는 항상 최신 어휘로 마이그·재색인 유지(~2,600건, rebuild_index 2605 — 통화 대수·비즈니스·국회도서관 인물/학위논문 용례 누적. 현 6노드 **157 액션** 어휘. 신규 액션(공개 표면·숙박·개체해소 등)은 capability 게이트로 보강, 다음 임베더 재학습 때 흡수).
