...
2026-09-12 수리: keep/amend는 장기 규정을 먼저 보존한 뒤 이번 턴 검토의 `criteria`를
실행 기준으로 전달한다. 옛 예상 IBL·전문가 선택·행동 힌트·접근법은 현재 지시에서 빼고
`guide_files`는 유지한다. 요약은 잘못된 필드의 검증 오류를 한 번에 알리며 기존 전체 재요청을
사용한다. 다음 턴의 동기 요약 실패도 원문을 보존하고 정지한다. 후처리 큐 비용은 실패 시도까지
기존 `postprocess.json`에 누적한다(캐시 포함 토큰·시도 실행시간, 전경·대기 제외, 누락은 미확인).
범위와 검증: [결과 전달·과제 갱신 수리](../../docs/TASK_WORK_NOTE_PLAN_2026_09_11.md).
