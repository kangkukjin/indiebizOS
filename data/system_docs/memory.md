...
- **닫힌 루프**: ③ AI 가 장소를 보고 `[self:forage]{op:"recall"}` 로 회상(도구 결과로 `<forage_memory>` 수령 — 옛 자동 주입 `_search_forage_memory` 는 2026-09-03 폐지, 아래 **진행** 참조) → ② AI가 포식(`file_find`/`grep`/`read`) → ④ 종료 훅에서 *일반화 가능한 지도 델타만* 증류(`_distill_forage_memory`, 날 내용·특정 파일 제외) → ⑤ 기존 라벨 위반 이질 내용은 surface 표식(필터버블 반대힘).
