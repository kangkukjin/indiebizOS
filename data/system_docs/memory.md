...
### 현재 성능 (2026-08-04 **로컬 맥미니 M4 Pro 재학습** — code Top-5 92.2%/desc 94.2%)

2026-08-04 재학습(163 액션 어휘, **2,988 코퍼스**, batch=8, epoch 5 최적·검증 0.882)의 측정표.
...
- **어휘 정합**: neighbor 통합 후 `[others:neighbors]`→`[others:neighbor]{op}` relabel + phone_sync·neighbor save/favorite 용례 보강. 코퍼스(usage_db)는 항상 최신 어휘로 마이그·재색인 유지(**3,032건**, 증류 누적 `ibl_distilled.json` 819건 — 통화 대수·비즈니스·공개 표면·창고·음악·웹앱 용례 누적. 현 6노드 **150 액션** 어휘. ★어휘를 지우면 코퍼스도 따라온다 — 음악앱 5기능 은퇴 때 `--check` 의 코퍼스 param 정합 가드가 죽은 파라미터를 잡아 용례 15건 삭제를 강제했다. 신규 액션(몸 부탁·USB 손발·신문 발행·내 음악 등)은 시딩 후 capability 게이트로 보강, 다음 임베더 재학습 때 흡수).
