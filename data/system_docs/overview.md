...
- **자동응답 V2**: AI 판단 → 비즈니스 검색 → 응답 생성 → [상세 문서](auto_response.md)

## 통합 아키텍처
시스템 AI와 모든 프로젝트 에이전트가 **동일한 코드베이스**를 공유합니다:
- **AIAgent 클래스**: 단일 코어로 모든 AI 처리 통합
- **공통 프롬프트**: `base_prompt_v2.md` 기반으로 일관된 AI 행동 원칙 적용
- **프로바이더 통합**: 새 AI 프로바이더 추가 시 시스템 AI + 모든 에이전트에 자동 적용
- **프롬프트 구조**: 공통 설정 + 조건부 프래그먼트 + 개별 역할 + 컨텍스트(동적 주입)
- **XML 태그 구조**: 모든 프롬프트에서 AI 파싱 정확도를 위해 XML 태그 사용

## 시스템 AI 위임 기능
시스템 AI는 프로젝트의 전문 에이전트에게 작업을 위임할 수 있습니다:
- **list_project_agents**: 프로젝트/에이전트 목록 조회
