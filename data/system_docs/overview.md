...
각 에이전트는 개별적으로 프롬프트(역할)와 도구를 설정할 수 있습니다.
(모델·API 키는 에이전트별이 아니라 시스템 전체의 모델 기어가 결정합니다 — 2026-06-30 per-agent 모델 폐지.)
...
## 현재 상태
- 마지막 업데이트: 2026-06-30
- IBL: 5노드 142 액션 (sense 44, self 44, limbs 17, others 11, engines 26)
- 설치된 도구 패키지: 38
- 기억 7종 (의미·작업·일화·절차·관계·자기상태 + 포식 기억 forager, 2026-06-20 추가)
- 최근: 모델 기어(계기판 변속) — 레버 하나(절약/균형/최대)로 시스템 전체 모델 등급 변속, 4축(분류·평가·실행·의식)×3티어, 핫리로드 + per-agent 모델 폐지(모델·키 티어 상속); 폰 엔진 번들을 data/bodies 프로파일에서 파생; 앱 모드 인터랙티브 렌더 프리미티브(인터랙티브 map·on: 뷰이벤트·동적필터); 부동산 직방 호가(sense:realty source:zigbang); 국회도서관 국가학술정보(sense:researcher·sense:paper source:nanet)
