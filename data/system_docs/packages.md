...
런처 → **내 어휘**에서 **단어묶음 저장고**를 우클릭 → **파일 가져오기**로 `.iblpack`을 넣는다. 받은 묶음은
...
런처 **내 어휘**를 고르면 제목·상단 버튼 없이 아이콘과 이름만 나타난다. 안경 메뉴에는 중복 진입점이 없다.
...
폴더 창은 제목 표시줄을 끌어 이동한다. 제목 표시줄에 초점을 두고 방향키로도 옮길 수 있다. 내 어휘 화면을 사용하는 동안 폴더별 창 위치를 기억하며, 화면 크기가 바뀌면 창이 바깥으로 벗어나지 않도록 조정한다.
