...
| **원격 런처** | `/launcher/app` | 데스크탑과 동형인 5탭(자율주행 / 조종실 / 앱 계기 / 공유창고 / 포식) |

각 기능은 **별도의 비밀번호**로 독립적으로 활성화할 수 있습니다.

**홈 화면 설치(2026-07-21)**: 둘 다 웹 앱 매니페스트를 실어서 "홈 화면에 추가"하면 각자 아이콘·이름을 가진
앱이 된다(런처 / **IBFind**). 스토어도 서명도 플랫폼별 빌드도 없다. 서비스워커는 **아무것도 캐시하지 않는다** —
둘 다 내가 소유한 기계의 라이브 뷰라서.

> **참고 — 원격 접근 ≠ 공개 표면**: 위 두 기능은 *나 자신*이 외부에서 내 시스템을 원격 제어하는
> 리모컨이다(비밀번호 필수). 이와 별개로 2026-07부터 *남*이 브라우저로 닿는 **공개 표면 가족**이
> 있다 — 개인 포털 `/h/`·공개 파일 `/s/`·가족신문 `/n/`·게시판 `/b/`·정기보고 `/r/`. 이들은
> `public-files` Cloudflare Worker가 경로 접두로 분기해 터널로 맥에 서빙하며(레벨/로그인 기반),
> 별도 문서는 [communication.md](communication.md) "공개 표면 — 커뮤니티당 노드 하나" 참조.
...
### 결과는 *어느 표면이 물었나* 로 착지한다

실행한 몸이 아니라 **요청한 표면**이 착지점을 정한다 — 폰 브라우저에서 누른 재생은 폰에서 나고, 데스크탑에서
누르면 그 기계에서 난다(명시 `mode` 는 여전히 우선). 다운로드는 손에 든 기기로 떨어진다.
★반대 방향(브라우저→PC)으로 같은 축을 태우려던 **clipbox** 는 2026-07-28 철회됐다 — '폰으로'는 종전 푸시 큐를 그대로 쓴다.

### 몸의 주소는 파생된다 (2026-07-21)

원격 런처·파인더 주소를 파일마다 설정해 두지 않는다. **창고가 실제로 서빙하는 얼굴**에서 `origin_host()` 가
파생하고(권위=`public_face.provider`, Cloudflare ↔ Tailscale 전환을 따라감), 인증 게이트는 **fail-closed**
(미등록 호스트=외부 취급, 프록시 신호를 `Host` 보다 먼저 읽는다 — cloudflared 가 Host 를 재작성하므로).
Cloudflare 발급(`cdn_provision`)은 터널뿐 아니라 R2 캐시 Worker 까지 만든다.
...
*마지막 업데이트: 2026-08-01 — **원격 표면의 실사용 수리 + 웹앱을 세다**: ①**윈도우 원격 NAS 파인더 폴더 진입 404**(2026-07-28): 최상위는 보이는데 하위 폴더 클릭이 전부 실패하던 진범은 경로를 인라인 `onclick` JS 문자열에 심을 때 작은따옴표만 이스케이프한 것 — 윈도우 경로 `D:\NAS\문서` 의 `\N`·`\문` 이 JS 이스케이프로 먹혀 뭉개졌다(맥 경로엔 `\` 가 없어 잠복하던 버그). `jsEsc` 신설(백슬래시+따옴표 JS 이스케이프 → HTML 엔티티로 속성 경계 보호). ★파생 함정: Python 안에 쓰는 JS 는 백슬래시가 2겹이다. ②**PC 관리 파일탐색**(윈도우+원격 NAS): `os.scandir` 전환(옛 pathlib 는 항목당 왕복이라 SMB 원격 폴더가 분 단위), SMB 오류를 500 추락 대신 502+진단으로, 드라이브(`C:\`)·UNC(`\\nas\공유`)·POSIX 겸용 경로 유틸. ③**clipbox 철회**(2026-07-28): 브라우저→PC 수신면을 만들었다가 되돌렸다 — '폰으로'는 종전 푸시 큐를 그대로 쓴다. ④**원격 파리티**(2026-07-29): 창고 이웃찾기 둘러보기·📣 공개 추천이 데스크탑과 같은 API·같은 계약으로 원격 런처에도(백엔드 신규 0). ⑤**웹앱 등기부**(2026-08-01): 이 몸이 서빙하는 면(런처·IBFind PWA·포털·공개파일·게시판·가족신문·정기보고)과 외부 배포 웹앱을 `[self:webapp]` 이 파생으로 세고 생존을 실측한다. ★같은 origin 에 PWA 를 둘 이상 두면 안드로이드가 알림 권한을 한쪽 WebAPK 에만 위임한다(웹 푸시 은퇴의 진범) — 알림이 필요한 면은 서브도메인으로 origin 을 독점해야 한다. 이전(2026-07-25) — **표면 분리 + 주소 파생 + 홈 화면 설치**: ①원격 런처가 5탭(자율주행·조종실·앱·공유창고·포식)으로 정리되고, **원격 런처(=PC 의 일부)와 폰 네이티브(=독립 시스템, 3탭)**의 조립을 코드에서 갈랐다(기질=`launcher_app_*` 탭 모듈 / 정체=`launcher_surface_remote|phone`). 폰 조종실은 로컬 완결(translate·validate·distill·catalog) — 옛 프록시 리모컨 은퇴. ②**착지점은 어느 표면이 물었나로 결정**(폰 브라우저 재생=폰 스피커), 클립보드는 반대 방향으로 `clipbox`. ③**주소 파생**: `origin_host()` 가 창고가 서빙하는 얼굴을 따라가고(권위=public_face.provider), 인증 게이트 fail-closed(프록시 신호 > Host). ④**홈 화면 설치**: 런처와 파인더(**IBFind**)가 각각 웹 앱 매니페스트를 실어 아이콘 있는 앱이 된다(서비스워커는 캐시 0=항상 라이브). 이전(2026-07-17) — 원격 Finder/런처(리모컨) 본문 불변. 공개 표면 가족(포털 `/h/`·공개파일 `/s/`·가족신문 `/n/`·게시판 `/b/`·정기보고 `/r/`, "커뮤니티당 노드 하나")과의 구분 주석 추가 — 상세는 communication.md. 이전: 2026-03-27*
