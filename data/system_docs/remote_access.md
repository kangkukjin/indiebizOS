...
주소록·건강·재무의 `/business/sync/*`, `/health/sync/*`, `/finance/sync/*`는 `sync_exchange`의 공통 스냅샷 교환 절차를 사용한다. 직접 스냅샷과 `{data: ...}` 봉투를 모두 받으며, 형식 확인→도메인 병합→최신 스냅샷 반환 순서다. LWW·tombstone·이미지/소유자 해소는 각 도메인 병합기가 맡는다. 이 URL들은 계속 원격 세션 인증 뒤에 있으며 공개 자산 경로가 아니다.
...
### 클라이언트 담당 에이전트 (2026-09-15)

웹앱의 채팅과 앱 요청은 `/m/run`(`/m/requests` 별칭)의 버전 1 요청 계약에서 합류한다. `client_agent`가 인증된 회원별 요청·중복·산출물 전달 상태를 관리하고 기존 회원 러너/IBL을 실행한다. 공개 웹 검색·원문 확인은 주인 브라우저와 개인 자료를 사용하지 않는 감사된 경로다. 보고서 앱은 실제 조사 후 산출물을 반환하고 브라우저 저장·지문 확인 및 `/m/receipts` 수신 뒤 전달 완료가 된다. 허브 텍스트 작업 자체에는 USB/네이티브 연결이 필요 없다. 주인 원격 런처·`/nodes/ask`는 기존 경로를 유지한다. 세부 계약과 후속 범위: `docs/CLIENT_AGENT_WEBAPP_IMPLEMENTATION_2026_09_15.md`.
