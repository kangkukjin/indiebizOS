...
누르면 그 기계에서 난다(명시 `mode` 는 여전히 우선). 다운로드는 손에 든 기기로 떨어지고, 클립보드는 반대 방향으로
같은 축을 타 **clipbox**(`/launcher/clipbox`)에 담긴다.
...
| `/launcher/clipbox` | GET/POST/DELETE | ✓ | 브라우저↔PC 메시지 함(데스크탑 '폰으로'가 적재, 조종실이 표시) |
...
- **조종실**: 자연어→IBL 번역 → dry-run 검수 → 실행 (+ 시스템 상태·모델 기어·주행기록·clipbox)
- **앱**: `app:` 블록에서 파생된 계기를 데스크탑과 같은 어휘로 렌더 (0토큰)
- **공유창고**: 내 창고 파인더(업로드·이동) + 이웃 탭(피드 카드·검색·리트윗·DM·인앱 파인더)
- **포식**: 파일/디스크 포식 브라우저
