...
*마지막 업데이트: 2026-08-17 — 수치 정합화: 6노드 **144 액션**(sense 40·self 48·limbs 14·others 17·engines 9·table 16)·**40 도구 패키지 + 5 extensions**. 스케줄 관련 어휘 주의 2건: ①**문장 자리의 이름은 `do` 로 통일**됐다(2026-08-15 — `trigger.pipeline`·`workflow.steps`·`schedule.pipeline`·`manage_events.event_action`·`delegate.steps` 가 `do` 별칭을 흡수. 핸들러 읽기키와 저장 데이터는 불변이라 기존 스케줄은 무손상) ②반복·조건 달성형 요구("매일 아침 확인", "조건 충족까지")는 스케줄이 아니라 **`[goal: "..."]{every, max_rounds, success_condition, strategy}` 블록**이 정본이고, 작성 전 `read_guide` 로 goal 가이드를 읽어야 한다(`max_rounds`/`max_cost` 필수 안전장치 규약). 본문 불변. 이전(2026-08-06) — 수치 정합화: 6노드 **150 액션**·40 도구 패키지. 본문 불변. 이전(2026-08-01) — 현 상태 정합화: **6노드 163 액션**(sense 48·self 52·limbs 18·others 18·engines 14·table 13)·42 도구 패키지. 스케줄러 본문 불변. 옆에서 달라진 것 둘: ①`[self:trigger]{op:list}`·`[self:switch]`·`[self:package]`·`[self:limb]` 가 목록을 내면서 `returns: effect` 를 달고 있어 `>> [table:*]` 파이프가 끊겨 있던 것을 **items 통화 병행 방출**로 봉합(★`self:trigger` 의 items 는 triggers 만 — 종류가 다른 옛 일정 목록을 한 통화에 섞는 것은 통화 수리가 아니라 의미 결정이다). ②부작용 여부가 통화(`returns`)에서 분리되어 `side_effect:` 선언이 됐다 — 스케줄/트리거 dry-run 검수가 삭제성 op 를 '부작용 없음'으로 통과시키던 부류의 해소. 이전(2026-07-25) — 당시 정합화: 6노드 162 액션. 스케줄 어휘(`[self:manage_events]`·`[self:schedule]`·`[self:switch]`·`[self:trigger]`·`[self:workflow]`)와 CalendarManager 데이터 구조는 변경 없음. **신문 발행이 결정론 액션으로 결정화**(2026-07-23): 스케줄에서 신문을 뽑을 때 자연어 위임(`[others:delegate]{scope:"system"}`) 대신 **`[engines:newspaper]{}`**를 직접 걸 수 있다(기본 백그라운드=즉시 반환 후 `newspaper_publish_state.json` 로 상태 확인, `wait:true`=동기 ~55초). 어느 표면에서 발행해도 같은 신문(제호·키워드=`outputs/newspaper_config.json` 단일 소스). 정기보고 '작성'은 본질이 LLM 글쓰기라 위임(`[others:delegate]{scope:"system"}`) 유지. **에피소드 로깅이 스케줄러 하달 경로에도 배선**(위임 루프 start/end 한 쌍 — 자가점검 `__health_check__` 는 제외해 주행기록 밀어냄 방지). 이전(2026-07-21) — `[self:manage_events]` op 어휘 단일화 합류: 파라미터 `action:`→`op:` 정규화(+`ops:` 블록으로 값 어휘화 list/create/update/delete/toggle/run_now, 구 action=은 aliases 호환). 이전(2026-07-17) — 현 상태 정합화: **6노드 157 액션**(sense 48·self 49·limbs 17·others 17·engines 13·table 13)·40 도구 패키지. 스케줄 어휘(`[self:manage_events]`·`[self:schedule]`·`[self:switch]`·`[self:trigger]`·`[self:workflow]`) 및 CalendarManager 데이터 구조는 변경 없음. 참고: 정기보고 04시 스케줄러가 은퇴된 `[self:report]` 대신 `[others:delegate]{scope:"system"}`(자율주행 위임, 헤드리스 fire-and-forget)로 교체됨. 이전(2026-06-27) — 앱 표면 품질 일괄 개선(라디오 즐겨찾기·CCTV 인앱 재생 stream 버튼·여행 날짜+한국 지방공항·투자 TIGER200·날씨 오송·문화 지역·길찾기 거리/예상시간) + 부동산 직방 호가(sense:realty source:zigbang)·AI 공모/창업(sense:contest/startup) + read_guide claude_code 노출 + 폰 네이티브 재빌드. 142 액션(sense 44·self 44·limbs 17·others 11·engines 26)·38 도구 패키지. 이전(2026-06-22) — 폐지된 도구명(`manage_schedule`·`list_switches` 도구) 잔재 정정: 현재 스케줄 어휘는 `[self:manage_events]`(등록/조회/삭제)·`[self:schedule]`(지연/예약/반복)·`[self:switch]{op}`·`[self:trigger]{op}`·`[self:workflow]{op}`. 본문 JSON/REST 모델·CalendarManager 데이터 구조는 변경 없음. 이전: 2026-03-27*
