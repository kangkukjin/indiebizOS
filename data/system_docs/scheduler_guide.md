...
**대화 없는 프로젝트 간 위임(2026-09-16)**: 소유 에이전트 없는 `run_pipeline`은 IBL을 직접 실행한다. 이때 `[others:delegate]{scope:"cross"}`는 시스템 작업 DB에 새 부모 작업을 만들고 프로젝트의 자식 작업을 연결한다. 접수 뒤 부모는 대기 상태이며, 기존 결과 회수·시스템 AI 보고 경로가 완료한다. 알림의 **스케줄 위임 접수**와 실제 재조사 완료를 구분한다. 트리거의 성공 이력은 파이프라인의 위임 접수 성공이고, 위임 결과는 부모 작업·시스템 AI 대화에서 확인한다. 발송 실패는 부모를 종료하며, 각 발화의 신원·위임 플래그는 실행 후 복원한다. 같은 프로젝트 위임은 기존 프로젝트 DB 경로를 유지한다. 포식기억 주간 재조사도 이 계약을 따른다.
...
- **문장 자리의 이름은 `do`** (2026-08-15 통일): `trigger.pipeline`·`workflow.steps`·`schedule.pipeline`·`manage_events.event_action`·`delegate.steps` 가 전부 `do` 별칭을 흡수했다(핸들러 읽기키·저장 데이터 불변 — 기존 스케줄 무손상).
- **반복·조건 달성형 요구("매일 아침 확인", "조건 충족까지")는 스케줄이 아니라 `[goal: "..."]{every, max_rounds, success_condition, strategy}` 블록이 정본** — 작성 전 `read_guide` 로 goal 가이드를 읽을 것(`max_rounds`/`max_cost` 필수 안전장치).
