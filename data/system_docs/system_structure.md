...
*마지막 업데이트: 2026-06-02 — 런처 3표면(자율주행/수동/앱) 정립 + 수동 모드(IBL 컴파일러 프론트엔드: /ibl translate·validate·execute·distill + ManualMode.tsx) 신설 + 앱 모드 계기 도서검색(국내/네이버/글로벌/고전) 추가 + realty 액션 개선(region 이름 자동해소·villa·dong 필터·에러 표면화). 이전(2026-05-31): THINK 경로 framing 재사용 게이트 추가(연속 turn에서 의식 Opus 호출 스킵). 이전(2026-05-28): 라운드 2 정리(195 액션) + op 어휘 단일화(24 액션 ops 블록) + 삼각 검증 인프라(`build_ibl_nodes.py --check`, pre-commit, self-check 합류) + dispatcher 표준화(9 패키지 `_OP_DISPATCHERS`) + consciousness 메타 인지 3 가드. 현재 5-Node 199 액션 / 36 패키지.*
