...
- **파일 목록** (12개):
  ...
  - `ibl.md` - IBL 명세 — 5-Node·109 액션·op 어휘화·삼각 검증
...
*마지막 업데이트: 2026-06-10 — 인지 경로 개편: 중급 모델 Reflex 전용화(무의식 EXECUTE는 본격 모델, 오분류 품질 방어) + 무의식 분류기 재조정(THINK 과잉 축소) + 의식 프롬프트에 "좋은 문제 규정"·"IBL과 코딩의 우선순위" 추가. 이전(2026-06-05~06): 안드로이드 얇은 부활([limbs:android]{op}) + 폰 컴패니언([sense:phone]{op}, NIP-17 한방향 피드) + Nostr DM NIP-17 전환 + 음악 작곡 은퇴. 이전(2026-06-04): IBL 사용성 재감사 종결. 이전(2026-06-02): 런처 3표면 + 수동 모드 + 앱 모드. 현재 5-Node 111 액션 / 35 패키지.*
