...
- **파일 목록** (12개):
  ...
  - `ibl.md` - IBL 명세 — 5-Node·122 액션·op 어휘화·삼각 검증
...
*마지막 업데이트: 2026-06-12 — 메신저/커뮤니티/비즈니스 IBL 앱모드 계기화(옛 bespoke 창 은퇴) + 자동응답 IBL화 + 폰↔PC business.db 합집합 동기화(self:phone_sync) + neighbor 통합(neighbors 은퇴) + periods→filter + 해마 로컬 재학습(M4 Pro, code Top-5 92.6%/desc 92.8%). 이전(2026-06-10): 인지 경로 개편(중급 Reflex 전용화·무의식 분류기 재조정·의식 프롬프트 메타-메타). 이전(2026-06-05~06): 안드로이드 얇은 부활 + 폰 컴패니언 + Nostr NIP-17 전환 + 음악 작곡 은퇴. 이전(2026-06-02): 런처 3표면. 현재 5-Node 122 액션 / 35 패키지.*
