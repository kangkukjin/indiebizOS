...
## IBL 도구 아키텍처 (Phase 24: 7-Node, verb 제거)
...
예시:
execute_ibl(code='[source:price]("AAPL")')
execute_ibl(code='[source:web_search]("AI") >> [system:file]("result.md")')
execute_ibl(code='[source:price]("AAPL") & [source:price]("MSFT")')
```
...
Phase 19→22→23: 인프라 노드 → system, 정보 수집 → source, 기기 → interface, 미디어 → stream, 제작 → forge, 위임 → team

| IBL 액션 | 기존 도구명 | 설명 |
|----------|-----------|------|
| `[team:delegate]` | `call_agent` | 다른 에이전트에게 작업 위임 |
| `[team:ask]` | `call_agent` | 에이전트에게 질문/위임 |
| `[team:ask_sync]` | - | 동기 에이전트 호출 (파이프라인용) |
| `[team:delegate_project]` | `call_project_agent` | 프로젝트 간 위임 (시스템AI 전용) |
| `[team:delegate_workflow]` | - | 워크플로우 기반 위임 |
| `[team:info]` | `list_agents` | 에이전트 정보 조회 |
| `[team:list_projects]` | - | 프로젝트 목록 조회 (시스템AI) |
...
### 인프라 노드 (항상 허용)
`system`, `team` — 모든 에이전트에 자동 제공 (`_ALWAYS_ALLOWED`)

### 7-Node 구조 (Phase 24: verb 제거)
| 노드 | 액션 수 | 카테고리 | 설명 |
|------|---------|---------|------|
| `source` | 105 | search, get, list, create, delete, write | 데이터 검색/조회 (외부 정보 + 내부 저장소) |
| `interface` | 79 | click, type, get, screenshot, control, list, read, send, search, delete | UI 조작 (브라우저, 안드로이드, 데스크탑) |
| `system` | ~54 | get, io, list, event, notify, run, control, fs, storage | 시스템 관리, 사용자 소통, 워크플로우, 파일시스템 |
| `forge` | 46 | create, get, list, run | 콘텐츠 생성 (슬라이드, 영상, 차트, 이미지, 웹사이트) |
| `stream` | 18 | get, control, search, play, stop | 미디어 재생 (유튜브, 라디오) |
| `messenger` | 9 | search, get, send, read | 연락처 관리, 메시지 전송 |
| `team` | 7 | - | 에이전트 위임/협업 |
...
---
*마지막 업데이트: 2026-03-05 (Phase 24: verb 제거, category 태그 도입, 시스템 AI 도구 확장)*
