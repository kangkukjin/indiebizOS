...
- `backend/`: 서버 소스 코드 — **층=디렉토리**(2026-08-05 물리 이동). 의존은 아래→위 한 방향:
  `base`(26) → `datastore`(40) → `ibl`(40) → `cognition`(47) → `services`(28) → `surface`(61). `.py` 총 298개(test 제외).
...
- `data/guides/`: 가이드 70개 (guide_db 등록 69). `codebase_map.md` 는 system_structure.md 에서 **자동 파생**이므로 직접 편집 금지
<!-- IBL_STATS:END -->
