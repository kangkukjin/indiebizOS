...
2. ✅ 채널 정책 — **판단으로 집행**(2026-09-18, 사용자: "판단할 수 있는 걸 측정 뒤로 미루지 말 것"): `CHANNELS` 의 `agent_message`·`switch` 예외를 없앴다.
   에이전트 간 경로가 글자 채널을 못 받던 것은 분류가 없어 생긴 결함이지 정책이 아니었다(세계 어휘는 모든 작업 에이전트의 것, §5.6) — `route("EXECUTE")`.
   스위치 명령도 주인의 명령이고 실행자는 같은 IBL 환경을 받으므로 가이드 목차·판정 원장·세계 지도가 빠질 이유가 없다 — 심층기억은 그 자아의 DB 가 있을 때만(자리표 러너에 project_path·agent_id 전달, 이름 없는 자아는 0토큰).
   실측(통합 후): agent_message 7블록(종전 6), switch 4~6블록(종전 1). 파이프라인 채널은 불변(고정물 14/14).
3. ✅ 후보 수 — `BUDGET` 표로 이전(값 불변): 해마 낱말 5·관용구 2, 심층·세계 가지 2→안 2·밖 1. 글자 채널 예산은 이미 데이터(`world_pulse_config.json`)라 두지 않는다. 검색기 통합은 하지 않는다(해마 전용 인코더 유지 — 세계 어휘 13/24 대 20/24 실측이 근거).
4. ✅ 가지 문서 기질 공통화 — §9 (2026-09-18 집행).
...
## 9. 가지 문서 기질 (2단계 ④, 2026-09-18 집행)

세 트리 기억의 문서 처리는 같은 원리였다 — 머리 `<!-- 표식 -->`, `## 절` 하나에 기계가 읽는 줄, `> 한 줄 요약`, `## 갱신 기록`,
문서가 색인보다 새로우면 절을 읽어 색인을 맞추는 동기화, 다시 그린 뒤 mtime 도장. 셋이 각자 갖고 있던 사본을
`backend/datastore/tree_doc.py`(173줄) 한 벌로 옮기고 세 모듈에서 221줄을 지웠다(순감 122줄).

| 기질(공통, `tree_doc`) | 기억별(각 모듈에 남는 것) |
|---|---|
| `marker_line/marker_re/read_marker/ensure_marker` | 표식의 이름·속성(`hippo-topic topic` · `memory-node agent,node` · `forage-doc body,root`) |
| `split_section/replace_section/section_body` (anchor 로 자리 지정) | 절의 이름(`## 용례`·`## 관용구`·`## 주행` / `## 기억` / `## 단언`)과 **줄의 문법**(후보의 뜻) |
| `gist_of/skeleton/append_ledger/one_line/meta` | 제목·자리표·추가 머리 줄(`guide:`), 포식의 표 머리 |
| `stamp_value/is_stale` | 도장이 사는 곳(해마 `.rendered` 파일 · 심층 DB meta · 포식 forage_meta) |
| `plan_sync(known, fresh, existing, key, changed)` — 세 갈래 계획 | **식별**(id / (장소·종류·문장)), **변경 판정**, **검증**(IBL 구문·몸 명사·관용구 문장 수), **저장**(SQL·색인·유일 키 순서) |

판단(사용자 우려였던 자리): 동기화의 기억별 식별·삭제·검증 규칙은 콜백과 실행 루프로 각 모듈에 남았다. 기질은 계획만 세운다 —
"색인에 없는 식별자는 새 줄"이라는 규칙 하나만 공통이다(사람이 적은 줄은 잃지 않는다, 세 모듈이 원래 같았다).

동작 차이(정직 기록): 포식 문서의 절 앞 빈 줄 정돈 — 기질은 절 앞을 빈 줄로 띄우므로 옛 포식 문서는 첫 재렌더에 빈 줄 하나가 들어간다.
포식 `parse_section` 은 `## 단언` 부분 문자열이 아니라 제목 줄로 절을 찾는다(산문 속 언급에 안 걸린다). 그 밖은 바이트 동일
(회상 고정물 14/14 동일 — 심층 지도·가이드 목차 렌더 경로 포함).
