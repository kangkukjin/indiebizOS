...
작성: 2026-09-10. 상태: **기본 감독 경로 구현**. 최초 조사 기준 코드: `f629f88f`.
아래 설계·기준선 조사는 구현 전 기록이다. 실제 구현 범위와 차이는 마지막 「구현 기록」이 정본이다.
...
### 4.3 제한된 업무 교환과 GAN 비유의 범위

생성 결과와 평가 피드백이 왕복한다는 점에서는 GAN을 연상할 수 있다.
원래 GAN은 생성기와 판별기를 대립하는 목적함수로 함께 학습하는 방식이다
([Goodfellow 외, 2014](https://arxiv.org/abs/1406.2661)). 여기서 제안한 구조는 같은 사용자 목표를 가진 두 역할이 추론 중 협력한다.
왕복 자체가 두 모델의 가중치를 학습시키지는 않는다. 경험 증류·후속 학습은 별도 경로다.
설계상 참고할 부분은 생성과 검증의 관점을 구분하는 것이며, 실행자가 평가자의 오판을 유도하도록 경쟁시키지는 않는다.

권고는 **책임은 유지하고 필요한 작은 업무만 교환하는 방식**이다. 성능 향상은 비교 실험에서 확인해야 한다.

| 교환 | 허용할 이유 | 경계 |
|---|---|---|
| 실행자가 계획의 오류를 지적하고 대안을 제시 | 실제 작업에서 얻은 증거로 관리자 계획도 교정해야 함 | 자발적 예외 신호로 받으며 정기 보고를 요구하지 않음. 범위·목표 변경은 감독이 기록 |
| 의식이 증거를 직접 확인·검증 | 실행자에게 자료를 다시 읽어 전달하게 하는 왕복을 절약 | 확인하려는 질문과 필요한 증거 범위가 정해져 있어야 함 |
| 의식이 이미 답을 아는 작은 패치를 작성 | 같은 문맥을 실행자에게 다시 설명하는 비용을 절약 | 한 후보 버전에 대한 한정 수정. 연쇄 탐색·새 제작으로 커지면 인계 |
| 실행자가 의식의 실질적인 수정에 대해 검증 근거 제공 | 의식의 직접 수정도 오류일 수 있음 | 로그·기존 검사로 충분하면 추가 호출하지 않고, 필요한 검사만 요청 |

실행자는 의식의 지시와 충돌하는 사실을 숨기거나 계획에 억지로 맞추지 않는다.
의식도 반론에 응답할 때 근거·기준을 제시하고 자신의 규정을 바꿀 수 있어야 한다.
동일 증거로 의견만 반복하는 왕복은 공통 예산에서 제한한다. 사실 확인 가능한 불일치는 도구 검증으로 좁히고,
목표·선호를 정할 정보가 없는 불일치는 사용자에게 미결정 상태를 명확히 전달한다.

의식이 작업을 직접 맡을지는 추가 호출 수만으로 결정하지 않는다.
문맥 전달 비용과 함께 직접 수행 시간, 다른 실행을 감독하지 못하는 시간, 독립된 검증 관점의 손실을 고려한다.
한 가지 질문에 답하거나 이미 확정한 오류를 고치는 일은 직접 할 수 있다.
새 자료 탐색·여러 파일 수정·긴 음성 생성처럼 후속 일이 계속 생기는 작업은 실행자의 책임으로 유지한다.
의식의 활동이 정한 감독 예산을 넘으면 범위를 축소하거나 현재 결과 참조와 함께 실행자에게 넘긴다.
한 과제를 감독하는 동안 계획 없이 두 번째 실행자가 되는 것을 막는 종료 조건을 코드에도 둔다.

같은 하위 작업을 옮길 때는 하네스가 작업 ID, 기존 담당자/새 담당자, 결과 버전, 남은 범위와 예산을 기록한다.
기존 담당자의 실제 쓰기가 멈췄는지 확인한 뒤 인계한다. 프로세스가 여전히 쓰고 있으면 소유권만 먼저 바꾸지 않는다.
기존 산출물·로그·현재 상태의 참조를 전달하며 인계용 장문 보고를 생성하지 않는다.
의식이 실질적으로 수정한 결과도 변경 흔적과 검사 근거를 통해 승인한다. 직접 고쳤다는 사실만으로 자동 합격시키지 않는다.
이를 위해 상시 세 번째 평가 에이전트를 만들 필요는 없다.
...
조사 당시에는 진행 정보만 읽을 수 있었다. 현재의 자동 관찰·개입 통합은 마지막 구현 기록을 참조한다.
...
### 6.4 의식의 활동도 같은 원장에 기록

현재도 의식 호출은 `set_step_role("consciousness")`로 역할을 표시하고,
프로바이더가 제공한 호출 라운드·사용량·지연을 `episode_logger`의 스텝/궤적 기록에 남긴다.
의식이 출력한 규정의 원문과 파싱 결과는 로그에 출력되며, 재규정은 `framing.revised` 사건으로도 남는다.
이는 현재 판단 호출의 기록이다. 확장된 의식의 도구 활동·업무 인계·감독 결정까지 완전히 연결된 것은 아직 설계 단계다.

새 감독은 실행자와 같은 기록 경로를 쓰고 다음 정보를 연결한다.

- 행위자 ID·의식/실행 역할·plan/review/verify 모드·과제/턴/job ID·원인 사건과 시각.
- 호출 시작·종료·도구 인자/결과 참조·오류·산출물 변경·소요 시간·관측 가능한 토큰과 비용.
- 검토한 사건 범위와 증거 참조, 유지/수정/재규정/보류/승인의 판정 및 짧은 판단 근거.
- 지시의 발행·전달·적용·폐기, 작업 담당자 인계, 승인 대상의 결과/기준 버전.

기록은 하네스가 수행한다. 의식에게 작업 뒤에 별도 일지를 작성하게 하지 않는다.
판정에 포함된 짧은 근거를 그대로 보존하고 이미 저장된 본문·이미지·음성을 로그마다 재출력하지 않는다.
숨은 내부 사고 전체를 로그로 확보한다고 가정하지 않는다. 기록의 대상은 관측 가능한 행동·출력·판정 근거다.
스레드·CLI·MCP 경계에도 역할과 과제 신원을 전파하고, 빠진 구간은 관측 누락으로 표시한다.

사용자는 한 과제의 시간선에서 누가 무엇을 했고 왜 바꿨으며 어디에 자원을 썼는지 볼 수 있어야 한다.
실행자에게는 실행에 영향을 주는 최신 지시·수정·근거만 전달한다. 의식의 전체 로그를 매 스텝 읽게 하지 않는다.
자기 로그 기록이 의식을 다시 깨우는 호출 순환은 막되, 의식의 도구 오류·실제 변경은 숨기지 않고 검사와 상태 갱신에 반영한다.
감독 자체의 정체·예산 소진은 공통 코드 감시로 처리하며, 서로를 계속 감시하기 위한 무한한 LLM 호출을 만들지 않는다.
...
역할 교환 실험에서는 의식이 실제 제작에 쓴 시간·인계 횟수·중복 수행·의식 직접 수정의 재교정도 측정한다.
엄격한 역할 분담과 작은 업무 교환 허용을 비교해, 왕복 절약이 감독 지연과 오류 증가를 상쇄하는지 확인한다.
...
- 실행자의 근거 있는 계획 반론, 작은 수정의 직접 처리, 의식 작업의 범위 확대 시 인계, 인계 중 기존 writer 생존, 의식의 실질적 수정에 대한 검증.
...
첫 실사용(ep3332)은 873.7초 동안 계획/점검/검수 각 1회와 승인 원문의 그대로 전달을 확인했지만,
점검 잠금 대기 약 40초, 시작 예고로 인한 stale 판정, 의식 입력 445,329토큰(설정 300,000 초과),
검수의 잘못된 도구 교정·중복 상태 조회와 비용 기록 누락을 드러냈다. 아래 구현은 이 실측 이후의
수리를 반영한다. 기존 에피소드·대화 원문은 변경하지 않았으며, 실사용 절감률은 후속 실행에서 측정한다.

### 적용된 경로

`cognitive_stream`이 실제 agent/task 신원을 가진 일반 턴에서 감독을 연다. 기본값은 활성이다.
THINK/REPAIR의 계획도 공통 `AIAgent(role="consciousness")`로 수행한다. 의식 역할의 프로바이더
인스턴스와 신원은 실행자와 분리하고 CLI resume 기록을 공유하지 않는다. 의식은 `supervision`
작업대에서 원문·도구 스키마를 읽고 기존 실행 도구를 선택해서 사용한다. 별도 인지 파이프라인을
재귀 호출하지 않으며, 계획을 실행 약속으로 오인하는 자동 후속 실행도 의식에는 적용하지 않는다.

| 구현 | 실제 책임 |
|---|---|
| `backend/base/supervision_bus.py` | agent+task별 등록, 실제 API 도구 호출 전후 관측, 인지와 IBL 층의 이음매 |
| `backend/cognition/conscious_supervisor.py` | 감독 수명, 이상 신호·예산, 전제 반박, 실행 경계 지시, 최종 승인·보완·과제 완료 |
| `backend/cognition/supervisor_runtime.py` | 의식 역할 호출·판정 계약·사용량 기록, 변경 블록만 보완하는 실행자 지시 |
| `backend/datastore/supervision_store.py` | 증거 원문, 응답 후보·버전·해시·읽기 범위, 원자적 블록 패치 |
| `backend/base/supervision_watch.py` | 시계와 로그 증분, 살아 있음/진척 분리, 단계·처리량·자원 정보, runner 종료 행 |
| `backend/surface/api_supervision.py`, `mcp_server.py` | CLI와 같은 작업대·행위자 문맥 연결 |
| `backend/base/supervision_hook.py` | Claude Code 네이티브 실행 전 지시 전달 |

정상적인 짧은 조회에는 의식 호출이 추가되지 않는다. 실제 실패·세계 변경·복잡한 작업은
응답 생성 전에 감독으로 승격한다. 연속 실패·같은 결과의 반복·진척 정체·긴 작업에서 중간 검토를
연다. 정상 진척 중에는 경과 시간만으로 중간 점검하지 않는다. 중간 의식은 읽기 전용 관찰이며
실행 경계는 모델 잠금을 기다리지 않는다. 직접 작업은 실행자가 멈춘 계획/최종 단계에서 한다.
도구 시작 예고나 경과 시간 숫자만 변하는 job status 응답은 진척으로 세지 않는다. 의식이 검토하는 동안 실제
작업이 진척됐으면 그 이전 상태의 개입 지시를 버린다. 의식의 읽기·로그는 새 감독을 유발하지 않는다.

실행자가 다음 도구를 호출하려 할 때 미전달 지시가 있으면 그 호출은 실행하지 않고
`not_executed + supervisor_instruction`으로 돌려준다. 지시 전달은 관측된 사실로 기록하며,
실행자가 지시를 이해하거나 성공적으로 적용했다는 뜻으로 기록하지 않는다. 적용 결과는 다음
실제 작업·검수의 증거로 확인한다.

### 응답·완료 계약

감독 대상의 초안 `text/final`은 사용자에게 먼저 내보내지 않는다. 실행자가 작성한 사용자용 본문을
후보 v1로 저장하고, 의식은 그 본문과 실제 산출물을 확인한다. 긴 본문은 블록 페이지로 끝까지
읽어야 한다. 페이지는 프로바이더의 결과 절단 한도 아래로 묶는다. 승인 JSON의 버전·SHA-256과
검수 범위를 코드가 대조한다. 승인하면 저장한 문자열을 그대로 전달한다.

보완은 같은 실행 세션에 인계하되 `patch(version, [{id, hash, text}])`로 바뀐 블록만 제출한다.
버전/해시가 오래됐거나 한 배치에 중복 블록이 있으면 전체 패치를 거부한다. 본문을 고칠 필요가
없으면 `keep`로 확정한다. 수정된 블록은 다시 검수해야 하며 `PATCH_DONE` 같은 내부 종료 신호는
사용자 본문이 되지 않는다. 의식 자신도 실행이 정지된 검수 경계에서 작은 수정과 테스트를 할 수 있다.

빈 판정·오류·누락된 본문·버전 불일치는 `UNKNOWN`이다. 미승인 후보는 미승인 상태 설명과 함께
보존해서 전달하며 성공 경험으로 증류하지 않는다. `pursuit done`은 먼저 완료 요청으로 남긴다.
이번 턴 승인만으로 전체 과제를 닫지 않으며, 전체 goal_criteria 승인과 원장 버전 검사를 통과해야
`done`을 기록한다. 도중에 실행자가 목표를 바꿨더라도 검수 입력에 원래 전체 목표가 함께 남는다.

### 기본 비용 한도와 로그

설정 위치는 `data/world_pulse_config.json`의 선택적 `conscious_supervisor` 객체이며, 빠진 값은
`conscious_supervisor.DEFAULTS`가 채운다. `enabled:false`면 기존 평가·반성 경로로 되돌아간다.

- 코드 감시 주기 5초, 기본 진척 정체 180초, 장시간 점검 480초. 도구가 신고한 단계별
  `stall_after_s`가 있으면 이를 우선한다. 나레이션의 모델 적재와 생성은 서로 다른 기준을 쓴다.
- 중간 검토·실행자 재규정 합계 최대 2회. 자동 중간 검토는 최소 240초 간격이다.
- 의식 호출당 180초 취소 신호, 도구 10회·API 역할 라운드 12회. 전체 도구 40회 중
  최종 검수를 위해 12회를 남긴다. 원격 도구 자체를 강제 종료하는 시간 제한은 아니다.
- 의식 입력 300,000·출력 16,000토큰 상한에서 최종용 입력 80,000·출력 4,000을 남긴다.
  측정된 provider usage로 다음 호출·도구 왕복을 제한한다. Claude Code는 응답 ID별 라운드 사용량을
  중복 없이 중간 반영하고, 취소로 마지막 result가 없어도 관측된 사용량을 원장에 남긴다.
  입력에는 캐시 읽기도 포함된다.
  이미 진행 중인 단일 모델 요청의 초과를 소급해서 막을 수는 없다.
- 보완 최대 2회. 그 뒤에도 미달이면 미승인으로 끝내며 전체 응답을 반복 재생성하지 않는다.

증거와 응답은 `data/spill/supervision/<turn-id>/`, 사건은 같은 폴더의 `events.jsonl`과
기존 trajectory 원장에 남는다. 역할·단계·모델·시간·토큰·증거 ID·판정·인계·응답 지문을 기록한다.
전체 입력 프롬프트를 매번 복사하지 않고 작업대 원문을 참조한다. 상세 사고 과정은 별도 요구하지 않는다.
에피소드의 최종 평가값은 APPROVED→ACHIEVED, REWORK→NOT_ACHIEVED, UNKNOWN→UNKNOWN으로 연결한다.

`cost.json`에는 사용자 턴 경과와 실행/감독의 호출·실패·모델 지연·사용량·폐기·지시 전달을 기록한다.
해마 주행의 IBL 비용 머리는 유지하고 그 뒤에 전체 비용을 붙인다. `avg_ms`는 성공 IBL 도구 지연 합이고
`avg_tokens`는 전체 턴 모델 사용량이므로 둘을 같은 측정 범위로 읽지 않는다.
응답 이후 기억 증류의 시간·토큰은 `postprocess.json`과 `distillation.cost`로 별도 기록한다.

계획에도 전체 IBL 사전을 붙이지 않고 액션 이름 지도와 도구 스키마를 제공한다. 포커스 액션의 계약은
기존 사전/실측 교재에서 주입하고, 그 밖의 액션은 `evidence id=ibl:node:action`으로 읽는다.
상태 재조회는 최초 snapshot 이후의 사건 변경분이다. CLI MCP 결과는 tool_use_id로 대응시켜
`model.native_tool` 중복을 제거한다. 깊은 기억은 같은 기존 ID에 대한 후보들을 한 번에 판정하고,
UPDATE에는 기존에 없는 사실만 적는다. 이미 포함된 사실이나 SAME 판정으로 본문을 갱신하지 않는다.

나레이션 로그에는 단계, 완료/전체 조각, 전체 글자 수, 예상 시간·기준 속도·실측 속도,
GPU 세션, 회수·정리·반납 상태를 함께 남긴다. 감독은 자기 턴의 도구 결과가 가리킨
`script_runs` 로그만 관찰하며, 전체 디스크를 훑거나 status를 LLM으로 반복 호출하지 않는다.

### 현재의 경계와 검증 범위

- API 도구와 MCP IBL에는 실제 실행 전 경계가 있다. Claude Code 네이티브는 PreToolUse 훅으로
  같은 경계를 사용한다. Codex 네이티브 셸/편집은 관찰 가능하지만 실행 전 훅이 없어
  **다음 MCP IBL 경계까지 개입 지시가 기다릴 수 있다**. 이 경우 감독은 실행자가 작업 중인
  동안 직접 도구를 실행하지 않는다. 의식의 Codex 네이티브는 읽기 샌드박스이며 실제 변경은
  공유 작업대와 기존 권한 검사로 수행한다.
- 시작된 원격 작업의 취소·속도 변경은 해당 도구의 기능에 달린다. 이 구현은 GPU 작업을 임의로
  끊거나 자동으로 중복 시작하지 않는다. 백그라운드 작업이 남아 있으면 의식의 직접 변경도 제한한다.
- 감시의 수명은 현재 인지 턴이다. 백엔드 재기동 뒤 독립적으로 감독을 부활시키거나,
  실행자를 영구 대기 상태로 바꿨다가 job 종료만으로 재개하는 지속 실행기는 추가하지 않았다.
  기존 job 원장·완료 알림은 유지된다. 공유 작업대 파일의 장기 보존량 자동 조절도 별도 운영 과제다.
- `backend/test_conscious_supervisor.py`는 실제 AIAgent 및 파이프라인을 연결한 모델 대역으로
  빠른 경로 비용, 원문 보존, 장문 뒤쪽 검수, 패치/CAS, 침묵 중 기동, stale 판정 폐기,
  단일 작성자, MCP 신원 복원, 과제 완료 권한, CLI 프로필을 검사한다. 전체 backend 회귀와
  어휘·층·몸 번들 관문도 수행한다. 실제 유료 모델·장시간 GPU 작업으로 비용/품질 개선을
  재측정한 결과는 아직 없으며, 위 기본 한도는 운영 로그로 조정할 출발점이다.
