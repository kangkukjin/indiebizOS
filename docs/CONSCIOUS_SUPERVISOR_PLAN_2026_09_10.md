...
진척은 하네스가 직접 확인한 증거와 실행자 보고를 구분해 저장한다.
구조적으로 판정 가능한 것은 코드로 계산하고, 의미가 모호하면 “정체 확정” 대신 “점검 필요”로 다룬다.
새 검색 결과의 해시가 바뀌었다는 이유만으로 목표에 가까워졌다고 단정하지 않는다.
실행자는 자연스러운 경계에서 `이정표 / 확보한 근거 / 다음 행동`만 짧게 기록한다.
매번 반성문을 만들거나 이를 요약하기 위한 별도 LLM을 돌리지 않는다.
...
중간 review에는 다음 패킷만 우선 보낸다.

1. 원래 사용자 목표·제약, 현재 규정 버전, 이번 단계의 달성 기준.
2. 충족/미충족/미확인 기준표와 이를 뒷받침하는 증거 ID.
3. 마지막 점검 뒤의 호출·결과 변화, 반복 실패와 모순의 발췌.
4. 새 산출물 참조, 현재 실행의 다음 행동, 소비한 예산.
...
- 실행자는 완료를 **제안**한다. 감독이 근거와 결과 버전에 결부된 승인 사건을 기록한 뒤 완료된다.
