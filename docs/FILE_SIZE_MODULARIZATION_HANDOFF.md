...
> 2026-07-17~18 세션. 상위 2개 완료, 잔여 11개 대기열.
> 다음 세션은 §5 대기열의 ①(build_ibl_nodes.py)부터 §3 방법론 그대로 진행하면 된다.
...
| 순위 | 파일 | 줄수 | 메모 |
|---|---|---|---|
| ① | `scripts/build_ibl_nodes.py` | 2772 | 빌드 스크립트(오프라인)라 위험 최소. 가드 함수 단위로 자연 분할. `--check` 자체가 회귀 테스트 |
| ② | `frontend/src/components/Launcher.tsx` | 1596 | 런처 3표면 토글 |
