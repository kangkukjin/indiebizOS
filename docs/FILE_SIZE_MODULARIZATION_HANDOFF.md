...
### 2-10. LectureWorkspace.tsx 2706줄 → 4파일 (2026-07-18)

| 파일 | 내용 | 줄수 |
|---|---|---|
| `lecture/editor.tsx` | SlideSpecEditor(스펙 폼)·SlideCard·AppendSlideButton + 필드 상수. **isBakedImageSlide 정의처**(본체 재수입) | 585 |
| `lecture/panels.tsx` | MaterialsPanel(재료·자동저장 노트)·DeckPanel(덱·삽입 거터·일괄생성) — SlideCard 는 editor 에서 | 777 |
| `lecture/chat.tsx` | AIChatPanel·LayoutSelect·ChatBubble + **formatDate**(본체 선택화면도 사용 — export) | 475 |
| `LectureWorkspace.tsx` (본체) | 본체·WorkspaceBody·선택화면·생성폼·헤더·내보내기·프리뷰 모달 | 909 |

- 상호참조 매트릭스(정규식)로 3클러스터가 자기완결임을 먼저 증명 — 의존은
  본체→(editor·panels·chat), panels→editor 단방향뿐.
- ★★**tsc 명령 함정(frontend 검증의 구멍)**: 루트 `tsconfig.json` 이 `files: []`+
  references 구조라 **`npx tsc --noEmit` 은 아무것도 검사하지 않고 0 종료**한다(공허 통과).
  실제 검사는 `npx tsc -p tsconfig.app.json --noEmit`. 이 세션의 이전 프론트 분할
  (Launcher·ForageBrowser·GenericInstrument)도 공허 명령으로 "통과"했었으나, 진짜
  명령으로 소급 전수 검사한 결과 신규 오류 0(기존 8건 = NewspaperInstrument 1 +
  types/index 5 + 이번 미사용 import 2 → 후자 2건 즉시 수정) — 소급 무결 확인.
- 실제로 밟은 회귀: `useRef<T>(null)` 제네릭 호출이 import 산출 정규식(`useRef\(`)에
  안 걸려 chat.tsx 에서 누락 → 렌더 시 ReferenceError(빈 화면). HEAD 판 stash 대조로
  회귀임을 판별 → window error 리스너로 스택 캡처 → 1줄 수정. 훅 검사는 **단어 경계**로.
- 검증: tsc(app) 신규 0 · 실브라우저(#/lecture-workspace — 선택화면 formatDate·강의
  열기 → 재료/덱 9장/AI 채팅 전부 렌더·슬라이드 클릭 편집모드 토글, 신규 콘솔 에러 0,
  HEAD 렌더와 동일 화면 대조).

### 2-11. indienet.py 1781줄 → 5파일 (2026-07-18)

| 파일 | 내용 | 줄수 |
|---|---|---|
| `backend/indienet_common.py` | 리프: pynostr/websocket 감지(HAS_NOSTR·더미 타입)·**_ON_PHONE(INDIEBIZ_PROFILE 분기 정의처)**·경로/릴레이/태그 상수 | 61 |
| `backend/indienet_publish.py` | PublishMixin: 보드 CRUD·글/프로필/아티클/소개 게시 | 467 |
| `backend/indienet_relay.py` | RelayMixin: _publish_event/_query_relays(웹소켓 fan-out)·글/DM 영구 캐시 | 418 |
| `backend/indienet_social.py` | SocialMixin: 팔로우·저자 피드/프로필·DM(NIP-04/17) | 574 |
| `backend/indienet.py` (본체) | Identity·Settings·IndieNet 코어(__init__~get_status)+**3믹스인 합성**+싱글턴+재수출 | 355 |

- agent_cognitive 선례 그대로(믹스인 합성 — `self.` 호출 전부 무변경). 상수·pynostr
  더미는 리프(indienet_common)로 내려 본체·믹스인이 공유(순환 0).
- ★포크-가드 allowlist 이주: INDIEBIZ_PROFILE 문자열이 indienet_common 으로 —
  `PROFILE_BRANCH_ALLOWLIST` 의 indienet.py 항목을 indienet_common.py 로 교체
  (믹스인·본체는 `_ON_PHONE` 이름만 참조라 가드-클린).
- ★AST 검사 사각 실사례: 싱글턴 전역(`_indienet_instance`)이 social 조각에 딸려가고
  본체 get_indienet 은 `global` 선언 때문에 AST 미해소 검사에 안 걸림(Store 취급) —
  **모듈 전역 + `global` 조합은 조각 경계에서 손으로 확인**할 것.
- 검증: py_compile · AST 미해소 0 · IndieNet 메서드 파리티 47/47 + 모듈 심볼 파리티 ·
  싱글턴 실기동(실제 identity 로드, MRO 확인) · 라이브 /ibl/execute `[others:feed]`
  릴레이 실왕복(실글 3건 — RelayMixin fan-out 종단) · --check(포크-가드 10개 유지) ·
  body bundle 재파생.

### 2-12. 같은 세션의 별개 수정 (참고)
...
- ~~**`self:read` start/end 별칭 흡수**~~ ✅완료(2026-07-18, `a19b3a2` — aliases: offset←start + read_op 초입 end→limit 계산 흡수, 라이브 4종 검증). 원문: 모델이 30% 확률로 `start`/`end`를 쓰는데 핸들러는
