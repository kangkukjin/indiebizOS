...
## §11 거친 영토 지도(territory) — go/skip 런타임 파생 + 상시-on 상한 (2026-06-21)

**문제(실로그 진단)**: "세바스찬 승 찾기" 질의에서 회상된 `map`이 **0건**이었다. 회상의 map 층은 query 필터(양성 키워드 관련성)인데, *인명* 질의는 "책들=소설"·"vault=학술제외" 같은 *장소 설명*과 키워드가 안 겹친다. 그래서 grep이 `책들`(소설 5000편)까지 통째로 긁어 **이승종 오탐 + 162초**가 났다. 즉 "알지만 안 쓴" 게 아니라 **배제 지식이 회상조차 안 됐다**.

**근본 통찰(사용자)**: `책들`은 *절대* dead_branch가 아니다 — 소설을 찾으면 *타겟*이다. **"죽었다"는 (장소 × 의도)의 관계이지 장소의 속성이 아니다.** `#28 vault=학술제외`는 `#29 vault=에세이`(정체)의 *그림자*일 뿐(의도=학술일 때만). 그림자를 저장하지 말고, **중립 정체(identity)만 띄우고 go/skip은 의도와 곱해 런타임 파생**하라.

**설계**: 2개 상시-on 층으로 정리.
- **owner**(몸 독립) = 누구 냄새. 모든 몸(웹 포함) 덮음. (기존)
- **territory**(열거가능 공간) = 영토 윤곽 냄새. "내가 무엇을 가졌나". **신규**.

**웹과의 관계(collapse 보존)**: territory는 *열거가능 공간*의 속성이라 디스크·코드만 갖는다. 웹은 영토가 없어(비열거) **자동으로 빠진다** — `if 웹` 코드 0, §9 열거-이진 위에 탐. 웹 map은 *항행 관습*(질의-형태적, "저자명 단독=동명이인")이라 query 필터로 잘 떠 상시-on 불필요. **디스크 영토 지도 = "내 것"의 경계 = 디스크-vs-웹 라우팅 게이트**(어느 영토와도 안 맞으면 웹/밖). 웹 = 여집합.

**구현**(`forage_memory.py`, 인지층→재시작 후 라이브):
- 스키마 `forage_map.territory INTEGER`(+ `_connect` 마이그레이션 ALTER). `note_map(territory=)` (한번 켜지면 유지, 끄기는 consolidation/forget).
- `recall()`: `territory` 리스트 = territory=1 행, **query 면제(상시-on)**, 단 질의가 *지명*하면 territory 냄새 생략→`map` 상세로. **하드 상한 `_TERRITORY_CAP=10`** + `_short()`(claim ≤64자) → 프롬프트 무한정 증가 차단(상위 confidence만 노출).
- `recall_xml()`: `<territory>` 블록 + go/skip 파생 note("의도와 맞는 가지 파고 안 맞으면 건너뛰고 어느것과도 안 맞으면 웹/밖").

**상한 보장**: 상시-on 기여분 = territory(≤10 짧은 줄) + owner(≤limit, 작음). 영토가 수백 개 쌓여도 상위 10만 냄새로 노출. 나머지는 query 필터로 강등.

**검증**: "Seung" 질의(영토 미지명)→영토 5건 냄새+owner / "소설 책" 질의(책들 지명)→책들 상세(`<locus>` []관습)+나머지 4 냄새(중복0) / cap=3→3건 / py_compile OK. 영토 앵커 5: 책들·vault·집필·임시·total/work2(깊은 #26·27·34는 상세 유지).
