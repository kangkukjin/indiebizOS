# IBL 전면 감사·보수 핸드오프 (2026-08-05) — **전 항 완결 (물리 이동 포함)**

> 정본. 메모리 `ibl-audit-repair-2026-08` 이 여기를 가리킨다.

## ▶ START HERE — 감사 부채 9항 + 물리 이동까지 전부 완료

**물리 이동 완료 — 2026-08-05 8차 세션 후반, 커밋 `d143b6a`.** backend 195모듈이
층 디렉토리(`backend/{base,datastore,ibl,cognition,services,surface}/`)로 이사했다.
★설계: **모듈 이름은 평면 유지**(41 패키지 bare import·폰 zip 평면·코퍼스 호환) —
`boot_paths.py` 가 층 디렉토리를 sys.path 에 등재, 디렉토리=층 선언이고
`check_backend_layers` 가 위치=선언 일치까지 검사한다. "data" 층의 폴더명은
런타임 `backend/data/` 와 충돌해 **datastore**.

**새 모듈을 만들 때**: 층 디렉토리 안에 만들고 check_backend_layers.py LAYERS 에
배정(가드가 미배정·오배치 둘 다 차단). 진입점(스크립트 등)이 backend 를 sys.path 에
넣으면 곧바로 `import boot_paths`. 폰 번들 engine_modules 는 backend 상대경로
(gradle 이 그대로 소비, zip 은 평면화). ⏳잔여: 폰 실기 재빌드(setup_phone.sh --build)
확인 1회.

---

**⑦ 후반부(인지 매듭 해체) — 2026-08-05 8차 세션, 커밋 `95a2bf1`~`abcc964`.**
...
**(8차 당시의 결정 기록 — 사용자 결정으로 같은 날 실행 완료):**
...
| 물리 이동 | ✅ 완료(8차 후반) | `d143b6a` — 195모듈 git mv + __file__ 산식 75파일 기계변환(4패턴·비정형 0) + boot_paths/conftest + 스캐너 재귀 6종 + allowlist 18경로 + ★자기수정 안전장치 경로 동행(selftest 24/24) + 폰 매니페스트 상대경로화. 검증=compileall·스모크·pytest 55·17가드·전 스위트·라이브 종단. ⏳폰 실기 재빌드. ①에서 안 특이점: 에러 우선순위 미세 역전 3건(phone_listen 등) 별건 |
...
| 8차 후반 | 물리 이동: 층=디렉토리 (195모듈, 평면 이름+boot_paths) | `d143b6a` |
