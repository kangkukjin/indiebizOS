...
### §9-5. 검증 회차에서 발견·수정 2건 (2026-07-19)

전체 재검증(정적 --check · 건강검진 · 오늘 새벽 실전 스케줄 4건 가동 확인)에서 발견:

1. **`;` 경계의 `_prev_result` 단절이 성공 경로에 없었다.** §9 는 "경계를 넘지 않는다"고
   선언했지만 실행기는 **실패 시에만** 리셋했다 — 앞 문장이 성공하면 `_auto_inject_prev` 가
   그 결과를 다음 문장 첫 step 의 `params._prev_result` 로 무조건 주입. `[A] ; [table:take]{}`
   처럼 뒤 문장 첫 step 이 prev 자동소비형이면 독립이 깨진다. §1C-2 의 `;` 테스트가
...
곁다리 확인: 오늘 새벽 첫 실전 가동 — 04:20 보고서·06:30 블로그·07:30 신문 전부 발행 성공,
실패 알림 0. 단 신문은 소스(`newspaper_current.md`)가 어제 그대로라 **옛 판 재발행** —
§9-1 판단 2("발행 버튼 트리거") 그대로 유효.
...
### §11-3. M3·M4·M5 (2026-08-22, 같은 날 후속) — 전부 집행
- **M3** `[try]{…} [catch]{…} [finally]{…}`(`_parse_try_block`/`ibl_control_blocks._execute_try` — 실행기 3종은 형제 모듈 `ibl_control_blocks.py`, 폴백 실행기는 `workflow_fallback.py` 로 분리[1500줄 규칙], catch·finally 안 `$error.error/.step/.node/.action/.summary` 치환, catch 도 실패=두 오류 동봉, finally=결과 불변) · `[on_error: stop|skip|null]` 문장 접두(파서가 그 문장의 step 에 `_on_error` 스탬프, `_handle_failure` 가 skip=직전 통화·null=빈 items 로 계속 + 봉투 `skipped_steps`·`warning`) · `??` 가지 괄호 파이프(`_parse_paren_branch` 재사용, `_execute_fallback` 이 가지를 서브파이프로).
- **M4** `[repeat: N | until 조건 | while 조건, max(필수), every, collect, as]{…}`(`_parse_repeat_block`/`_execute_repeat`): until=몸 뒤 평가(`parse_with_vars` 가 몸통 `$변수`→step 인덱스 `body_vars` 를 적고 실행기가 회차 결과로 바인딩), while=앞 평가(바깥 `_var_values`), `$i` 치환, `halted: max|wall|budget|error|condition_error` 정직 신고, every ≤60s·전체 ≤300s·max ≤1000·하위 step 500. each 에 `collect:true`(회차 `_result.items` 평탄).
- **M5** `[table:reduce]{init, step, as}` 코어 src(`router: system`, `func: table_reduce`, `common/safe_expr.py` 화이트리스트 — compute 와 같은 함수 집합; 상태 dict·분기는 명시 거절) · 자동 스필 `common/spill.py`(이음매 통화 >200K자 → `data/spill/*.json` + 참조 봉투 `{items:[], ref, _spilled}`, 마지막 step 은 안 함, 소비자 4곳 투명 해소: data-ops `_get_items`·each `_each_input_rows`·`$items` 바인딩·write 싱크; 소유 선언 cache·쓰기 때 24h GC) · 재개(실패 봉투 `resume:{from_step, prev_ref}` — `_handle_failure` 가 직전 통화를 스필; `execute_ibl(code, resume)` 이 그 step 부터 `context._prev_result` 로, 앞 단 `$변수` 참조 남으면 정직 거절, `results[].step` 원 번호).
