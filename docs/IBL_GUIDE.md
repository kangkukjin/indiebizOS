...
| 연산자 | 이름 | 의미 | 예 |
|---|---|---|---|
| `>>` | Sequential | 앞 결과를 뒤로 자동 전달 | `[sense:search_news]{query:"경제"} >> [engines:take]{n:3}` |
...
대표 액션 맛보기:
- sense: `stock` `weather` `search_naver` `search_news` `paper` `legal` `realty` `kosis` `world_bank` `restaurant` `travel` `radio` `cctv`
...
예: `[sense:search_naver]{query:"AI"} >> [engines:filter]{where:"title != "} >> [engines:take]{n:3}`
