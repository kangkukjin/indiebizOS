...
**단일 통화**: `items` = `[{…열린 dict…}]`. 가장 흔한 관습은 카드(`{title,meta,summary,url,image?}` → filter/take/document/newspaper/cards)이지만 *title조차 보장 아님*(열린 항목). 옛 table(수치/시계열)=items **행 dict**로(첫 키=x축, 나머지=수치 시리즈 → 소비자 `_get_table`/chart가 키 순서로 table 재구성). 옛 document=items **type+text 항목**(소비자 table:document가 감지). 즉 생산자는 items 하나만 내고, *소비자가 자기 view로 재구성*한다.
...
python scripts/consciousness_schema_check.py   # 의식 출력 스키마 이름 드리프트
```
`--check`: 삼각(src↔tool.json↔handler `_OP_DISPATCHERS`)·op enum·코퍼스 param·app 블록·transform 태그·폰 매니페스트 정합.
...
```
[sense:search_naver]{query:"AI"} >> [table:filter]{where:"title != "} >> [table:take]{n:3}
[sense:world_bank]{indicator:"GDP",country:"한국"} >> [table:chart]{chart_type:"line"}
```
