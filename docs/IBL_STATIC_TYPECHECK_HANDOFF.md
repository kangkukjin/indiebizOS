...
## 2026-09-08 직접 통화 입력의 검사 일치

`flow.input_params`는 파이프 대신 직접 공급할 통화 슬롯 목록이다. 모든 슬롯에 non-null 값이 있으면 파이프보다 우선하고, 둘 이상이면 입력 묶음으로 타입 검사한다. `join`은 `[left, right]`를 선언한다. T1 머리·T2 이음매 기아와 열/종류 검사가 같은 선언을 읽는다. 한쪽 누락은 면제하지 않으며 미상 참조는 기존의 보수적 판정을 유지한다. 액션 이름을 검사기에 하드코딩하지 않는다. 회귀: `backend/test_direct_currency_inputs_2026_09_08.py`.

### 리터럴 식 사전 검사 (2026-09-08)
`flow.scalar_expr_params`는 런타임이 첫 비어 있지 않은 값을 고르는 식 슬롯의 순서다.
검사기는 액션 이름을 모르며 선언된 슬롯의 문자열 또는 매핑 값을 `common.safe_expr.compile_expr`로만 검수한다.
변수·자리표가 있는 식은 실행 때 결정하므로 유보한다. 실제 값의 계산·0 나눗셈·열별 자료 품질은 여전히 실행 검증이다.
이 관문은 보고서 끝의 dict 생성 오류 때문에 이미 끝난 수집까지 다시 시작하는 일을 방지한다.

### 필터 조건 검증 공유 (2026-09-21, episode 3942)

`flow.row_condition_params`는 첫 비어 있지 않은 조건 슬롯의 우선순위를 선언한다
(filter: `[where, condition]`). 검사기는 `common.row_conditions.validate_where`로
조건 문법·정규식·참조 열을 읽는다. 액션 이름별 분기는 없다. 실행도 행 순회 전에
같은 검증을 거치므로 빈 입력·논리식 단락 평가·context 선택이 오류를 숨기지 않는다.
`title ~= ...`는 미지원 연산자이며 `{field:"title",op:"matches",value:"패턴"}`으로
쓴다. 동적 조건은 바인딩 후 검증하며 값 타입 비교는 실제 행에서 수행한다.
회귀: `backend/test_filter_precheck_2026_09_21.py`.

공통 모듈 변경에 따라 record-ops 의존 지문도 재감사했다. 관리 기록 조회는
객체/AND 목록만 허용하고 `_match`의 기존 객체 비교를 사용한다. 접근 정책·필드
권한 검사와 값 비교 구현은 유지되며 관리 기록/API 회귀를 함께 확인한다.
