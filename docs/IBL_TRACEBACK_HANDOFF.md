...
## 6. 다음 단계 (품질 축) — ✅ 2026-08-27 집행됨

`criteria` 품질 계약으로 같은 날 집행 — 정본 = `docs/IBL_QUALITY_CONTRACT_HANDOFF.md`
(단계별 계약=criteria param, error_type "quality" 사용 개시, goal 라운드 배선 포함.
작성 시점 린트는 교재 규범으로 먼저, 정적 검수 편입과 증류 셋째 신호는 실사용 관찰 후).
