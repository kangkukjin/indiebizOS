# 방법의 지도 — 작은 어휘로 실행 범위를 넓히는 구현 설계

상태: **키워드·별칭 기반 첫 구현 완료**. 설계 기준 코드: `06737045`. 사용자와 연구 에이전트의 지식 지도 논의 및 현재 세션의 구체화를 반영한다. 현재 구현·운영 설정·검증 결과는 §10에 기록한다. §4의 임베딩 공유·벡터/RRF는 조건부 후속 설계이며 아직 구현하지 않았다.

## 1. 구현할 것

이번 일을 할 수 있는 도구·방법의 **이름과 짧은 설명 0~4개**를 골라, 본격적인 사고를 시작하기 전에 프롬프트에 넣는다.
**IBL의 어휘는 도구를 가리키고 문장은 도구를 조합해 일을 한다. 방법의 지도는 더 많은 도구·방법의 이름을
공급해 실제로 선택할 수 있는 수단을 넓힌다.** 모델은 이름에서 사용법·적용 조건을 떠올리고 실행 통로를 연결한다.
도서관 카탈로그라는 비유에서 중요한 것은 이번 일을 해낼 방법의 이름을 찾아 주는 기능이다.

정식 개념명은 **방법의 지도**로 둔다. 현재 자료의 대부분은 도구이며, 이름 붙은 분석법·알고리즘·설계 방법도
같은 지도에 들어갈 수 있다. 일반 주제의 관련성보다 **그 이름이 이번 과제에 쓸 수단을 제시하는가**를 기준으로 편집·평가한다.
현재 어휘 검색기가 이 적합성을 완전히 판정한다고 가정하지 않는다.
편집 정본: [방법 지도 편집 기준](../data/knowledge_catalog/README.md).
기존 `knowledge_catalog` 설정·사건·파일 경로는 호환성을 유지하고, 모델에 주는 블록은 `<method_map>`으로 부른다.
...
우선 이 자료만으로 유용한 방법의 이름이 나오는지 확인한다. 이름 붙은 분석법·알고리즘·사고 방법도 확장할 수 있지만,
...
심층기억·포식기억의 개인 사실 자동 주입을 다시 켜는 작업도 아니다. 이 카탈로그는 일을 할 도구·방법의 목록이다.
...
```xml
<method_map>
방법의 지도: 이번 일에 쓸 수 있는 도구·방법의 이름입니다. 적합한 것만 활용하세요. 도구의 설치·사용 가능 여부는 별도 확인이 필요합니다.
...
주요 판정은 **이번 일을 해낼 적절한 방법이 선택지에 더해지고 실제 결과가 나아졌는가**다. 이름을 인용했거나
...
`anatomy.md`·`memory.md`에는 방법의 지도와 개인 기억의 구분 및 실제 주입 위치를 기록한다.
현재 설계 문서를 추가하는 단계에서는 기존 시스템이 이미 이 기능을 제공한다고 문서를 고치지 않는다.

## 10. 첫 구현과 운영 (2026-09-16)

### 반영한 범위

- 기존 지도·조사본에서 **66항목**을 정리했다. 분류·세계의 명사는 YAML에만 있으며 새 IBL 낱말은 없다.
- `knowledge_catalog.py`는 스키마·출처 경로 검증, 내용 지문을 가진 불변 스냅샷, SQLite FTS5 빌드·검사,
  이름·별칭·설명·분류의 어휘 검색을 소유한다. 이름·별칭을 우대하고 설명의 일반 단어 하나만으로 선택하지 않는다.
  명확한 후보가 있으면 일반 설명 단어만 맞는 약한 후보로 자리를 채우지 않는다. 분류명은 순위에만 보탠다.
  한글 띄어쓰기·조사와 영어 대소문자를 정규화한다. FTS 순위는 동점 후보 정렬에만 쓴다.
- 질문 앞에서 모델을 불러오거나 다운로드하지 않는다. 초기 16개 표현 시험을 통과해 임베딩 분리는 미뤘다.
  어휘로 연결되지 않는 바꿔 말하기까지 이해한다고 보장하지 않는다. `semantic_enabled=true`도 현재는
  어휘 검색을 유지하며 사건에 `semantic=unavailable`을 남긴다.
- 보조 질의는 짧은 지시어 후속 발화에서만 사용하며, 현재 질의 후보가 있으면 사용하지 않는다.
  이전 사용자 발화 최대 2개·400자, 현재 질의가 8,000자를 넘으면 앞·뒤 각 4,000자로 검색 범위를 제한한다.
  원래 사용자 메시지 자체는 바꾸지 않는다. 이 제한 때문에 긴 본문 가운데만 있는 주제는 놓칠 수 있다.
- `catalog_recall.py`는 적용 범위·600자 렌더·궤적을 소유한다. 공통 파이프라인의 과제 연결 뒤,
  의식 호출 전에 한 번 선택한다. `execution_memory`를 통해 의식과 실행자가 같은 조각을 받는다.
  프롬프트 재조립은 그 조각을 재사용하고 안정 프롬프트·해마 점수는 그대로 유지한다.
- 색인이 없거나 손상·구버전이면 현재 스냅샷의 어휘 검색(`lexical_fallback`)으로 진행한다.
  정본·설정 오류는 주입을 생략하고 `error`를 남긴다. 취소는 기존 턴 중단 경로로 전달한다.
- 원본 파일 변경은 `--check`가 지문으로 검출한다. 원본을 다시 읽어 카탈로그를 자동 수정하지 않는다.
  검토 후 명시적으로 빌드해야 새 원본 지문이 기록된다.

### 설정과 유지보수

정본 맥의 런타임 설정에만 다음을 적용했다. 저장소에 개인 설정 파일을 추가하지 않는다.

```json
"knowledge_catalog": {
  "enabled": true,
  "enabled_agents": ["study:agent_001"],
  "max_items": 4,
  "max_chars": 600,
...
미설정은 꺼짐이다. 다른 에이전트를 켜려면 표시 이름 대신 실행 키를 추가한다.
`enabled_agents` 생략은 주인 신원의 공통 대상, `[]`는 모두 제외다.
설정은 매 턴 읽으므로 `enabled=false`가 다음 턴부터 적용된다.
실행 중에는 기존 `/world-pulse/config` GET→수정→PUT 경로로 다른 설정을 보존해 저장한다.

```bash
.venv/bin/python3 scripts/build_knowledge_catalog.py          # 검토한 정본의 로컬 FTS 색인 생성
.venv/bin/python3 scripts/build_knowledge_catalog.py --check  # 정본·출처·색인 일치
.venv/bin/python3 scripts/build_knowledge_catalog.py --check --query '교대에 필요한 인원을 배정해'
.venv/bin/python3 -m pytest backend/test_knowledge_catalog.py -q -o addopts=''
```

새 항목은 YAML 편집→출처 확인→빌드·검사 순서로 추가한다. 파생 DB는 git 밖에 둔다.
`knowledge_catalog.selected` 사건은 꺼짐·대상 제외·무관 결과·예산 부족·오류·선택을 구별하고
선택 ID·리비전·주/보조 질의·문자 수·생략 이유·지연을 담는다. 사용자 질문 원문을 중복 기록하지 않는다.

### 첫 구현 당시 검증 결과와 남은 판단

- 전용 회귀 **45개 통과**: 이름 없는 질문 16개, 무관 질문 7개, 한글 조사·띄어쓰기,
  XML 이스케이프·문자 예산, 후속 질문·주제 전환, 외부 주체·설정 변경, 손상·낡은 색인,
  잘못된 정본, 동시 대화, 취소, 실제 THINK/EXECUTE/REPAIR 의식·실행 전달, 일반 단어로 인한 동반 오탐을 검사했다.
- 전체 백엔드 검사: **4,900 통과·1 생략·1 실패**(457.95초). 실패는 새 색인 빌더의
  `sqlite3.connect`에 명시적 timeout이 빠진 규약 위반이었다. `timeout=10` 보완 뒤
  카탈로그 45개와 동시성 규약 3개를 합친 **48개가 통과**했다. 커밋 관문의 설정 상한 검사도 반영해,
  설정 예산을 제한하면 궤적에 요청값·적용값·clamped를 함께 남기고 회귀로 검증한다. 마지막 후보 문턱 보완·timeout 수정 뒤
  전체 검사를 다시 돌리지는 않았으며, 관련 검사만 재실행했다. 층·IBL 파생 문서·폰 번들·색인 검사도 통과했다.
- 16개 검색 질문은 모두 상위 4항목 안에 허용 집합 중 하나를 포함했다. 주입량 **141~159자**.
  특정 제품 하나를 유일 정답으로 강제하지 않았다. 이것은 작은 검색 시험이며 보편적 재현율 주장이 아니다.
- 정본 맥에서 준비 상태의 스냅샷 조회+검색+렌더 **300회**: p50 **0.605ms**, p95 **0.637ms**,
  최대 **3.597ms**. 15만 자 입력 1회는 **3.054ms**(앞·뒤 제한 적용).
  YAML 첫 파싱·모델 응답·기존 연상·궤적 DB 쓰기는 이 지연 수치에 포함하지 않았다.
  로컬 원시 결과: `outputs/knowledge_catalog_validation_2026_09_16.json`.
- 실제 답변 품질의 무주입/이름/이름+설명 A/B 비교는 아직 하지 않았다.
  추가 모델 호출은 없지만 THINK에서는 같은 조각이 의식과 실행 각각의 입력에 실린다.
  문자 수를 토큰 수로 바꾸어 비용이 측정됐다고 보고하지 않는다.

## 11. 방법의 지도로 개념 정리 (2026-09-16)

사용자 판정에 따라 이름 공급의 목적을 **실행에 쓸 도구·방법의 범위 확대**로 명확히 했다.
모델 주입문의 제목·태그와 정문·기억 문서·도구 가이드를 같은 개념으로 맞췄다.
편집 기준은 `name=정확한 통용 이름`, `hint=가능한 연산·산출물`, `aliases=작업 상황`,
`path=방법을 찾는 분류`, `source=적용 근거`로 구체화했다.
검색 알고리즘·설정·대상 에이전트·최대 4항목/600자 예산은 유지한다.
이는 검색기가 의미를 이해하도록 개선했다는 주장이 아니다. 다음 품질 판단은 제품명을 숨긴 작업 질문에서
유효한 방법이 선택되는지, 유사 주제의 부적합한 도구가 빠지는지, 실제 결과가 나아지는지를 본다.

이번 문구·태그 변경은 기존 카탈로그 회귀 45개와 색인 검사·폰 번들 파생으로 검증했다.
전체 백엔드 회귀는 이 문구 변경 뒤 다시 실행하지 않았다.

## 12. 충실도 감사와 데이터 보완 (2026-09-16)

[고정 과제 감사](METHOD_MAP_AUDIT_2026_09_16.md)로 방법의 등재 여부와 실제 선택을 나눠 측정했다.
32개 방법과 16개 도구를 보완해 114항목이 됐다. 조사·인과·판단·검증·학습 방법의 공백을 메우고,
기존 상세 가이드에 있으나 카탈로그에 빠졌던 기본 도구를 복원했다.
추가 확인 질문 24개에서 유효 후보 선택은 2개에서 20개로 늘었다. 4개 검색 누락은 그대로 공개했다.
개선용 질문은 24/24, 무관 질문은 8/8 생략, 주입량은 149~240자였다.
동일 작성자의 취약 영역 표본이므로 보편적 완성도·실제 작업 성공률로 해석하지 않는다.
항목별 근거·적용 조건·대안은 `docs/world_map/F_methods_and_validation.md`에 둔다.
운영 알고리즘과 4항목/600자 예산은 유지하며, 관련 회귀 95개를 통과했다.
