...
## 6. 재학습 (09-18 집행 — 사용자 지시)
실행기억 다듬기가 끝난 코퍼스(3,635행·학습쌍 8,534)로 해마 재학습. 최적 epoch 7·검증 0.895. 직전 라이브 모델과 동일 분할 A/B 에서 6지표 전부 우세(code Top-5 82.8→88.8, desc Top-1 63.7→71.0) → 채택, epoch 체크포인트 삭제 → 재색인(행 3,635 = 벡터 3,635, 전부터 있던 벡터 없는 23행 해소) → `api.py restart --wait`(ACTIVE). 라이브: 고친 `$위치` 관용이 "근처 밥집"에 1·2위로 회상되고, `$요약.message` 알림 관용이 번역에 쓰인다. 롤백 = `data/models/ibl_embedding.bak.20260918_014001`. 프로브 13개 추가(`cloud_training/compare_models.py`), 이력 = `data/guides/hippocampus_retraining.md` §5.
남은 것: 프로브에서 빠진 3건의 시딩(`sense:feed` 자료원 이름·`self:read` PDF 표·`table:ai` 걸러내기) · 번역기(경량 모델)가 회상된 관용을 무시하고 인자를 지어내는 것(`finance{start_date}`)은 해마 밖의 문제 — 조인 인자 검사가 경고로 잡는다.
