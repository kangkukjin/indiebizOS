...
> **2026-07-28 개정 — 목표 2 의 clipbox 철회, 푸시 큐 원복**: 사용자 판단("원격런처
> 수신은 너무 느리고 불편, LTE 여도 폰네이티브가 받아야") 으로 아래 B(clipbox
> 메시지함+원격런처 수신면)를 전부 걷어내고, '폰으로'를 **원래(c52132a) 방식**으로
> 되돌렸다: `/launcher/clip-to-phone` 전용 라우트가 폰 사전의 [limbs:phone]
> clipboard+notify 고정 봉투를 `ibl_engine._forward_to_phone` 으로 전달 — 직결(Wi-Fi)
> 우선, LTE/CGNAT 면 heartbeat 롱폴 푸시 큐(실측 LTE 6.4초 동기 완료). ★사전 물리
> 분리(6cb85ea) 후 맥 엔진엔 limbs:phone 이 없어 /ibl/execute 경유 불가 — 전용
> 라우트가 엔진 사전 밖에서 고정 봉투를 보낸다(어휘 사용 아님, 리모컨 계약).
> ask 경로는 채택 안 함(HTTP 직결뿐이라 LTE 불가). 우편함 철회는 유지.
> **귀결: 하트비트+푸시큐 레일은 '폰으로'가 다시 승객이라 매장 보류**(사용자 결정).
