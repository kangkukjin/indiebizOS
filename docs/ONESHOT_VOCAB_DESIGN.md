...
**남은 것(코드 밖)**: ~~커밋~~(`d6eb7e0` push) · ~~재학습~~(아래) · A36 폰 실기 확정 · 실사용 증류 관찰(§11-6 — 성패 지표).

**재학습 채택 (2026-08-19 당일)**: M4 로컬, epoch 5 최적(검증 0.878), 코퍼스 3,362. compare 게이트 = aggregate code 전면 우세(T1/3/5 +0.5/+2.0/+1.5p)·desc T1 +0.9p(T5 -0.2p 노이즈), 프로브 38→40(**struct 신규 적중**·해커뉴스 회복)·경계(read·ask) 보존. ★table:ai/brief 순수 임베딩 프로브는 양쪽 공통 미적중이나 라이브 번역은 정확(08-07 교훈: 프로브 실패≠런타임 실패) — 실측: "회의록 사진에서 결정사항"→struct(schema 자가 구성)·"시세 두 문장 보고"→stock&stock>>union>>brief·used/read 회귀 무손상. 백업=`ibl_embedding.bak.20260819_175944`(롤백=rm+mv→rebuild_index→재기동). keeper 표준 절차 준수(keeper_off→.venv 재기동→워밍업→재개).
