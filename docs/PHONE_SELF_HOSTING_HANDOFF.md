...
4. **하네스를 폰 번들로**: `phone-companion/app/build.gradle`의 `_ENGINE_MODULES`에 추가 — `world_pulse_collectors`·`world_pulse_health`·`prompt_builder`·`agent_cognitive`·`consciousness_agent`·`agent_runner`·`goal_evaluator`·`providers/*`·`conversation_db` + `data/common_prompts/*` 에셋. (De-risk 완료: 벤더 SDK 지연 import라 폰 import 안전.) ← **다음 시작점**
