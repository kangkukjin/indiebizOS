...
3. **공유 = 기본 비공유(opt-in)**. 사용자 지정한 것만. 현재 지정 = **business.db 하나**(연락처/이웃 포함). 일정은 동기화 아님(mac_only 빌림).
...
- **신원 단 빌림**: 폰-자아가 mac_only(예 `self:manage_events`) 요청 → `_forward_to_mac`이 맥서 실행하되 *폰-자아 신원*으로 기록.
