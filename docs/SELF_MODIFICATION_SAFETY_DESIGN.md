...
### 적용은 도는 턴이 0 일 때만 — 재기동 관문 (2026-09-02, #repair 절단율 16%)

08-05~09-02 실측: `#repair` 턴 59건 중 12건이 ORPHAN(16%, 전체 턴 8.4%의 두 배)이었다.
원인 넷 중 구조적인 것 하나 — 수행자는 **예약한 턴**만 기다렸다. 그 턴이 닫힌 뒤 사용자가
곧바로 다음 명령을 내리면(ep1916 의 적용이 ep1917 "#repair 수리가 적용된거야?" 를 잘랐다)
수행자는 새 턴을 모른 채 라이브에 썼고 리로드가 **다음 턴**을 잘랐다. "예약한 턴이 닫혔다"는
"아무 턴도 안 돈다"가 아니다. 봉합은 두 겹이다:

- **① 정적 대기(`red_apply.wait_quiescent`)**: 턴 종료·증류 재합류 뒤에 **도는 턴이 0** 이
  될 때까지 더 기다린다. 출처 = `/health` 의 `live_turns`(몸 자신의 신고), 옛 몸이면 원장의
  최근 열린 행(고아 행은 나이로 제외). 그 사이 들어온 새 명령은 정상 처리된다 — 적용이
  양보한다. 상한 600초(`QUIESCE_CAP_S`, 좌초 신고 30분 아래)에 닿으면 강행하되 잘렸을 수
  있는 턴을 **이름으로** `followup.json`(`quiesce_outcome="cap"`, `live_turns_at_cap`)에
  남기고 red_report 가 `<wait outcome="quiesce_cap">` 로 다음 턴에 띄운다. 남의 관문(다른
  수행자가 쓰는 중)도 '도는 것'으로 본다 — 그 리로드 위에 또 쓰지 않는다. 몸이 30초
  이상 안 닿으면 자를 턴이 없다(`no_body`).
- **② 재기동 관문(`backend/base/reload_gate.py`)**: 0 을 본 순간 관문 파일
  (`data/system_ai_state/reload_gate.json`, raised)을 세우고 **되묻는다** — 관문 직후
  들어온 턴이 있으면 관문을 내리고 다시 기다린다. 쓴 뒤엔 written 으로 올려 리로드~부팅
  창을 덮고, **새 몸이 부팅에서 회수한다**(`EpisodeLogger.install → clear_at_boot`; TTL
  60/120초는 폴백 — 관문이 영구화되면 그것이 곧 브릭이므로 TTL 은 헌법이다). 그 창에
  들어온 새 턴은 파이프라인 진입점 `cognitive_stream` 이 **첫 문장**으로 관문을 묻고 일을
  시작하지 않은 채 "재기동 중, 같은 내용을 다시 보내 달라"를 돌려준다. ★옛 몸 안에서
  기다리게 하는 것은 답이 아니다 — 그 기다림은 옛 몸과 함께 죽는다. 되돌려보내는 것이
  유일하게 정직한 선택이다. 사용자 표면은 전부 이 진입점을 지난다(관문
