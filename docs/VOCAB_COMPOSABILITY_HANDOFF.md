...
> **2026-09-08 정정**: B26-1의 조건 선별·중복 제거를 절단으로 취급한 규약은 은퇴했다. 현재는 집합을 다시 정의하면 total을 결과 기수로 갱신하고, take의 표본 신고와 상류의 실제 잘림은 보존한다. B26-2의 summary도 집합 재정의 때 제거한다. 아래는 당시 사건 기록이다. <!-- retired-ok: 개정 전 계약의 역사 기록, 현재 계약은 바로 앞 정정문 -->
...
- **후속 실측 — 실제 일간 보고서 3종의 "한 문장 초안" (2026-08-27 저녁, 사용자 질문)**: 51회차의 "매일 쓰는 보고서"가 가리키던 실물(AI 팁·AI 동향·부동산)로 같은 질문을 다시 실측했다. ①**AI 동향** ✅ 6 step·105초 — `$기준선 = [self:read]{_coverage_ledger.json}` 을 ai instruction 에 주입해 **커버리지 원장과의 겹침 제거까지 문장 안에서** + 3-way 검색 + 3섹션 분류 + group_by document. 한계: union 후 분류라 ai 가 버리면 섹션이 빈다(이번 실측에서 기술·연구 0건) — 섹션 보장이 필요하면 섹션별 `$변수` 가지로. ②**부동산** ✅ 7 step·113초 — `rotation.json 읽기 >> sort{last_visited} >> take{1}` 로 **순회 커서 선출**(세종시 정확), ai 로 지역명 추출, each→molit 실거래 12건, 철학 기준 평가+총평(기존 판정 '보류' 참조까지). 초안 품질이 실전급. 한계: DB 누적·커서 전진 같은 **상태 쓰기**는 문장 밖(발행 후 에이전트 몫). ③**AI 팁** ⚠️ 기술적으론 조립 가능하나 **가이드 헌법이 파이프라인을 명시 금지**("자막을 하나씩 읽고 판단", deep_research 원칙) — 한 문장의 몫은 재료 수집까지. 곁가지 갭 2건(보고만): `table:compute` 는 수치 함수만이라 문자열 가공(split) 불가 — ai 한 칸으로 우회(35초 비용), 어휘 확장 여부는 표준/사전 경계 판단 사안. 검색이 날짜 필터 없이 2~3주 묵은 기사를 물어온다 — days/date 파라미터 조합 표면 점검 후보. 부수: 세 실행 전부 표면 이탈 후 **ticket recover 로 회수**(F51-1 규약 실사용 3건 추가).
- **후속 판정 뒤집기 — "자막을 읽고 판단"도 문장이 된다 (2026-08-27 저녁, 사용자 반문이 촉발)**: 사용자 반문 "원샷 AI 가 어휘에 있는데 왜 안 되나" → 실측하니 **금지가 아니라 결함이었다.** 낱말은 이미 있었다 — `[self:struct]`(본문→schema 레코드, grounded 원문 대조)가 정확히 "자막 증류"의 동사다. 막던 것은 struct 의 파이프 본문 이음매 셋: ①본문 필드 사슬(text/content/summary/message)이 transcript·preview 를 몰랐고 ②transcript 의 외부화 봉투(saved_to_file+file_path, 10,000자 초과)를 안 따라가 **안내 message 를 원문으로 오독**(조용한 품질 실패 — 안내문에서 "기록"을 추출), ③외부화 파일이 `[MM:SS]` 병기 포맷이라 grounded 대조가 구조적으로 전멸(실측: 추출 16건 전원 탈락). 수리(ai-ops/handler.py 한 이음매): 외부화 봉투 따라가기(+실종 파일 폴백)·타임스탬프/헤더 정규화·본문 사슬에 transcript·preview 편입(message=최후 폴백). 실측: `each{후보2} do: "transcript >> [self:struct]{schema: 팁…, grounded: true}"` — 5/5 완주, 두 영상(각 2만자대 자막)에서 **grounded 통과 42행**(팁+실행단계+도구+_quote 원문 발췌), keep 병합으로 출처(제목·채널·URL) 동반. 가드=`test_struct_body_seam.py` S1~S4(★S4 가 실종 파일 미처리 결함을 시험 작성 중 적발 — 폴백 수리). 배터리 1037 passed. ★가이드(youtube_ai_tips_report.md ⚠️파이프라인 금지)는 이 결함 시절의 판정 — **증류 초안 단계는 이제 문장이 된다.** 에이전트 루프에만 남는 몫 = 원장 갱신·재탕 판정(tips DB 대조)·사용자 맥락 함의·영상별 동적 재시도. 가이드 개정 여부 = 방법론이라 사용자 판정 대기.
- **품질 진단 실측 — v1→v2 재실행 대조 (2026-08-27 밤, 사용자 질문 "품질 부족은 문법 탓인가 일처리 탓인가")**: 세 초안을 항목별로 재실행 대조. **판정: 대부분 일처리(단계별 파라미터·지시·조합 선택) 탓 — 문법 개정이 필요한 결함은 없었다.** ①부동산 v2 = `fields` 열 계약+총평을 `[table:brief]` 별도 문장으로 분리 → 표가 발행급으로 정리(None 유령 행 소멸, 중복 열 소멸), 총평 3문장 산문 별도 파일. ②동향 v2 = 섹션별 가지+가지 안 ai(fields)+3일 신선도 컷 → 열은 깨끗해졌으나 **섹션이 더 비었다: `branches_honesty` 가 원인을 정확히 신고** — 투자 가지 rows_in 8 → rows_dropped 8(전원 묵은 기사), 기술 가지(hn days:3) 입력 0. 즉 신선도 문제는 문장이 아니라 **검색 재료**(쿼리당 1검색이 가이드의 "섹션별 3회+" 규정에 미달) — 처방=어휘가 이미 가진 `queries`(배치 팬아웃)·`headlines`·복수 소스로 가지 입력을 넓히기. ③팁 v2 는 전 항 실측(grounded 42행)으로 이미 발행급 재료. **문법·어휘 쪽 소견 3건(별도 판정 후보)**: (a) `$변수 >> [action]` 파이프 머리 불가 — "파싱 실패: $평가", 소비는 param 주입(`items:"$평가"`)만. 인체공학 마찰이며 개정=언어 개정 사안. (b) `[table:compute]` 문자열 함수 부재(수치 화이트리스트만) — 지역명 추출 같은 결정론 가공이 ai 한 칸(비용·비결정)으로 우회됨. 사전 화이트리스트 확장 후보. (c) `[table:document]` 가 `_ai` 등 내부 표식 열을 표에 노출 — emitter 기본 숨김 후보. 산출물: /tmp/draft_probe/{ai_trend_초안2, 부동산초안2, 부동산초안2_총평}.md
- **"초안" 은퇴 — 완성 보고서가 IBL 프로그램으로 (2026-08-27 밤, 사용자 방향 제시)**: "파이썬이면 몇백 줄도 짜는데 IBL 로 좋은 보고서를 쓸 수 있어야지. 한계면 IBL 을 고치고." 부동산 보고서를 **13문장 완성 프로그램**으로 실측 — 순회 커서 선출 → molit 12건 → 철학 평가(fields) → 지역 뉴스 가지 → 가격 지형 분석·총평(brief 2편) → **blocks 혼합 문서**(heading+산문 paragraph+평가 table+뉴스 cards) → 원장 갱신([self:edit], 사본 실증). 13/13 완주 ~100초, 산출물 품질이 실 보고서 수준(가격대 3층 구조 서사·후보 우선순위·현장 확인 항목·기존 판정 참조). **판정: 한계는 언어가 아니라 조합 노력** — "초안"이라는 개념 자체가 보수적 프레임이었다. 조립이 적발·수리한 것: **B52** blocks 구조 필드의 `$변수` 주입이 JSON 문자열로 들어가 표가 문자 단위로 쪼개짐 → `coerce_json_param` 범용 게이트+doc_build 되읽기(가드 P31, `2dc1c1d9`). 새 트레이스백이 첫 실전 값(binding 오류에 사용 가능 필드 열거). **판정 후보 3건(언어·사전)**: ①`$변수 >> [액션]` 파이프 머리 불가("파싱 실패") — 개정=언어 개정 사안 ②고전 변환자(select/sort/filter/dedup/groupby/compute/union/flatten)는 items **리터럴 거절**(씨앗=take 단일 진입 설계)이면서 `$변수` 문자열은 수용 — 의도된 비대칭인지 설계 확인 필요 ③`[table:compute]` 문자열 함수 부재. 남은 에이전트 몫=조합 자체(어느 가지·어느 지시), 실패 시 재구성, 그리고 §품질은 원샷 지시문의 질에 비례 — 품질 계약(traceback error_type=quality 예약)이 다음 이음매.

### 팁 보고서 완성 프로그램 1회차 — v1(16문장)→v2(23문장), 수리 2건 (2026-08-28, 하네스 세션)
- **결과**: `IBL_REPORT_PROGRAMS_HANDOFF.md` §2 루프의 첫 집행. AI 팁 보고서를 검색(3-way &)→원장 안티조인→info 팬아웃→6개월 술어→심사(verdict 보존)→transcript>>struct(grounded) 증류→절제(재탕 대조 포함)→brief 5편→blocks 조판→원장 3종 델타+edit 사본 실증까지 **23문장 55step 완주**. v2 산출물이 가이드 골격·머리줄 집계(§3 정의)·재탕 주의·타임스탬프·자막 실패 정직 표기까지 충족 — 실전 최근호(08-27 코딩호) 대비 남은 갭은 소재 수준(그날 검색 풀)과 도입 서사 정도. 산출물/프로그램: 스크래치 `tips_v1/`(exec_body_v2.json·ai_tips_report_2026-08-28_자동화_v2.md).
- **수리 ①** `where` 부정 멤버십 부재(`fb6eacb3`): api_transforms 는 not_in/not_contains 를 팔면서 where 는 정직 거절만 — 원장 제외(id 357건)를 술어로 못 썼다. _OPS 추가(value_semantics 한 벌·결측 좌변 부정 불주장)+_WORD_OPS 밑줄 워드(문자열형 침묵 강등 차단). 가드 N1~N8(동형성 시험 포함).
- **수리 ②** 봉투 다이어트의 errors 침묵 모순(`a5a8a700`): each 부분 실패(자막 1/4)의 errors[] 가 다문장 요약에서 통째로 접히고 message 만 "errors 참조" — 회수 불능. summarize_result 가 다이제스트(3건·300자 상한+절단 신고)로 나른다. 가드 E1~E4.
- **관찰(수리 아님·처방 있음)**: ①groupby 봉투는 items 미러가 없다(반환 계약=table) — 값 주입용 사슬은 compute 로 끝내라(`${x.items.0.f}`). ②한글 단위가 경로에 먹힌다("$엠.items.0.M개"→경로 ".…M개") — 괄호형 `${…}` 처방이 이미 언어에 있다(workflow 시그니처 조항). ③다문장 프로그램엔 resume 이 없다 — 대신 **[self:write] 사본(items 봉투 JSON)을 [self:read] 가 통화로 복원**(왕복 대칭 실측 ✓)하므로 비싼 중간 결과(증류 27행)를 파일로 접고 뒷단만 재실행하는 v2b(12문장) 패턴이 성립. ④유튜브 자막은 플래키(같은 영상이 v1 성공·v2 실패) — 실패 영상 교체·재시도는 기존 판정대로 에이전트 몫 경계. ⑤ticket 은 hex 8~32자만 받는다. ⑥uvicorn 리로드가 라이브 실행 중이면 옛 워커가 완주하고 새 프로세스가 표면을 받는다 — 완주는 됐지만 실행 중 backend 파일 편집은 피할 것(이번엔 운). ⑦brief 산문은 봉투 요약에서 160자 절단 — 산문의 정본은 document/파일이지 봉투가 아니다.

### 동향·부동산 완성 프로그램 — 세 보고서 검증 완료 (2026-08-28, 하네스 세션 후속)
- **동향(v1→v3, 24문장 54~60step)**: 기준선 4원장 읽기($변수 주입) → 3섹션 검색(queries 배열 팬아웃) → date 술어 신선도 → 섹션 ai(직전 보고서·커버리지 원장 주입, NEW 하드룰) → NEW 핵심 기사 crawl 팬아웃 → 게이트([if: $신규합 == 0] 스캔로그 분기, 식 할당 `$투자신규.count + $기술신규.count` 실사용) → 산문 7편 → 조판. **산출물이 §3 델타 서사를 실제로 수행** — 직전 호 날짜 참조·커버리지 갭 적발(Grok 4.6)·NEW 라벨의 date 근거·방법론 자기 점검의 규칙별 실행 관측·존속 판정·지켜볼 점 은퇴 규칙까지. v2=crawl 원문 반영으로 깊이 확보, v3=문체 수리(반말 0건 실측).
- **부동산(21문장 44step)**: `$회전.queue` 파이프 머리로 순회 커서 선출(explore_first 규칙을 not_in/in 술어로) → [try]{지역DB read}[catch]{time} 첫방문 분기 → naver 4-branch(deposit_min/max 서버 필터) + molit 병행 → 철학 5축 ai 평가(50행, 통과 6) → 지역문·매물문·§2b 대조문·산문 → **md+HTML 이중 렌더**(공유창고 규약 실증) → 평가/통과 델타 + rotation 커서 전진 edit. 산출물 = 5축 풀 분석+실거래 표(전환율 계산)+관심 판정+재평가 트리거+매물 6건 골격 완전 준수+삼단 대조("이지선다" 판정). 세 보고서 중 최고 품질.
- **이 회차의 수리(전부 push)**: ③gnews·naver date ISO+행 생성자 한 벌(`f6939b65`) ④queries 선언 [string,array](`67ccb5c4` 부근) — 상세는 각 커밋. 관찰 추가: ⑧임의 JSON 읽기는 dict 봉투 — 배열 필드는 `$변수.경로` 파이프 머리로 방출(rotation.queue 실측). ⑨날짜 필터는 값·리터럴 aware 정합 필수(naive 혼합=8/27 판정대로 정직 거절 — 리터럴을 `…T00:00:00Z` 로). ⑩지시문 안에 JSON 을 박으면 날 따옴표로 파서가 정직 거절 — 재료 주입은 $변수 or 따옴표 없는 평문으로. ⑪blocks 조건부 절(함의 §3-5 "생략" 규약)이 언어에 없다 — 절 제목을 산문에 위임하는 우회, 정식 표현은 설계 후보. ⑫YAML 설명문 속 "콜론+공백"이 패키지 전체를 죽인다 — pre-commit 관문이 적발(`522c3cd1`).
- **판정: 세 일간 보고서 모두, 가이드 품질 기준을 충족하는 완성 산출물을 내는 IBL 프로그램이 검증됐다** (사본 실증 — 실원장·정본 outputs 무접촉). 남은 에이전트 몫 경계: 자막 실패 영상 교체(팁)·조건부 작성 게이트의 최종 판단과 방법론 원장 갱신(동향)·DB upsert 정식 반영과 커서 규칙 0-4 완전 구현(부동산)·특별기고 처리. **스케줄(정기보고) 편입 여부 = 사용자 판정 대기.** 프로그램 원문·산출물 보존 = `data/_backups/2026-08-28_report_program_experiments/` (스크래치는 휘발).

### 언어 개정 4건 일괄 집행 — "에이전트 몫 경계" 재분류 (2026-08-28, 사용자 판정 "언어의 한계 부분은 다 고쳐", `ee3201e1`)
- 완성 프로그램 아크가 경계로 남긴 것을 셋으로 갈라 보고(①언어 공백 ②헌법상 분업 ③조합 미완), ①의 4건을 판정받아 집행:
  ㉠**each `on_error:"keep"`** — 실패 행이 `{원 행…, _error}` 로 통화에 흐른다(전량 실패 포함·opt-in·진단층 불변). 자막 실패 영상 교체가 이제 `keep → filter _error exists → 재팬아웃` 조합이 된다.
  ㉡**if 파이프 불일치=통과**(`_if_skipped` 표식·은퇴 등록 if-nomatch-message-envelope) — 빈 통화 폴백 `>> [if: empty($items)]{대안}` 성립. 부동산 커서 규칙 0→1→2 사슬이 문장이 된다.
  ㉢**blocks `when` 조건부 절**(+`blocks_omitted` 신고) — 함의·특별기고의 "없으면 섹션 생략" 규약이 blocks 로.
  ㉣**괄호형 확장 경로** — `${x.items.*.f}` 열 벡터(변수 안티조인 성립)·끝 `?` 옵셔널(결측 경로·미기록 변수=빈 값/빈 통화).
- ★교훈: 경로 문법은 **발견(ibl_vars)·해석(field_path)·자리표(_STEP_RESULT_RE) 세 자리가 한 벌** — 자리표만 좁게 남으면 확장 참조가 리터럴 `{{_step_…}}` 로 하류에 새는 침묵 실패(라이브 실증이 적발, 두 자리만 고친 첫 프로브에서 재현). ★uvicorn 부모·자식이 둘 다 8765 를 리슨하는 것은 정상 구조다 — 고스트 오진 말 것(이번엔 진짜 원인이 자리표 방언이었다).
- 가드 L1~L12(`test_language_revision_2026_08_28.py`)·배터리 green·라이브 실증 4/4. 남은 에이전트 몫 = ②헌법상 분업(JSON 원장=`[self:script]` 등록 몫·발행 승인=신뢰 문제)과 ③조합 노력뿐 — **언어 탓 경계는 소거됐다.**

### 53회차 — 축적 왕복(sink→source) 격자 (2026-09-02, 훈련 턴 · 무수정 — 보고서 `outputs/imagination_training/2026-09-02_53회차.md`)
- 축: 축적 어휘 8싱크(write/spreadsheet/sheet/ledger/notebook/memory/finance/원장 누적 관용구) × 되읽기 × 소비 5종 = 36칸 전수 실행 + 격리 34칸. 근거: 47회차 축적 8건이 전부 "검수만"이었고 행동 지표에서 축적·시간 문형이 각 2 로 최저. 깨끗 24 · 꼬임 4 · 결함 6 · 내 오류 2. 리허설 격리 실측 training 227/usage 23.
- **밭 이관 2건(두 번째 발견)**: `B53-1` dry-run 이 `$변수[.경로] >>` 파이프 머리에 거짓 빨강(`api_ibl.py:584`, 실행은 ✓ — B49-1 속) → **검수↔실행 정합 census+관문**. `B53-2`/`B53-3` 고전 변환자의 `items:"$변수"` 주입이 columns/rows 봉투를 못 읽고(`data-ops/handler.py:63-73` 가 `{"items":obj}` 로 감싸 `derive_items` 미경유 — brief·파이프 이음매는 ✓) 오류문이 자기모순("받은 봉투의 키: ['items']") + `content:"$변수"` 가 통화 없는 원형·선별 전 `data` 를 저장(B51-1 속) → **items 주입 자리 census+관문**.
- 수리성: `B53-4` 파이프 싱크가 정직 표지(`warning`·`_untransformed`)를 원장 파일에 저장, 되읽기가 현재 사건처럼 재방출(제외 목록이 `branches_skipped` 손 목록 — `HONESTY_KEYS` 정본으로). `B53-5` memory save 가 유효집합 밖 category 를 말없이 '기타'로(`memory_db.py:40-43`) → 같은 category 검색 0건. `B53-6` finance 되읽기 items 가 카드(title/meta/summary/url)라 금액·id 없음 — 합계·조건·되먹임 삭제 전부 불가, 같은 봉투의 `table` 은 데이터(단일 통화 위반, `finance-record/handler.py:216-235`). 대조군 ledger(id)·sheet(_row)·notebook(source_id)·memory(memory_id)는 통과 — finance 만 남은 카드 통화.
- 마찰: `F53-1` 파이프 싱크 `>> write` 가 생산자 `message` 유무로 텍스트/JSON 을 가른다(11회차 판정) — 원장 누적 관용구 `$본.items & (새 조회 >> …) >> union >> dedup >> write` 는 JSON 시드에서 멱등(4/4)이나 첫 저장을 JSON 으로 만드는 스위치가 언어에 없다(우회=변환자 하나) → **판정 V53-1**(PENDING). `F53-2` 교재 드리프트: read xlsx "TSV 텍스트"(실측 items 통화) · 08-27 후보 ① "`$변수 >> 액션` 불가" 는 실측으로 닫힘(성공) — 교재엔 파이프 머리 `$변수` 가 없음. `F53-3` 같은 xlsx 셀을 read 는 숫자·sheet find 는 문자열로(독자 타입 census 후보). `F53-4` 문자열 where 의 `${…*.f}` 오류문이 구조형 정답을 안 가리킴.
- 관찰: B51-4(passthrough 를 "부분 실패·절단"으로 부르는 승격) 미수리 상태 재확인 · 효과 봉투 id 되먹임(`$b.business.id`)·파일 변수 `count()`/경로·09-01 `$변수.경로 & (괄호)` 전부 정상.

#### 53회차 후속 — V53-1 ⓑ 집행 + 원장 12항목 전수 수리 (2026-09-02 `#repair` 턴, 사용자 판정 "ⓑ 채택, 나머지 다 고쳐")
- **밭 이관 2건 관문 신설**: `scripts/check_validate_parity.py`(검수↔실행 정합 — fixture 134·교재 9 전수, 파싱되고 액션이 사전에 있는데 valid:false 면 차단; 본체 `api_ibl.validate_code()` 를 라우터와 공유) · `scripts/check_items_injection.py`(AST — items 파라미터를 읽는 함수는 `coerce_items_payload` 경유, 221 파일). 둘 다 pre-commit.
- 수리: B53-2(data-ops items 주입 게이트 위임·`currency_shape_note`) · B53-3(행 모양 형제 dict 자백 + `_v4_var_payload` 파생) · B53-4(싱크가 `HONESTY_KEYS`+`_`메타 제외·`excluded_meta` 신고) · B53-5(memory category 정규화 신고·검색 거절) · B53-6(finance items 데이터 필드) · F53-2/3/4 · B51-4(`HONESTY_ROUTE_KEYS`·`describe_promoted`) · **V53-1 ⓑ** `[self:write]{format:"json"}`(교재·카탈로그·retired 등록). 1500줄 규칙으로 `sink_ops.py`·`envelope_scope.py` 분리.
- ★경계 교훈: `_get_items` 에 columns/rows 파생을 넣자 38회차 계약(표형 유지·혼합 거절) 가드 7건이 빨강 — 되읽기 게이트는 **주입 경로**에만 선다. 가드가 그 경계를 적는다(`test_B53_2_fix_lives_at_injection_not_in_get_items`).
- ★동시 세션 사건: 시스템 자기수리가 이 보고서를 읽고 병렬 커밋(`cf0d175d`·`dc22e688`·`9c0babf3`) — 공유 인덱스라 이 턴의 편집 일부가 그쪽 커밋에 실렸다(내용 동일). 배터리 1345 passed · 잔여 2 failed 는 그 세션의 미커밋 작업(json원장 args 드리프트·`test_self_reflect_gate.py` `__main__`).
- **시딩(같은 날, 사용자 지시)**: 53회차 시드 7건 `scripts/seed_imagination_round_53.py`(축적 되읽기·원장 안티조인 구조형 where·원장 누적 `format:"json"`·스냅샷 델타·id 되먹임). 시드 검증이 적발한 곁가지 **S53** — sheet append 0행 도착이 거절이던 것을 빈손 성공으로(F20-3 판정 적용, 가드 동반).

### 54회차 — 시간 왕복(defer→fire) 격자 + 같은 세션 수리 (2026-09-02 — 보고서 `outputs/imagination_training/2026-09-02_54회차.md`)
- 축: 미루는 어휘 4종(trigger·schedule·manage_events·workflow 호출) × 보존 차원 6종(프로젝트 문맥·$변수/중첩 인용·결과 행선지·실패 정직성·cron/시각 표현력·되읽기 통화) = 27칸 + 격리 9칸. 등록만 보던 과거 회차와 달리 **발화 시점**(스케줄러 틱·타이머·run_now)까지 실측. 깨끗 12 · 꼬임 3 · 결함 8 · 내 오류 1(do 안 큰따옴표 겹침 — 파서 정직 거절).
- 결함: `B54-1` 트리거가 발화 시 등록 프로젝트를 모름(레거시 `"."` → 패키지 도구 전부 거절, `project_id` 손 명시만 생존) · `B54-2` 표면 `[self:schedule]` 이 시스템 AI 소유로 귀속(`api_ibl` 기본 agent_id "system_ai" 가 경로보다 먼저) → 발화가 시스템 AI **채팅 LLM 턴**(대화 2행 실측) · `B54-3` 지연 결과 전달 함수 미존재(AttributeError 삼킴·실패 무전달) · `B54-4` **같은 분 due 작업 중 하나만 돈다**(`_save_config` 공유 임시 파일명 경주, "작업 시작" 로그 앞에서 죽음 — 라이브 7/4·재시험 4/1·격리 4/1; `add_history` 무잠금 → 이력 유실) · `B54-5` `manage_events{do:IBL}` 가 액션 이름으로 저장(발화 "알 수 없는 작업", run_now 는 성공 보고) · `B54-6` `run_count/last_run` 죽은 필드 · `B54-7` 간격형 cron 분 침묵 소실(B9 속) · `B54-8` `config` 무검증 + 가이드가 안 도는 형태(`weekdays:["mon"]`·`once`)를 가르침.
- 마찰: `F54-1` 평일 cron `1-5` 미지원+검수 사각 · `F54-2` 등록 즉시 따라잡기 발화 · `F54-3` `date "… HH:MM"` 원문 저장→침묵 미발화(시간 표기 파서 둘) · `F54-4` 이력이 통화 아님 · `F54-5` do 속 `$변수` 등록 시점 동결(교재 부재).
- **같은 세션 수리 전수(사용자 위임 "네 추천대로")**: 등록 프로젝트 저장·소유자 경로 발화(B54-1/2) · `_deliver_result_to_chat` 신설(B54-3) · 잠금+고유 임시 파일 부류 스윕 `calendar_manager`/`safe_store`/`trigger_engine`(B54-4) · IBL do→run_pipeline 정규화+`explain_run_now`(B54-5) · 이력이 레코드 갱신·통화 필드(B54-6/F54-4) · cron 요일 범위/콤마/`*/N`/이름 + 간격형 분 0 강제 + 검수가 같은 파서로 판정(`cron_to_config` 공개명, F54-1/B54-7) · `normalize_schedule_config` 한 벌 + 가이드 재작성 + `retired_contracts` 등록(B54-8, 관문이 startup.md 잔재 적발) · 첫 발화 규칙 `created_at`(F54-2, 지난 `at:` 거절) · `_split_date_time`(F54-3). 가드 `test_imagination_round54_repairs.py` 19건 · 배터리 1388 passed · 라이브 종단 `_probe54b.py` 전 항목 ✓.
- ★교훈: **발화는 두 번째 실행기다** — 등록에서 통과한 문장이 발화에서 문맥(프로젝트·행위자·origin)을 잃는다. 리허설의 `origin:"training"` 도 시간 경계를 못 넘어 건강 원장·시스템 AI 채팅에 샜다(훈련 가이드 §3-5 위생 규약 신설). ★스레드 경주는 "작업 시작" 로그 앞에서 죽으면 흔적이 없다 — 격리 재현이 유일한 눈이었다.
- **시딩(같은 날, 사용자 지시)**: 54회차 시드 8건 `scripts/seed_imagination_round_54.py`(trigger list/history·manage_events list 되읽기 filter/select · 평일 cron `1-5` 알림/장부 append 트리거 · `$t.trigger.id` 되먹임 update · `[self:schedule]{minutes}` 지연 JSON 저장 · `[self:manage_events]{do}` 실행 이벤트) — 전건 validate + 스크래치 라이브 8/8 뒤 `add_examples_batch(manual_seed)`, `ibl_distilled.json` 동승.

### 55회차 — 설계 왕복(engines:arch_* 10종, 미조합 100%) (2026-09-06, 훈련 턴 · 무수정 — 보고서 `outputs/imagination_training/2026-09-06_55회차.md`)
- 축: 건축 어휘 10종 × 조합 7차원(생성→식별자 되먹임 · 목록 팬아웃 · 스칼라 보고서의 통화 진입 · 효과(도면)의 행선지 · 조건 분기 · 파생 되먹임 · 문서 산출) = 19과제(실측 17 + 검수 2). 선정 근거=arch 가족이 미조합 액션 중 **유일하게 교재에도 조합 0**, 사용자 `건축` 프로젝트 실물 5설계 접지, 09-06 렌더 관문 출하. 깨끗 7 · 꼬임 6 · 불가 5 · 내 오류 1.
- **한 줄 진단**: 10종 중 items 를 내는 것은 `arch_list` 하나 — 나머지 9종은 scalar/effect 라 변환자·emitter·이항 결합·each 수집 **어디에도 못 들어간다**. "미조합"은 게으름이 아니라 통화 경계의 구조적 배제였다.
- 갭: `G55-1` **`[table:each]` 가 do 의 스칼라·효과 결과를 통화에 접붙일 수 없다**(면적표 2건·도면 4장이 계산·생성되고 값만 폐기, passthrough_rows 만 남음) — 대조로 **같은 액션을 `&`+union 으로 부르면 효과 봉투가 행으로 보존**된다(두 조합자가 같은 봉투를 정반대로 처리). 우회는 do 다문장+`[self:struct]` AI 되파싱뿐 = **이미 float 인 값을 행당 LLM 한 번으로 되사는 구조**(실측 3초/행) → **판정 G55-1**(권고 ⓐ `collect: true` 신설, 비파괴). `G55-2` items→**image 블록 N개** 표현 없음(자동 블록화=cards 뿐): union 으로 모은 도면 2행이 문서에서 경로 표로 렌더, 리터럴 blocks + `${변수.path}` 손타이핑만 그림이 됨 → **판정 G55-2**(권고 채택).
- 결함: `B55-1` **`arch_create`·`arch_modify` 가 dry-run 안전 라벨 "read"** — 실제로는 설계 JSON 을 생성·수정한다(이번 회차가 그 라벨로 3개 만들었다). 뿌리=`ibl_safety` 가 스스로 적어 둔 '정직한 예외'(`side_effect: true` 선언)를 패키지가 안 씀. 소비자=조종실 dry-run·`ibl_health_check` read-only 게이팅·훈련 규약의 판단 근거. 부류 census=실물 2건(작음) → 수리는 선언 + **category create/update/delete 인데 side_effect 미선언이면 빌드 실패**하는 관문까지.
- 마찰: `F55-1` scalar/effect 액션의 **반환 키가 교재에 없다** — ⟨열⟩ 지식 `data/ibl_return_shapes.json` 은 kind 분포 `{items:122}`·engines 0건이라 `$변수.경로` 우회를 쓰려면 매번 한 번 돌려 눈으로 읽어야(shape sweep 확장 대상). `F55-2` `arch_list` 는 `id`, 나머지 9종은 `design_id`(팬아웃마다 손 번역 — 3회차 F1 병기 원칙 적용 후보). `F55-3` 관찰: 같은 액션의 봉투 모양이 report_type 에 따라 message/effect 로 갈림(두 번째 발견이면 census 이관).
- 위생: 실측 전량 `origin:"training"` — 회차 창 `action_health` **training 63 · usage 0**. 스크래치 설계 3건·PNG 5장 삭제로 `건축/outputs/house-designs` 39→31(회차 전과 동일), 사용자 설계 5건 무손상, 알림 1건 생성 후 삭제. 라이브 코어 편집 0.
- **집행(같은 날 밤, 사용자 판정 "arch 엔 관심 없다 — 보편적이면 수리" → "국소만 빼고 다 고쳐"; F55-2/F55-3 arch 국소 제외)**: G55-1 `[table:each]{collect: true}` — 효과·스칼라 결과의 행 승격, 승격기 `common.currency.effect_row` 단일화(union `_effect_row` 위임) · G55-2 `[table:document]{as: "images", src_field, caption_field}`(+cards/table 강제, `images`·`src_field`·`images_skipped` 신고, 경로 칸 전무=정직 실패) · B55-1 `side_effect: true` 선언(arch_create·arch_modify) + **부작용 정직성 관문** `scripts/iblbuild_side_effect_scan.py`(도구 함수 해소 def/맵/인라인 if-체인 → 도달 가능한 쓰기 원시 AST 스캔, 첫 census 17건 중 오탐 15=str.replace·list.append 를 os/shutil 수신자 한정으로 제거 → 실물 arch 2 + 캐시성 5(web search/crawl·arch get/report/list)는 `side_effect: false` 사유 선언, 미상 0) · F55-1 shape 스윕의 `kind: scalar` ⟨키⟩(최상위+한 겹), `action_health.keys` 열 + 엔진 기록, `--from-health` 실사용 수확(주간 스윕 동승), 카탈로그 ⟨열⟩/⟨키⟩ 라벨 분리, 정적 검사기 ⟨키⟩ 무시. 가드 `backend/test_imagination_round55_repairs.py` 14건.

### 56회차 — 현재 값·함수 문법과 기존 도구의 연결 (2026-09-25, 훈련 턴 · 무수정)
- 보고서·증거 정본: `docs/experiments/imagination_training_2026_09_25/`의 `report.md`·`evidence.json`. 가이드 규정 출력 사본: `outputs/imagination_training/2026-09-25_56회차.md`. 미조합 메뉴에서 생활·콘텐츠 12과제(전건 검사, 10실행)를 구성: 기대 결과 7건, 실패 3건, 예약·발신 검수만 2건. 격리·대조·건강 조회까지 HTTP 52요청.
- **B56-1 발견**: `table:join` 명시 left/right가 목록·Record를 타입 미선언으로 거절(`tool.json`의 해당 필드에 description만 있음). 현재 파이프도 입력 자리 없음. 동일 입력을 `json(...)` 문자열로 감싸면 성공하고 기존 판본 병렬 공급도 성공 → 결합 알고리즘이 아니라 선언/라우팅 경계. 수리성.
...
### 58회차 — 2배 훈련·단항 도구 연결 전수 수리 (2026-09-25)
- 기본 상한 12×2=24과제 검사, 20과제 실제 실행. 지출·매물·강의·콘텐츠의 6문형을 다뤘다. 7과제에서 파이프 입력 누락을 재현했고, 3개 작성 오류는 제품 결함과 구분해 교정했다. 최종 24검사·20실행 기대값 통과, 전 요청 origin=training.
- **B58-1(B56-1 부류 확장) 수리**: 19개 flow 전수 조사 후 빠진 10개 단항 어휘의 명시 입력 계약 연결. 단항 flow의 입력 자리·봉투 근거 선언을 빌드 관문이 강제한다.
- **B58-2 수리**: 명시 input_envelopes의 원천 미완료 근거를 공통 어댑터가 보존. 잎 도구의 행 정규화가 표지를 버려도 부분 결과로 보고하며 업무 행·의도한 selection은 유지한다.
- **B58-3 수리**: chunk의 0행 거절·잘못된 입력 NameError·본문 없는 행의 조용한 생략을 수리. 생략 수·인덱스를 신고하고 현재 판본에서 PARTIAL_SOURCE로 연결한다.
- 새 회귀 40개, 추가 라이브 경계 4개 통과. 외부 웹·모델 호출·자료 쓰기·발신·예약·알림·해마 시딩 없음. peek 기준선 0행 확인. 상세·전체 회귀: `docs/experiments/imagination_round58_2026_09_25/report.md`. **발견 부류 3건 미수리 0.**

### 59회차 — 2배 훈련·중첩 빈값과 원천 근거 수리 (2026-09-25)

- 기본 상한 12×2=24과제 검수·20실행. 초기 기대값 5실패에서 수리 후 44검사 모두 통과.
- B59-1: flatten의 입력 존재와 출력 기수 혼동을 분리해 모든 참고문헌이 빈 경우 정상 0건 처리.
- B59-2: 생략 행을 공통 rows_dropped로 신고하고 소비한 items 봉투의 실패·절단을 row_honesty로 보존. 자동 승격과 명시 경로·숫자·벡터 접근은 정본 경로 관찰 훅으로 함께 처리.
- B59-3: 중첩 rows_dropped 완료 근거를 수집하고, 출처별 절단 범위를 판정해 다른 출처의 selection이 unknown 절단을 가리지 못하게 수리.

### 60회차 — 2배 훈련·반복 누적과 조건·변수 수리 (2026-09-25)

- 24과제·24검수·20실행. 같은 문장 전후 대조 36/44 → 44/44 통과. 보고서: [60회차](experiments/imagination_round60_2026_09_25/report.md).
- B60-1: 미확정 누적값·콜백 인자의 +를 숫자로 조기 확정하지 않고 실행 시 검사. 반복·reduce·람다·함수의 목록/문자열 연결 복구.
- B60-2: while 조건 전에 현재 i를 공급. 횟수 식의 바깥 범위 평가와 중첩 i 복원 유지. 아직 없는 i를 횟수로 쓰는 검수 오통과도 차단.
- B60-3: until 본문→조건과 고정 양수 반복의 확정 대입을 검사기에 반영. 0회 경로·조건부 대입·each 읽기 전용·예산은 유지. 새 회귀 68개 통과.
- 새 회귀 36개, 관련 121개, 관리 기록/API 29개 통과. 전체 backend 6067 통과·1 skip. 실제 API 전후와 추가 catch 복구 검증 완료. 상세: `docs/experiments/imagination_round59_2026_09_25/report.md`. **발견 부류 3건 미수리 0.**

### 61회차 — 2배 훈련·조건부 타입과 반환·복구 검사 (2026-09-25)

- 24과제·24검수·20실행. 전후 30/44 → 44/44 통과. [보고서](experiments/imagination_round61_2026_09_25/report.md).
- B61-1: Union 대안을 공통 순회·평탄화하고 보간·필드/인덱스·연산 검사를 각 값 계약에 맞춤. 동적 문자열 키는 실행 시 필드 존재 검사.
- B61-2: 무조건 반환한 뒤의 코드를 반환형·진행 환경에서 제외하되 정적 오류 검사는 유지. 양수 반복·until의 확정 반환과 함수·each·병렬·프로그램 프레임 검사.
- B61-3: catch/finally에서 실패 직전까지 재대입된 기존 값의 타입을 실행 시 검사. 대입 전후·복구 도중 실패와 새 변수 미정의 검사 유지.
- 새 회귀 51개·기존 핵심과 합산 228개 통과. 발신·저장·예약·알림은 검수만. 발견 부류 3건 미수리 0.


### 62회차 — 2배 훈련·내장 함수 전달과 단일 목록 (2026-09-25)

- 24과제·24검수·20실행, 동일 문장 전후 37/44 → 44/44 통과. [보고서](experiments/imagination_round62_2026_09_25/report.md).
- B62-1 수리: Callable로 검수한 내장 함수가 값 평가에서 IR 오류로 멈추던 경로를 공통 호출에 연결. 16종 내장 함수×4맥락 census와 동적 인자 수·예산·근거·지문·전송 경계 검사.
- B62-2 수리: `[true]`·`[false]`·`[null]`·`[abs]`를 명령으로 오인하던 파서의 헤드 분류를 수리. 기존 제어문·중첩 목록 검사, 라이브 격리 4/4 통과.
- 새 회귀 106개·관련 279개 통과. 외부 발신·자료 쓰기·예약·알림·시딩 없음. 발견 부류 2건 미수리 0.


### 63회차 — 2배 훈련·잘못된 행과 병렬 결과 타입 (2026-09-25)

- 24과제·24검수·20실행, 동일 문장 전후 39/44 → 44/44 기대값 통과. [보고서](experiments/imagination_round63_2026_09_25/report.md).
- B63-1 수리: reduce의 객체 필터링과 inner join의 비객체 continue가 행을 버리고 성공하던 경로를 입력 사전검사로 변경. 원래 인덱스·좌우 출처 신고, 빈 목록·유효 결합 계약 유지. reduce 30맥락·join 24맥락과 형제 변환자의 행 보존/거절 census를 회귀로 고정.
- B63-2 수리: 고정 목록·병렬 결과·명시 입력의 위치별 타입을 보존. 제어 합류와 목록 연결을 구분하고 함수·레코드 전달에도 적용. 동적/미확정 순서는 기존 공통 타입 검사 유지.
- 새 회귀 75개. 발신·저장·예약·알림은 검수만, 사용자 데이터 변경·시딩 없음. 발견 두 부류 미수리 0.


### 64회차 — 2배 훈련·수집한 함수의 재사용 (2026-09-25)

- 24과제·24검수·20실행, 동일 문장 전후 40/44 → 44/44 기대값 통과. [보고서](experiments/imagination_round64_2026_09_25/report.md).
- B64-1 수리: 콜백의 전체 env 복사를 자유 변수 캡처로 변경. 중첩 람다의 이름 가림·생성 시점 값·실패 근거를 유지하고 무관한 입력이 요청 지문에 끼어들지 않도록 검사.
- B64-2 수리: 내부 요청 지문에서 ResultValue 안의 함수·중첩 컨테이너를 재귀 변환. 공개 wire의 함수 거절과 실제 인자 수 오류는 유지.
- 새 회귀 29개·관련 229개 통과. 발신·저장·예약·알림은 검수만. 발견한 두 원인 미수리 0.


### 65회차 — 2배 훈련·결합 열의 정체성과 위치 (2026-09-25)

- 24과제·24검수·20실행, 동일 원문 37/44 → 44/44 기대값 통과. [보고서](experiments/imagination_round65_2026_09_25/report.md).
- B65-1 수리: inner/items 결합의 충돌 열 이름을 행 쌍마다 다시 정하지 않고 입력 전체 스키마에서 한 번 결정. 희소 행의 출처 열 변경과 후속 조회 실패 방지.
- B65-2 수리: inner/table의 왼쪽 셀을 선언 열 수에 맞게 읽어 생략된 후행 셀을 null로 보완. 오른쪽 값의 열 이동 방지.
- 결합 모드·희소 필드·행 길이·접미사·중복 키·입력 불변성 census를 새 회귀 46개로 고정, 관련 302개 통과. 발신·저장·예약·알림은 검수만. 발견 두 원인 미수리 0.


### 66회차 — 2배 훈련·정렬 기준 누락의 거짓 성공 (2026-09-26)
