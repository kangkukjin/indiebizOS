...
[ibl_v2 가이드](../../data/guides/ibl_v2.md), 설계/구현 경계는
...
기존 관용구·스케줄·앱은 판본 1로 보존한다. 새 조합은 판본 2 교재로 작성한다.
판본 2 함수는 workflow의 새 id 또는 해마의 판본별 정의로 저장하며, 검사·회상·증류·실적도 판본을 구분한다.
명시 계약이 없는 어휘와 기존 함수는 원래 JSON 봉투를 반환하는 호환 어댑터로 연결한다.
모델의 첫 생성 성공률·품질/시간/전체 토큰 우위는 미측정이며, 별도 비교 실험은 수행하지 않았다.
운영 연결과 이관 기록은 [후속 구현](../../docs/IBL_V2_ASSET_INTEGRATION_2026_09_23.md)에 있다.
...
[입력·불명·배치 계약](../../data/guides/judgment.md) · [설계](../../docs/JEV_JUDGMENT_2026_09_21.md).
...
**수집 구분·문맥 선택(2026-09-15)**: `sense:crawl{op}`은 기본 `content`와 `links`·`metadata`를 선언하며 같은 스냅샷에서 보기를 고른다. 메타정보는 원문 값·출처·충돌을 보존한다. 기능어 `table:filter`의 선택 인자 `context:{before,after,by,limit}`은 같은 그룹의 앞뒤 행·실패 행·일치 계수를 보존한다. 기존 호출 의미는 유지하며 파서 문법·노드·액션은 추가하지 않는다. 이를 인자로 특화한 `[fn:본문에서찾기]`를 수동 선정집에 등록한다. 한 문장이라도 선정집의 `always_on` 본문과 정확히 일치한 관용구는 지도에 노출한다. [설계와 검증](../../docs/WEB_COLLECTION_RESOLUTION_2026_09_15.md).
...
**개인 사전** — 필수 공급자 밖의 내용어. sense·limbs·engines뿐 아니라 self의 음악·블로그처럼 표준 노드에 속하는 선택 패키지도 포함한다. 정의(`ibl_nodes_src/`·패키지 `ibl_actions.yaml`)·구현(패키지 핸들러)·파라미터 별칭(`aliases:`)·프롬프트 설명까지 전부 데이터가 소유한다.

**어휘 레고박스 보호 기준 (2026-09-13)**: `data/vocabulary_policy.yaml`이 표준 노드와 필수 공급자의 선언을 소유하고, 노드 yaml의 `always_on`과 빌드가 대조한다. `always_on`은 노드 선별에서 유지한다는 뜻이며 그 노드 안의 모든 내용어 패키지가 필수라는 뜻은 아니다. 필수 공급자는 기존 UI 분류의 ibl-core·system_essentials·data-ops·visualization을 공통 선언으로 옮겼다. 필수/선택 기능이 섞인 system_essentials는 묶음 전체를 보호한다. 배포 출처를 나타내는 core_manifest는 제거 금지 목록이 아니다. 파서·문법은 바꾸지 않는다.
...
2026-09-13 함수 입력 연결은 위 문법 층의 명시적 개정이다. 입력 변수의 이름이나 도메인 어휘를 엔진에 넣지 않고 첫 파이프의 구조로만 연결한다. 파서 표기·desugar 타깃·always_on·STANDARD_CORE_NODES의 노드 집합은 바뀌지 않는다.
...
`self:script`의 `input_as`는 실행 파이프 봉투 전체를 stdin의 지정 키로 인계한다.
...
- **함수 입력 연결(2026-09-13 언어 개정)**: 프로그램 함수·관용구 본문의 첫 파이프가 `$목록 >> …`처럼 맨몸 자유 변수로 시작하면, 호출의 앞 통화가 그 변수 한 개를 채운다. `[생산자] >> [fn:이름]{나머지 인자}`와 `[fn:이름]{목록:$값, …}`를 함께 지원한다. 명시 인자가 우선하며 빈 목록도 덮어쓰지 않는다. 앞 통화가 없거나 다른 인자가 빠지면 기존처럼 오류다. 병렬·조건·경로 참조·후속 문장의 변수를 추측하지 않고, 독립 문장 경계를 넘어 입력을 받지 않는다. 저장 워크플로의 `params_default` 계약은 유지한다. 실행·교재·선정집 검증은 `workflow_contract.pipe_input_param`의 동일한 구조 판정을 쓴다. [수리 근거와 검증](../../docs/IBL_FN_PIPE_INPUT_2026_09_13.md).
...
- **노출 조정(2026-09-21 사용자 선정)**: 최근 72시간에 호출이 없던 `고치고확인하기`·`미처리만고르기`는 등록과 명시 호출을 유지하고 상시 지도·잎 액션 병기에서 제외한다. `직전보고서찾아읽기`는 전역 노출로 올리지 않고 [AI 동향 보고서 가이드](../../data/guides/ai_trend_report.md)에서 소개한다. 파일명 순서·앞 160줄 계약과 추가 열람·첫 작성 처리를 함께 안내한다. 노출 원장은 `data/idioms/curated.json`과 운영 `always_on`을 맞춘다.
...
- **파일 듣기** (2026-09-10): `[sense:listen]{path}`는 파일 전사, `{path, question}`은 소리 내용 분석, `{path, op:"inspect"}`는 원본 신호 검사다. path 생략 시 기존 마이크 동작. 파일에 마이크는 불필요하며 실행·감독은 같은 구간 분석 증거를 재사용한다. [오디오 듣기 가이드](../../data/guides/audio_listen.md).
<!-- RUNS_ON:START -->
...
입력 기아 검사는 각 독립 문장의 머리에 적용한다. 줄바꿈으로 앞 통화가 넘어오지 않는다. `same-kind`·`pair` 변환자는 확정된 단일 표 입력을 거절하며, 스칼라·미상 입력의 실행 시 구조 해소는 허용한다. 직접 입력은 사전의 `flow.input_params`(기본 슬롯 묶음), `input_alternatives`(기존 대체 슬롯 묶음), `input_bundle_param`(분기 목록 슬롯)을 읽는다. 정상 0행도 통화다. 구글뉴스는 유효한 빈 RSS를 `success:true, items:[], count:0`으로 반환하고 HTTP·수신·파싱 오류는 실패로 구분한다. 배치의 일부 실패는 성공한 행을 보존하며 `errors`에 남긴다.

**AI 호출 경계 검사**: `[table:ai]`의 선택 `contract`는 행별 중첩 배열의 전수 ID·필수 필드·허용값을 선언한다. 정적 검사는 선언과 투영 충돌, 실행은 호출 전 입력과 응답 직후 새 출력을 같은 계약으로 검사한다. `inspect:batch|each`는 준비된 실제 입력의 요청 크기·반복 배열을 모델 없이 조사하고 원본 items와 inspection을 반환한다. 자동 요약·절단하지 않으며, inspect와 criteria는 함께 쓸 수 없다. 업무 의미의 정확성·실제 토큰 절감은 별도 검증이다. [계약·사용 예·한계](../../docs/IBL_AI_BOUNDARY_CONTRACTS.md).
...
- **비변환 소비자는 `pipe_in: true` 로 선언한다**(2026-09-18): 변환자가 아닌데 앞 통화를 읽는 액션(write·copy·sheet·read·ask·struct·chart·document·spreadsheet·structure·render·image_read·os_open·output·delegate·publish·android) — 핸들러가 `_prev_result` 를 읽으면 선언한다. 정적 검사 **T3 죽은 이음매**가 이 선언을 읽는다: `A >> B` 에서 B 가 변환자도 flow·`pipe_in` 선언자도 아니고, 주 인자(target_key)를 가진 액션이 op 말고 비어 있으면 **경고** — 실측 `[sense:here] >> [sense:restaurant]{}`(query 없음으로 실패)·`… >> [table:brief] >> [self:notify_user]{}`(빈 알림). 값을 넘기는 길은 `$이름 = …` 뒤 `"$이름.필드"`, `$items`, `[table:each] { …'$it.필드'… }`. ★경고지 거절이 아니다 — `>>` 의 의존은 통화와 **성공** 둘이라 `빌드 >> 배포`·`스냅샷 >> 탭` 처럼 통화가 안 넘는 이음매는 정당하다. 없는 op 값은 타입 검사도 error 로 낸다(실행기의 확정 거절을 실행 전에, `ibl_param_vocab.unknown_op_message` 한 벌).
...
**실행 전 반복 계획 분석 (2026-09-23)**: 위 검사 창구는 `preflight`도 반환한다. 확정된 목록 건수보다 작은 each 한도(`each_input_limit`), 반복에서 바뀌지 않는 여러 행을 AI 어휘에 다시 전달하는 구조(`repeated_ai_batch`)를 위치·근거와 함께 warning으로 낸다. `ai_call`·`flow` 선언과 공통 IR을 읽으며 업무별 어휘 이름으로 분기하지 않는다. 지역·외부 함수와 중첩 반복의 선언된 AI 어휘 방문 상한을 계산하되 미상은 `null`/`unknowns`로 남긴다. 배타적 분기를 합산한 보수적 상한이며 스크립트·도구 내부 호출, 조건 판단, 재시도·토큰·시간은 포함하지 않는다. 경고는 실행을 차단하지 않고 입력 축소도 하지 않는다. 상세 범위·검증 = `docs/IBL_PREFLIGHT_ANALYSIS.md`.
...
**원장 행의 형태 (2026-09-08)**: `self:ledger`의 append/upsert는 객체 행만 받는다. 비객체를 섞은 입력은 전체 쓰기를 거절하고, 저장된 비객체도 select에서 조용히 빼지 않고 손상으로 신고한다. set의 임의 JSON 값 저장은 별도 계약이다. 닫히지 않은 객체·배열은 파싱 오류이며 문자열로 대신 넘기지 않는다. 현장 수리: `docs/AI_REPORT_REPAIR_2026_09_08.md`.

**집합과 잘림의 구분 (2026-09-08 봉투 의미 개정)**: `count`는 반환 행수, `total`은 현재 집합의 기수다. `filter`·`dedup`·`groupby`·`since`·`flatten`·중복 제거를 하는 `merge`는 집합을 다시 정의한다. 기존 `total`이 있으면 결과 행수로 갱신하며, 정상 선별만으로 `truncated`를 켜지 않는다. 70행 중 조건에 맞는 25행 전부를 냈으면 `count:25, total:25, truncated:false`다. 반면 `take`·원천 조회의 `limit`은 집합 일부를 반환하므로 기존 `total`을 유지하며 잘림을 신고한다. `sort`·`select` 등 행을 보존하는 변환자는 이 범위를 유지한다. 없는 `total`을 추측해 만들지는 않는다.

상류의 실제 `truncated:true`는 집합이 바뀌어도 보존한다. 이때 `total == count`는 **관측한 입력에서 만든 결과 기수**일 뿐, 미수집 원천까지 완전하다는 뜻이 아니다. 원천 누락은 페이지·범위를 나눠 가져와야 하며 하류 `filter/select/take`로 복구되지 않는다. 요청한 상위 N개 표본이면 추가 수집이 필요 없고, 전달 메시지만 잘렸다면 보관된 변수·파일에서 필요한 부분을 읽는다. 새 집합으로 바뀐 뒤의 상류 `summary`는 제거한다. 수리 근거: `docs/IBL_POPULATION_SCOPE_REPAIR_2026_09_08.md`.

리터럴 행도 별도 생성 동작 없이 첫 변환자의 `items:`로 직접 넣는다: `[table:compute]{items:[{보증금:9000, 면적:99}], set:{㎡당만원:"round(보증금 / 면적, 0)"}}`. `each`는 행마다 실제 동작이 필요할 때 쓴다.
...
- **단항**(앞 결과 1개): `filter{where}` · `sort{by, desc}` · `take{n}` · `select{columns}` · `dedup{by}` · `groupby{by, agg}` — 단항은 `|` 단축 문법 지원. `dedup`의 `key`는 `by`의 별칭이며 단일·복합키 모두 같은 정규화 경로를 쓴다.
...
실행 봉투의 `results[]`는 중간 단계, `final_result`는 최종 반환값이다. 모델·MCP 도구에는
...
가리킨다. 원장 10행 뒤 `take{n:-4}`의 결과는 마지막 4행이며, 앞 단계의 10행은
그 변환이 실패했다는 뜻이 아니다. 저장된 결과는 `ref.path`로 읽고 재실행하지 않는다.

표시 정책은 `lifecycle_policy.yaml/envelope_preview`를 `model_result_view.project_result`가
읽어 적용한다. 단계·오류 진단의 키와 상태는 유지하고 긴 문자열만 명시적 미리보기로 접는다.
API 공급자 6개와 MCP는 `ibl_result_transport`를 공유한다. 한도 안에서는 전달 바이트를
보존하고, 초과하면 원문 저장 후 구조를 축약한다. 행 축소본의 `transport_ref`는 전송 직전
원문, `result_ref`는 최초 실행 원문이며 서로 덮지 않는다. 큰 오류의 `_transport_omitted`는
상세가 참조에 있음을 알린다. 전송 spill의 24시간 보관은 모델 표시 예산과 별도 계약이다.

함수 호출의 실행 기록과 반환값도 구분한다. `[fn:이름] >> …`와 호출 결과를 담은
맨몸 변수는 몸이 반환한 값을 전달한다. 본문 문자열을 반환하는 함수 뒤에
`self:write`를 연결하면 실행 기록 대신 본문이 저장된다. 함수의 진단은 `results[]`와
명시적 `.final_result` 접근에 남는다. 실패 봉투·트레이스백은 값으로 벗기지 않는다.
이는 기존 함수 반환 약속의 실행 경계 수리이며 새 문법이나 어휘를 추가하지 않는다.
...
턴 변수는 실제 값과 구조를 함께 보존한다. 다음 호출의 검사기는 이 구조를 이어받고, 모델에는 `turn_vars.types`로 행의 열·봉투·중첩 필드·병렬 분기를 알려준다(`live`는 기존 이름 목록). 봉투의 바깥 열과 내부 `items` 열은 다르며, 펼치거나 합치기 전에는 내부 열을 바깥 열로 간주하지 않는다. `flatten{keep:["data.symbol"]}`은 점 경로의 부모 값을 **`data.symbol`이라는 열**로 보존한다. 기존 점 포함 리터럴 키가 우선이며 충돌은 `_2` 접미사로 구별한다. 원천 액션의 보장 구조는 사전의 `result_shape`·`result_shape_variants`·`result_shape_rules`에 둔다. 관측 표본·동적 소스·알 수 없는 구조를 닫힌 열로 승격하지 않는다. [형 전달 수리와 검증](../../docs/IBL_SHAPE_HANDOFF_2026_09_12.md).
...
반복 헤더의 옵션 쉼표도 다른 IBL 경계와 같은 인용·이스케이프 규칙을 따른다. `until`/`while` 조건 문자열 안의 이스케이프된 따옴표와 쉼표는 조건의 일부이며, 뒤의 `max`·`collect` 옵션을 삼키지 않는다(2026-09-11 결함 수리). 중괄호·대괄호·소괄호 내부의 쉼표도 옵션을 나누지 않는다. 조건식 평가와 반복 상한은 기존 계약을 유지한다.
...
`each`의 do는 한 번 파싱한 실행 트리와 별도의 값으로 실행한다. 파이프 결과·함수 인자·반복 값·행 데이터를 do 소스 문자열에 삽입해 행마다 다시 파싱하지 않는다. 문자열 보간과 값 참조, 이미 데이터가 된 문자열을 내부 표현에서 구별하며, 바깥 파이프와 내부 파이프의 슬롯 번호도 별도 이름 공간을 갖는다.

이것은 기존 표기의 실행 구현 변경이다. 기존 따옴표·스칼라 호환 규칙과 관용구 정의는 유지한다. 원격 실행은 버전된 IR 부속으로 코드와 묶인 값을 함께 전달하므로 양쪽 엔진의 해당 버전 지원이 필요하다. 구현·불변식·실제 실패 재생과 검증 한계는 [코드·값 분리 설계](../../docs/IBL_TYPED_CODE_IR_2026_09_08.md)를 참고한다.

지연 코드 경계는 앞 실행 결과가 없는 첫 step에서도 세운다. 본문 안의 `$items`는 그 본문의 파이프 통화이며, 바깥 바인더가 미리 해석하지 않는다. 부분 보간도 표시용 원문이 아닌 참조 조각으로 해소한다. data-ops의 명시 통화(`items`, `table`, `inputs`, 입력 쌍)는 자동 전달되는 앞 결과보다 우선하고, 빈 목록도 명시 입력이다.

지연 본문은 작성 위치의 함수 정의를 호출할 수 있으며 지역 정의가 우선한다. 함수의 값 범위는 닫혀 있다. `$it`는 해당 each의 do에서만 지역 행 이름이고, 함수의 다른 위치에서 사용하면 명시 인자로 받아야 한다. 전역 집합 예약 이름은 `$items`다. 수리 근거와 호환 범위는 [6회차 문장 시험](../../docs/IBL_BOUNDARY_PROBE_ROUND6_2026_09_08.md)에 기록했다.

일반 파이프의 변수 보간도 같은 IR 바인더를 쓴다. 삽입한 데이터 안의 `$items`는 문자 그대로 남고, 원문에 작성된 `$items`만 다음 단계에서 해석한다. `$q = $r.queue`처럼 식으로 꺼낸 목록·객체는 진단 봉투와 구별해 보존한다. `$q >> ...`, `items:$q`, 조건·식의 `$q.경로`는 원형 값을 읽으며 목록 방출은 `{items:[...]}`가 된다. 기존 `.value` 진단 접근은 유지하되 실제 객체에 같은 키가 있으면 사용자 데이터가 우선한다. 내부 표식은 `_value_result`이며 일반 사용자 객체의 `value` 키만 보고 봉투로 판정하지 않는다. [실제 에피소드 재생과 계약](../../docs/IBL_EPISODE3176_REPAIRS_2026_09_08.md).
...
회원 액션은 파일/프로그램·재생·폰 제어의 body 경로와 감사된 라디오 검색 hub 경로다.
body의 member_transform:모듈:함수는 path_audited를 검증한 뒤 턴 임시 폴더에서 처리하고 회원 기기의
저장 영수증을 받아 결과를 반환한다. 기존 files/$file 인라인 첨부 문법은 유지한다. side_effect는 부작용 분류, lands_on은 위치, principal은 권한으로 서로 대체하지 않는다.
...
구체적 계약: [공동 업무 가이드](../../data/guides/managed_records.md).
