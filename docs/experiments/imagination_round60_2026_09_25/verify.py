"""Recompute assertions from recorded HTTP responses, independent of pass flags."""
import json
    ...
    for row in rows:
        ...
        try:
            if check:
                ...
                assert r['executed'] is True and r['success'] is True
                assert r['source_complete'] is True
                assert r['value'] == expected[name]
                if isinstance(expected[name], bool):
                    assert r['value'] is expected[name]
            passed = True
