...
- ibl_hippo_index.json : {dim, count, examples:[{id,intent,ibl_code,nodes,category,difficulty,
  source,success_count,fail_count,avg_ms,avg_tokens}]}  (벡터 행 순서와 1:1)
...
def main() -> int:
    ...
    for row in rows:
        ...
        meta = {
            ...
            # 시간·토큰 선택압(2026-08-30): 성공 비용 EWMA(-1=미측정) — 렌트 몸도 표시 규약 공유
            "avg_ms": float(row["avg_ms"]) if row["avg_ms"] is not None else -1.0,
            "avg_tokens": float(row["avg_tokens"]) if row["avg_tokens"] is not None else -1.0,
        }
