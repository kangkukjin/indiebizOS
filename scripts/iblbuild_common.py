...
PHONE_VERIFIED_PACKAGES = {
    ...
    "finance-record",  # 재무기록(self:finance save/query/sync) — health-record 와 대칭(모듈레벨 stdlib sqlite 만, ingest 의 무거운 파서는 지연 import). ★sync 가 폰서 의미를 갖는다: 결제 알림 포획소가 폰에 있으므로 폰은 adb 없이 자기 파일을 직접 읽는다(finance_sync.collect_from_phone 의 INDIEBIZ_PROFILE=phone 분기) — USB 를 기다리던 수거가 몸 안에서 끝난다. 폰↔맥 원장 머지 = finance_ledger_sync + 폰 phone_finance_sync(알림 포획 직후 자동 1왕복, 2026-09-18).
    ...
    "ai-ops",    # 원샷 낱말(self:struct·table:ai·table:brief — 2026-08-19). 모듈레벨 stdlib만(json), oneshot_facade/ingest_engine 은 함수 안 지연 import(둘 다 폰 번들 층에 실존 — 폰 원샷=deepseek_http, 비전=Gemini REST[아이콘 생성기 선례]). table:ai 가 group:transform 이라 anywhere 필수(통화는 몸 무관). 맥 인프로세스 배터리+import 시뮬 검증, ⏳A36 대표 액션 실기 확정.
