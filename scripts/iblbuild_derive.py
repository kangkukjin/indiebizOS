...
def derive_fixtures(data: dict) -> dict:
    ...
    **op 축**(2026-08-05 감사 ⑤): 액션 레벨 필드에 더해 `ops.fixture`/`ops.exempt`
    형제 맵도 모은다. op 항목의 키는 `node:action#op` — 액션 레벨 키의 접두이므로
    정렬하면 액션 바로 뒤에 자기 op 들이 붙는다.

    **변이 축**(2026-08-22, F20-1 판정): 반환 열이 op 이 아니라 **param 으로 갈리는**
    액션의 `shape_variants:` 도 모은다 — 키는 `node:action@param=값`. 별도 섹션에
    두는 이유: 건강검진·통화 스윕·returns 드리프트의 측정 우주(`fixtures`)를 넓히면
    외부 API 를 매일 더 두드리고 지표 분모도 바뀐다. 변이는 **카탈로그 ⟨열⟩ 관측
    전용**이라 그 우주와 섞지 않는다(읽는 쪽=scripts/ibl_shape_sweep.py 하나).
    """
    fixtures: dict = {}
    exempt: dict = {}
    variants: dict = {}
    ...
    for node_name, node in nodes.items():
        ...
        for action_name, action in (node.get("actions", {}) or {}).items():
            ...
            for op_name, code in sorted((ops.get("fixture") or {}).items()):
                if isinstance(code, str) and code:
                    fixtures[f"{qual}#{op_name}"] = code
            for op_name, why in sorted((ops.get("exempt") or {}).items()):
                if isinstance(why, str) and why:
                    exempt[f"{qual}#{op_name}"] = why
            sv = action.get("shape_variants")
            if isinstance(sv, dict):
                for label, code in sorted(sv.items()):
                    if isinstance(code, str) and code:
                        variants[f"{qual}@{label}"] = code
    return {
        "_comment": (
            "GENERATED by build_ibl_nodes.py — 액션의 fixture:/exempt: 필드와 "
            "ops.fixture/ops.exempt 형제 맵에서 파생(op 항목 키=node:action#op). "
            "직접 수정 금지(소스=패키지 ibl_actions.yaml / 코어 ibl_nodes_src). "
            "ibl_health_check.py 가 읽는다. shape_variants(키=node:action@param=값)는 "
            "카탈로그 ⟨열⟩ 관측 전용 — 건강검진 우주(fixtures)와 섞지 않는다."
        ),
        "fixtures": dict(sorted(fixtures.items())),
        "exempt": dict(sorted(exempt.items())),
        "shape_variants": dict(sorted(variants.items())),
    }


def derive_shell_shadow(data: dict) -> dict:
    """병합된 data 에서 액션별 `shell_shadow:` 블록을 모아 data/shell_shadow.json 을 파생 (2026-09-05).

    셸 그림자 관문(backend/base/shell_shadow_gate.py — Claude Code PreToolUse 훅·run_command)의
    **판정표**다. 낱말이 "나는 이 셸 명령의 자리다"를 자기 정의에 선언하면(heads·argmap·native·
    redirect·python_write) 빌드가 여기로 모으고, 관문은 이 표 하나만 읽는다 — 관문 코드에 낱말
    이름이 없다(헌법 '표준/사전 경계': 내용어는 데이터로만). 낱말이 은퇴·제거되면 그림자도
    함께 빠진다(고아 없음 — fixture 파생과 같은 구조).

    표 항목엔 `params`(선언된 param 이름 — 거절문의 '다른 param' 안내)를 함께 싣는다.
    """
    shadows: dict = {}
    nodes = data.get("nodes", {}) if isinstance(data, dict) else {}
    for node_name, node in nodes.items():
        ...
        for action_name, action in (node.get("actions", {}) or {}).items():
            if not isinstance(action, dict):
                continue
            ss = action.get("shell_shadow")
            if not isinstance(ss, dict):
                continue
            qual = f"{node_name}:{action_name}"
            params = []
            tk = action.get("target_key")
            if tk:
                params.append(tk)
            for p in (action.get("params") or {}).keys() if isinstance(action.get("params"), dict) else []:
                if p not in params:
                    params.append(p)
            for p in (action.get("aliases") or {}).keys() if isinstance(action.get("aliases"), dict) else []:
                if p not in params:
                    params.append(p)
            entry = {"heads": [str(h) for h in (ss.get("heads") or [])]}
            for k in ("native", "redirect", "python_write", "python_write_hint", "argmap"):
                if k in ss:
                    entry[k] = ss[k]
            entry["params"] = params
            shadows[qual] = entry
    return {
        "_comment": (
            "GENERATED by build_ibl_nodes.py — 액션의 shell_shadow: 블록에서 파생. 직접 수정 금지"
            "(소스=패키지 ibl_actions.yaml / 코어 ibl_nodes_src). backend/base/shell_shadow_gate.py 가 읽는다"
            "(Claude Code PreToolUse 훅 + in-process run_command). heads='명령 [필수 flag]', "
            "argmap=셸 인자→param 대응, native=같은 일을 하는 네이티브 도구, redirect=리다이렉션 쓰기, "
            "python_write=파일을 쓰는 인라인 파이썬."
        ),
