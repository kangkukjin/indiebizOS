...
#
# ★★편향 고지 (2026-08-15 3차 — 이 지표를 인용하기 전에 읽을 것):
#   distilled 는 *전 실행*이 아니라 **새로운 실행**만이다. ibl_usage_rag.distill_experience
#   가 `해마 점수 < DISTILL_THRESHOLD(0.7)` 일 때만 증류하기 때문 — 해마가 이미 아는
#   문장은 실행돼도 여기 안 남는다. 단발 조회는 대개 고점수라 빠지고, 새 조합은 저점수라
...
def render(full, before=None):
    ...
    for label, key in (("행동 — 새 실행만 (증류)  ※조합률 과대추정", "행동"),
                       ("교재 — 전 코퍼스", "교재")):
        print()
        print("─" * 62)
        print(f" [{label}]")
